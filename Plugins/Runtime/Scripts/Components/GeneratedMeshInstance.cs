...
namespace InternalRealtimeCSG
{
    ...
    public struct MeshInstanceKey : IEqualityComparer<MeshInstanceKey>, IEquatable<MeshInstanceKey>
    {
        ...
        {
            return new MeshInstanceKey(
                meshDescription.meshQuery,
                meshDescription.surfaceParameter,
                meshDescription.subMeshQueryIndex
            );
        }
        ...
        public bool Equals(MeshInstanceKey left, MeshInstanceKey right)
        {
            if (System.Object.ReferenceEquals(left, right))
                return true;
            if (
                System.Object.ReferenceEquals(left, null)
    ...
    public sealed class GeneratedMeshInstance : MonoBehaviour
    {
        ...
        [HideInInspector]
        public bool HasGeneratedNormals = false;
        ...
        [NonSerialized]
        [HideInInspector]
        public float ResetUVTime = float.PositiveInfinity;

        [HideInInspector]
        public Int64 LightingHashValue;
        ...
        [NonSerialized]
        [HideInInspector]
        public System.Object CachedMeshRendererSO;
        ...
        {
            if (
                (!PhysicsMaterial || PhysicsMaterial.ToNativeId() != 0)
                && (!RenderMaterial || RenderMaterial.ToNativeId() != 0)
            )
            {
                if (SharedMesh)
                {
                ...
                else if (!ReferenceEquals(SharedMesh, null) && SharedMesh.ToNativeId() != 0)
                    return false;
                return true;
            }
            return false;
        ...
        internal void Awake()
        {
            ...
            )
            {
                ...
                {
                    if (PhysicsMaterial)
                        MeshDescription.surfaceParameter = PhysicsMaterial.ToNativeId();
                }
            }
            else
            {
                ...
                {
                    if (RenderMaterial)
                        MeshDescription.surfaceParameter = RenderMaterial.ToNativeId();
                }
        ...
        internal void OnEnable()
        {
            ...
            if (transform.parent && !transform.parent.GetComponent<GeneratedMeshes>())
            {
