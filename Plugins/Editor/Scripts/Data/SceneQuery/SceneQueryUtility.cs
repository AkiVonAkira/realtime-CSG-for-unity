...
namespace InternalRealtimeCSG
{
    ...
    {
        public PointSelection(int brushNodeID, int pointIndex)
        {
    ...
    internal static class SceneQueryUtility
    {
        ...
        public static GameObject[] GetRootGameObjectsInScene(Scene scene)
        {
            ...
            var transforms = ObjectFindExtensions.FindAllTransforms();
        ...
        internal static GameObject GetUniqueHiddenGameObjectInSceneWithName(
            Scene scene,
            string name
        )
        {
            ...
            for (int i = 0; i < rootGameObjects.Length; i++)
            {
                ...
                if (root.hideFlags != HideFlags.None && root.name == name)
                {
                    if (foundRoot)
                    {
                ...
                for (int j = 0; j < rootChildren.Length; j++)
                {
                    ...
                    if (child.hideFlags == HideFlags.None || child.name != name)
                        continue;
        ...
        internal static bool GameObjectContainsAttribute<T>(GameObject go)
            where T : Attribute
        {
        ...
        public static CSGNode GetTopMostGroupForNode(CSGNode node)
        {
            ...
            while (parent)
            {
                ...
                var parentOp = parent.GetComponent<CSGOperation>();
                if (parentOp && parentOp.HandleAsOne && !parentOp.PassThrough)
        ...
        private static void DeselectAllChildBrushes(
            Transform transform,
        ...
        )
        {
            ...
            for (int i = 0, childCount = transform.childCount; i < childCount; i++)
            {
                ...
                if (
                    !childNode
                    || (childNode is CSGModel)
                    || ((1 << childNode.gameObject.layer) & visibleLayers) == 0
                )
                    continue;
        ...
        private static bool AreAllBrushesSelected(
            Transform transform,
            HashSet<GameObject> objectsInFrustum
        )
        {
            ...
            for (; i < childCount; i++)
            {
                ...
                if (childOperation == null || !childOperation.PassThrough)
                {
                    objectsInFrustum.Remove(childGameObject);
        ...
        public static bool GetItemsInFrustum(Plane[] planes, HashSet<GameObject> objectsInFrustum)
        {
            ...
            foreach (var model in InternalCSGModelManager.Models)
            {
                if (!ModelTraits.WillModelRender(model))
                    continue;
                found =
                    InternalCSGModelManager.External.GetItemsInFrustum(
                        model,
                        planes,
                        objectsInFrustum
                    ) || found;
            ...
            for (var i = items.Length - 1; i >= 0; i--)
            {
                ...
                while (true)
                {
                    ...
                    if (!parent || !AreAllBrushesSelected(parent.transform, objectsInFrustum))
                        break;
        ...
        internal static PointSelection[] GetPointsInFrustum(
            Camera camera,
            Plane[] planes,
            CSGBrush[] brushes,
        ...
        private static Vector2 _prevSceenPos = new Vector2(
        ...
        static GameObject FindFirstWorldIntersection(
            ...
            List<CSGBrush> ignoreBrushes = null,
        ...
        )
        {
            ...
            var ignoreGameObjectArray =
                (ignoreGameObjects == null || ignoreGameObjects.Count == 0)
                    ? null
                    : ignoreGameObjects.ToArray();
            {
                ...
                try
                {
                    gameObject = HandleUtility.PickGameObject(
                        screenPos,
                        ignoreGameObjectArray,
                        out materialIndex
                    );
                }
                finally
                {
                    model = EndPicking(flagState, gameObject);
                }
            }

            if (!object.Equals(gameObject, null) && model)
            {
                if (ignoreGameObjects != null && ignoreGameObjects.Contains(model.gameObject))
                {
                    Debug.Assert(false);
                }
                else
                {
                    ...
                    var ignoreBrushesArray =
                        (ignoreGameObjects == null || ignoreGameObjects.Count == 0)
                            ? null
                            : ignoreBrushes.ToArray();
                    if (
                        FindWorldIntersection(
                            model,
                            worldRayStart,
                            worldRayEnd,
                            out _deepClickIntersections,
                            ignoreInvisibleSurfaces: ignoreInvisibleSurfaces && !wireframeShown,
                            ignoreUnrenderables: !wireframeShown,
                            ignoreBrushes: ignoreBrushesArray
                        )
                    )
                    {
                        var visibleLayers = Tools.visibleLayers;
                    ...
                    if (ignoreGameObjects != null)
                    {
                        ...
                        foreach (
                            var component in model.generatedMeshes.GetComponentsInChildren<GeneratedMeshInstance>()
                        )
        ...
        public static bool FindClickWorldIntersection(
            Camera camera,
            Vector2 screenPos,
            out GameObject foundObject,
            bool ignoreInvisibleSurfaces = true,
            bool ignoreDeepClick = false
        )
        {
            MeshRenderer meshRenderer = null;
            int materialIndex = 0;
            return FindClickWorldIntersection(
                camera,
                screenPos,
                out foundObject,
                out meshRenderer,
        ...
        )
        {
            ...
            var worldRayVector = (
                worldRay.direction * (camera.farClipPlane - camera.nearClipPlane)
            );
            ...
            foundObject = FindFirstWorldIntersection(
                camera,
                screenPos,
                worldRayStart,
                worldRayEnd,
                out meshRenderer,
                out materialIndex,
                deepClickIgnoreGameObjectList,
                deepClickIgnoreBrushList,
                ignoreInvisibleSurfaces
            );
        ...
        public static LegacyBrushIntersection FindMeshIntersection(
            Camera camera,
            Vector2 screenPos,
        ...
        )
        {
            ...
            while (hit != null)
            {
                ...
                if (
                    rh.transform.GetComponent<MeshRenderer>()
                    &&
            ...
            if (
                float.IsNaN(intersectionPoint.x)
        ...
        public static bool FindUnityWorldIntersection(
            Camera camera,
            Vector2 screenPos,
            out GameObject foundObject,
            bool ignoreInvisibleSurfaces = true
        )
        {
            ...
            */
            ...
            try
            {
                gameObject = HandleUtility.PickGameObject(screenPos, false, null);
            }
            finally
            {
        ...
        public static bool FindWorldIntersection(
            Camera camera,
            Vector2 screenPos,
            out LegacyBrushIntersection intersection,
            bool ignoreInvisibleSurfaces = true,
            bool ignoreUnrenderables = true,
            CSGBrush[] ignoreBrushes = null
        )
        {
            var worldRay = HandleUtility.GUIPointToWorldRay(screenPos);
            return FindWorldIntersection(
                camera,
                worldRay,
                out intersection,
                ignoreInvisibleSurfaces,
        ...
        )
        {
            ...
            var forceIgnoreInvisibleSurfaces =
            ...
            for (var g = 0; g < InternalCSGModelManager.Models.Length; g++)
            {
                ...
                if (
                    ignoreUnrenderables
                    && !ModelTraits.WillModelRender(model)
                    && !Selection.Contains(model.gameObject.Id())
                )
                    continue;
            ...
            if (
                !InternalCSGModelManager.External.RayCastMulti(
                    foundModelCount,
                    __foundModels,
                    rayStart,
                    rayEnd,
            ...
            for (var i = 0; i < modelIntersections.Length; i++)
            {
                ...
                if (intersection != null && modelIntersection.distance > intersection.distance)
                    continue;

                var brush = modelIntersection.gameObject.GetComponent<CSGBrush>();
                if (
                    BrushTraits.IsSurfaceUnselectable(
                        brush,
                        modelIntersection.surfaceIndex,
                        brush.ChildData.Model.IsTrigger,
                        ignoreInvisibleSurfaces
                    )
                )
                    continue;
        ...
        #region FindWorldIntersection
        static bool FindWorldIntersection(
            CSGModel model,
            Vector3 worldRayStart,
            Vector3 worldRayEnd,
        ...
        )
        {
            ...
            LegacyBrushIntersection[] modelIntersections;
            if (
                !InternalCSGModelManager.External.RayCastIntoModelMulti(
                    model,
                    worldRayStart,
                    worldRayEnd,
                    ignoreInvisibleSurfaces,
            ...
            for (var i = 0; i < modelIntersections.Length; i++)
            {
                ...
                if (
                    BrushTraits.IsSurfaceUnselectable(
                        ...
                        brush.ChildData.Model.IsTrigger,
                        ignoreSurfaceFlags: ignoreInvisibleSurfaces
                    )
                )
                    continue;
                ...
                if (
                    foundIntersections.TryGetValue(currentNode, out other)
        ...
        public static bool FindMultiWorldIntersection(
            Vector3 worldRayStart,
            Vector3 worldRayEnd,
            out LegacyBrushIntersection[] intersections,
            bool ignoreInvisibleSurfaces = true,
        ...
        public static bool FindBrushIntersection(
            CSGBrush brush,
        ...
        )
        {
            ...
            if (
                !InternalCSGModelManager.External.RayCastIntoBrush(
                    brush.brushNodeID,
                    rayStart,
                    rayEnd,
                    modelTransformation,
        ...
        public static bool FindSurfaceIntersection(
            Camera camera,
            CSGBrush brush,
            Matrix4x4 modelTransformation,
            Int32 surfaceIndex,
        ...
        )
        {
            ...
            intersection = null;
            if (!brush || InternalCSGModelManager.External.RayCastIntoBrushSurface == null)
                return false;

            if (
                !InternalCSGModelManager.External.RayCastIntoBrushSurface(
                    brush.brushNodeID,
                    surfaceIndex,
                    rayStart,
                    rayEnd,
