...
namespace RealtimeCSG
{
    ...
    {
        internal static bool FindSiblingIndex(
            this HierarchyItem self,
            Transform searchTransform,
            int siblingIndex,
            int searchTransformID,
            out int index
        )
        {
            if (self.ChildNodes == null || self.ChildNodes.Length == 0)
            {
                index = 0;
                return false;
            }
            ...
            if (self.ChildNodes[last].LastLoopCount != currentLoopCount)
            {
                if (self.ChildNodes[last].Transform != null && self.ChildNodes[last].Transform)
                    self.ChildNodes[last].CachedTransformSiblingIndex = self.ChildNodes[last]
            ...
            while (imin <= imax)
            {
                ...
                if (self.ChildNodes[imid].LastLoopCount != currentLoopCount)
                {
                    if (self.ChildNodes[imid].Transform != null && self.ChildNodes[imid].Transform)
                        self.ChildNodes[imid].CachedTransformSiblingIndex = self.ChildNodes[imid]
                            .Transform.GetSiblingIndex();
                    else
                ...
                else
                {
                    ...
                    if (imid > 0)
                    {
                        ...
                        {
                            if (
                                self.ChildNodes[imid - 1].Transform != null
                                && self.ChildNodes[imid - 1].Transform
                            )
                                self.ChildNodes[imid - 1].CachedTransformSiblingIndex =
                                    self.ChildNodes[imid - 1].Transform.GetSiblingIndex();
                            else
        ...
        internal static bool AddChildItem(this HierarchyItem self, HierarchyItem item)
        {
            ...
            if (
                self.FindSiblingIndex(
                    item.Transform,
                    item.CachedTransformSiblingIndex,
                    item.TransformID,
                    out index
                )
            )
        ...
        public static void Init(this HierarchyItem self, CSGNode node, Int32 nodeID)
        {
            ...
            self.TransformID = node.transform.ToNativeId();
            self.NodeID = nodeID;
        }
    }
}
