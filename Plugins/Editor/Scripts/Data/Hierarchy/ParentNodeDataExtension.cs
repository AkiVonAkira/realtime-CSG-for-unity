    ...
    internal static class ParentNodeDataExtensions
    {
        ...
        internal static bool UpdateNodePosition(HierarchyItem node, ParentNodeData top)
        {
            ...
            while (iteratorParent != null)
            {
                ...
                else if (iterator.LastLoopCount != currentLoopCount)
                {
                    var iteratorTransform = iterator.Transform;
                    var iteratorParentTransformID =
                        iteratorTransform.parent == null
                            ? 0
                            : iteratorTransform.parent.ToNativeId();
                    ...
                    if (iteratorParent.TransformID != iteratorParentTransformID)
                    {
                        ...
                        var defaultCSGModel = InternalCSGModelManager.GetDefaultCSGModelForObject(
                            iteratorTransform
                        );
                        if (defaultCSGModel != null && defaultCSGModel.transform != null)
                            defaultCSGInstanceID = defaultCSGModel.transform.ToNativeId();
                        if (
                            defaultCSGInstanceID == 0
                            || defaultCSGInstanceID != iteratorParent.TransformID
                        )
                        {
                            RemoveNode(node, top);
                            AddNode(node, top);
                    ...
                    if (
                        !iteratorParent.FindSiblingIndex(
                            iteratorTransform,
                            iterator.CachedTransformSiblingIndex,
                            iteratorTransformID,
                            out iteratorChildIndex
                        )
                    )
                    {
                        RemoveNode(node, top);
                    ...
                    {
                        var prevTransform = iteratorParentChildNodes[
                            iteratorChildIndex - 1
                        ].Transform;
                        if (!prevTransform)
                        {
                            return false;
                    ...
                    else if (iteratorChildIndex < iteratorParentChildNodes.Length - 1)
                    {
                        var nextTransform = iteratorParentChildNodes[
                            iteratorChildIndex + 1
                        ].Transform;
                        if (!nextTransform)
                        {
        ...
        internal static bool AddNode(HierarchyItem node, ParentNodeData top)
        {
            ...
            while (iterator != null && iterator != top.Transform)
            {
            ...
            if (iterator == null || top.Transform == null || top.Transform == defaultModelTransform)
            {
            ...
            while (ancestorDepth >= 0)
            {
                ...
                int childIndex;
                if (
                    !lastParent.FindSiblingIndex(
                        ancestor,
                        ancestor.GetSiblingIndex(),
                        ancestor.ToNativeId(),
                        out childIndex
                    )
                )
            ...
            while (ancestorDepth >= 0)
            {
                ...
                newAncestor.TransformID = newAncestor.Transform.ToNativeId();
