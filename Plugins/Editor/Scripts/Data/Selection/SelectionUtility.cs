    ...
    internal static class SelectionUtility
    {
        ...
        internal static CSGModel LastUsedModel
        {
            get
            {
                ...
                if (returnModel != null && returnModel.gameObject.activeInHierarchy)
                {
                    return returnModel;
                }
        ...
        public static Transform FindParentToAssignTo(LegacyBrushIntersection intersection)
        {
            ...
            )
            {
                ...
                if (lastModel == null || CSGPrefabUtility.IsPrefabAsset(lastModel.gameObject))
                    return null;
        ...
        public static Quaternion FindDragOrientation(
            SceneView sceneView,
            Vector3 normal,
            PrefabSourceAlignment sourceSurfaceAlignment,
        ...
        )
        {
            ...
            {
                default:
                case PrefabSourceAlignment.AlignedFront:
                    srcRotation = Quaternion.LookRotation(MathConstants.forwardVector3);
                    break;
                case PrefabSourceAlignment.AlignedBack:
                    srcRotation = Quaternion.LookRotation(MathConstants.backVector3);
                    break;
                case PrefabSourceAlignment.AlignedLeft:
                    srcRotation = Quaternion.LookRotation(MathConstants.rightVector3);
                    break;
                case PrefabSourceAlignment.AlignedRight:
                    srcRotation = Quaternion.LookRotation(MathConstants.leftVector3);
                    break;
                case PrefabSourceAlignment.AlignedTop:
                    srcRotation = Quaternion.LookRotation(
                        MathConstants.upVector3,
                        MathConstants.forwardVector3
                    );
                    break;
                case PrefabSourceAlignment.AlignedBottom:
                    srcRotation = Quaternion.LookRotation(
                        MathConstants.downVector3,
                        MathConstants.backVector3
                    );
                    break;
            }
            ...
            switch (destinationSurfaceAlignment)
            {
                ...
                case PrefabDestinationAlignment.AlignSurfaceUp:
                {
                    ...
                    if (normal.sqrMagnitude == 0)
                        normal = GeometryUtility.SnapToClosestAxis(
                            sceneView.camera
        ...
        public static EventModifiers CurrentModifiers
        {
            get { return currentModifiers; }
            internal set { currentModifiers = value; }
        }
            ...
            for (int sceneIndex = 0; sceneIndex < SceneManager.sceneCount; sceneIndex++)
            {
                ...
                for (int i = 0; i < rootGameObjects.Length; i++)
                {
                    ...
                    for (int c = 0; c < children.Length; c++)
                    {
                        ...
                        if (
                            gameObject.activeInHierarchy || (gameObject.hideFlags != HideFlags.None)
                        )
                            continue;
        ...
        public static void HideUnselectedObjects()
        {
            ...
            for (var i = 0; i < models.Length; i++)
            {
                ...
                var meshContainerChildren =
                    model.generatedMeshes.GetComponentsInChildren<Transform>();
            ...
            {
                selectedIDs.Add(selected[i].ToNativeId());
            ...
            for (var sceneIndex = 0; sceneIndex < SceneManager.sceneCount; sceneIndex++)
            {
                ...
                for (var i = 0; i < rootGameObjects.Length; i++)
                {
                    ...
                    for (var c = 0; c < children.Length; c++)
                    {
                        ...
                        if (selectedIDs.Contains(gameObject.ToNativeId()))
                            continue;
        ...
        )
        {
            ...
            var selectedObjectsOnClick = new List<EntityId>(SelectionIdExtensions.GetEntityIds());
#else
            var selectedObjectsOnClick = new List<int>(SelectionIdExtensions.GetEntityIds());
#endif
            ...
            if (EditorGUI.actionKey)
            {
                if (gameobject != null)
                {
                    ...
                    if (selectedObjectsOnClick.Contains(objectId))
                    {
                        selectedObjectsOnClick.Remove(objectId);
                    }
                    else
                    {
                        selectedObjectsOnClick.Add(objectId);
                        addedSelection = true;
                    }
            ...
            else if (Event.current.shift)
            {
                if (gameobject != null)
                {
                    selectedObjectsOnClick.Add(gameobject.Id());
                    SelectionIdExtensions.SetEntityIds(selectedObjectsOnClick.ToArray());
                    addedSelection = true;
                }
            }
