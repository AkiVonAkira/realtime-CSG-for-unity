...
namespace InternalRealtimeCSG
{
    internal sealed partial class MeshInstanceManager
    {
...
#else
        public const HideFlags ComponentHideFlags =
            HideFlags.None
        ...
        private static readonly HashSet<GeneratedMeshes> validateGeneratedMeshes =
            new HashSet<GeneratedMeshes>();
        private static readonly HashSet<CSGModel> validateModelWithChildren =
            new HashSet<CSGModel>();
        private static readonly HashSet<CSGModel> validateModelWithoutChildren =
            new HashSet<CSGModel>();
        ...
        static GeneratedMeshes EnsureOneValidGeneratedMeshesComponent(CSGModel model)
        {
            ...
            {
                var models = foundGeneratedMeshes[i]
                    .GetComponentsInParent<CSGModel>(includeInactive: true);
            ...
            else if (foundGeneratedMeshes.Length == 1)
            {
        ...
        internal static bool ValidateModelNow(CSGModel model, bool checkChildren = false)
        {
            ...
            if (!checkChildren && model.generatedMeshes && model.generatedMeshes.owner == model)
                return true;

            if (!ModelTraits.IsModelEditable(model) || !model.isActiveAndEnabled)
                return false;
        ...
        private static bool ShouldRenderHelperSurface(RenderSurfaceType renderSurfaceType)
        {
            ...
            {
                case RenderSurfaceType.Hidden:
                    return CSGSettings.ShowHiddenSurfaces;
                case RenderSurfaceType.Culled:
                    return CSGSettings.ShowCulledSurfaces;
                case RenderSurfaceType.Collider:
                    return CSGSettings.ShowColliderSurfaces;
                case RenderSurfaceType.Trigger:
                    return CSGSettings.ShowTriggerSurfaces;
                case RenderSurfaceType.ShadowOnly:
                case RenderSurfaceType.CastShadows:
                    return CSGSettings.ShowCastShadowsSurfaces;
                case RenderSurfaceType.ReceiveShadows:
                    return CSGSettings.ShowReceiveShadowsSurfaces;
            }
            return false;
        }
        ...
        */
        ...
        public static UnityEngine.Object[] FindRenderers(CSGModel[] models)
        {
            ...
            foreach (var model in models)
            {
                ...
                foreach (
                    var renderer in model.generatedMeshes.GetComponentsInChildren<MeshRenderer>()
                )
                {
                    if (!renderer)
                        continue;
        ...
        internal static void ResetScene(Scene scene)
        {
            ...
            for (int m = 0; m < meshInstances.Count; m++)
            {
                ...
                var modelTransform = meshInstanceTransform
                    ? meshInstanceTransform.parent
                        ? meshInstanceTransform.parent.parent
                        : null
                    : null;
        ...
        public static RenderSurfaceType GetSurfaceType(
            GeneratedMeshDescription meshDescription,
            ModelSettingsFlags modelSettings
        )
        {
            ...
            switch (meshDescription.meshQuery.LayerParameterIndex)
            {
                case LayerParameterIndex.LayerParameter1:
                {
                    switch (meshDescription.meshQuery.LayerQuery)
                    {
                        ...
                        case LayerUsageFlags.ReceiveShadows:
                            return RenderSurfaceType.Hidden;
                ...
                case LayerParameterIndex.None:
                {
                    switch (meshDescription.meshQuery.LayerQuery)
                    {
                        case LayerUsageFlags.None:
                            return RenderSurfaceType.Hidden;
                        case LayerUsageFlags.CastShadows:
                            return RenderSurfaceType.CastShadows;
                        case LayerUsageFlags.ReceiveShadows:
                            return RenderSurfaceType.ReceiveShadows;
        ...
        static void RemoveMeshInstanceComponents(
        ...
        static void GetGameObjectAndGeneratedMeshInstance(
            List<GameObject> unusedInstances,
            GeneratedMeshDescription meshDescription,
            out GameObject meshInstanceGameObject,
            out GeneratedMeshInstance meshInstance
        )
        {
            if (unusedInstances == null || unusedInstances.Count == 0)
            {
            ...
            for (int i = 0; i < unusedInstances.Count; i++)
            {
                ...
                if (!unusedInstances[i].TryGetComponent(out instance))
                    continue;
                if (
                    meshDescription.meshQuery.LayerParameterIndex
                    == instance.MeshDescription.meshQuery.LayerParameterIndex
                )
                {
        ...
        public static GeneratedMeshInstance CreateMeshInstance(
        ...
        )
        {
            ...
            GetGameObjectAndGeneratedMeshInstance(
                unusedInstances,
                meshDescription,
                out meshInstanceGameObject,
                out meshInstance
            );
            ...
            var containerStaticFlags = GameObjectUtility.GetStaticEditorFlags(
                generatedMeshes.owner.gameObject
            );
            ...
            if (meshDescription.surfaceParameter != 0)
            {
                ...
                if (obj)
                {
                    switch (meshDescription.meshQuery.LayerParameterIndex)
                    {
                        case LayerParameterIndex.RenderMaterial:
                        {
                            renderMaterial = obj as Material;
                            break;
                        }
                        case LayerParameterIndex.PhysicsMaterial:
                        {
                            physicsMaterial = obj as PhysicsMaterial;
                            break;
                        }
                    }
                }
        ...
        public static void UpdateHelperSurfaceGameObject(
            GeneratedMeshes container,
            HelperSurfaceDescription helperSurfaceDescription
        )
        {
            if (
                !helperSurfaceDescription.GameObject
            ...
            )
            {
                ...
                helperSurfaceDescription.MeshFilter =
                    helperSurfaceDescription.GameObject.AddComponent<MeshFilter>();
                helperSurfaceDescription.MeshRenderer =
                    helperSurfaceDescription.GameObject.AddComponent<MeshRenderer>();
            }
            if (
                helperSurfaceDescription.MeshFilter.sharedMesh
                != helperSurfaceDescription.SharedMesh
            )
                helperSurfaceDescription.MeshFilter.sharedMesh =
                    helperSurfaceDescription.SharedMesh;

            var sharedMaterial = MaterialUtility.GetSurfaceMaterial(
        ...
        public static HelperSurfaceDescription CreateHelperSurfaceDescription(
            GeneratedMeshes container,
            ModelSettingsFlags modelSettings,
            GeneratedMeshDescription meshDescription,
            RenderSurfaceType renderSurfaceType
        ...
        internal static void ClearOrCreateMesh(
            string baseName,
            bool editorOnly,
            ref bool hasGeneratedNormals,
            ref Mesh sharedMesh
        )
        {
            ...
            sharedMesh = new Mesh();
            sharedMesh.name = $"<{baseName} generated {sharedMesh.ToNativeId()}>";
        ...
        private static void GenerateLightmapUVsForInstance(
            GeneratedMeshInstance instance,
            CSGModel model,
            UnwrapParam param
        )
        {
            ...
            if (model.ShowGeneratedMeshes)
                pingObject = instance;
            Debug.Log(
                "Generating lightmap UVs (by Unity) for the mesh "
                    + instance.name
                    + " of the Model named \""
                    + model.name
                    + "\"\n",
        ...
        {
            return Mathf.Abs(instance.SharedMesh.bounds.size.x) <= MathConstants.EqualityEpsilon
        ...
        {
            return surfaceType == RenderSurfaceType.Collider
        ...
        static StaticEditorFlags FilterStaticEditorFlags(
            StaticEditorFlags modelStaticFlags,
            RenderSurfaceType renderSurfaceType
        )
        {
            ...
            var walkable =
                renderSurfaceType != RenderSurfaceType.Hidden
                && renderSurfaceType != RenderSurfaceType.ShadowOnly
                && renderSurfaceType != RenderSurfaceType.Culled
                && renderSurfaceType != RenderSurfaceType.Trigger;
            if (!walkable)
                meshStaticFlags = meshStaticFlags & ~StaticEditorFlagsCompat.NavigationStatic;
            ...
            if (renderSurfaceType == RenderSurfaceType.ShadowOnly)
                meshStaticFlags =
                    meshStaticFlags
                    & ~(StaticEditorFlags.LightmapStatic | StaticEditorFlags.ReflectionProbeStatic);
#endif
        ...
        static string MaterialToString(Material mat)
        {
            ...
            if (!mat)
                return "invalid";
            return mat.name + " " + mat.ToNativeId().ToString();
        }
        ...
        public static void Refresh(
            CSGModel model,
            bool postProcessScene = false,
            bool onlyFastRefreshes = true
        )
        {
            if (!ModelTraits.IsModelEditable(model))
                return;
        ...
        public static void Refresh(
            ...
            bool postProcessScene = false,
            bool onlyFastRefreshes = true,
            bool skipAssetDatabaseUpdate = true
        )
        {
            if (RealtimeCSG.CSGModelManager.IsInPlayMode)
                return;
            ...
            if (gameObject.hideFlags != gameObjectFlags)
            {
                gameObject.hideFlags = gameObjectFlags;
            }
            if (instance.hideFlags != meshInstanceFlags)
            {
                instance.hideFlags = meshInstanceFlags;
            }

            var showVisibleSurfaces =
                instance.RenderSurfaceType != RenderSurfaceType.Normal
                || (
                    RealtimeCSG.CSGSettings.VisibleHelperSurfaces
                    & HelperSurfaceFlags.ShowVisibleSurfaces
                ) != 0;
            ...
            var newStaticFlags = FilterStaticEditorFlags(
                GameObjectUtility.GetStaticEditorFlags(owner.gameObject),
            ...
            if (needMeshRenderer)
            {
                ...
                var shadowCastingMode = owner.IsTwoSidedShadows
                ...
                if (instance.RenderSurfaceType != RenderSurfaceType.ShadowOnly)
                {
                    ...
                    if ((owner.AutoRebuildUVs || postProcessScene))
                    {
                        if (
                            (
                                float.IsPositiveInfinity(instance.ResetUVTime)
                                || ((Time.realtimeSinceStartup - instance.ResetUVTime) > 2.0f)
                ...
                {
                    var meshRendererComponentSO =
                        instance.CachedMeshRendererSO as UnityEditor.SerializedObject;
                    if (meshRendererComponentSO == null)
                    {
                        if (meshRendererComponent)
                        {
                            instance.CachedMeshRendererSO = meshRendererComponentSO =
                                new SerializedObject(meshRendererComponent);
                        }
                    }
                    else if (!meshRendererComponent)
                    {
                        instance.CachedMeshRendererSO = meshRendererComponentSO = null;
                    }
                    if (meshRendererComponentSO != null)
                    {
                        bool SOModified = false;
                        meshRendererComponentSO.Update();
                        var scaleInLightmapProperty = meshRendererComponentSO.FindProperty(
                            "m_ScaleInLightmap"
                        );
                        var scaleInLightmap = owner.scaleInLightmap;
                        if (
                            scaleInLightmapProperty != null
                        ...
                        var autoUVMaxDistanceProperty = meshRendererComponentSO.FindProperty(
                            "m_AutoUVMaxDistance"
                        );
                        var autoUVMaxDistance = owner.autoUVMaxDistance;
                        if (
                            autoUVMaxDistanceProperty != null
                        ...
                        var autoUVMaxAngleProperty = meshRendererComponentSO.FindProperty(
                            "m_AutoUVMaxAngle"
                        );
                        var autoUVMaxAngle = owner.autoUVMaxAngle;
                        if (
                            autoUVMaxAngleProperty != null
                        ...
                        var ignoreNormalsProperty = meshRendererComponentSO.FindProperty(
                            "m_IgnoreNormalsForChartDetection"
                        );
                        var ignoreNormals = owner.IgnoreNormals;
                        if (
                            ignoreNormalsProperty != null
                        ...
                        var minimumChartSizeProperty = meshRendererComponentSO.FindProperty(
                            "m_MinimumChartSize"
                        );
                        var minimumChartSize = owner.minimumChartSize;
                        if (
                            minimumChartSizeProperty != null
                        ...
                        var preserveUVsProperty = meshRendererComponentSO.FindProperty(
                            "m_PreserveUVs"
                        );
                        var preserveUVs = owner.PreserveUVs;
                        if (
                            preserveUVsProperty != null
                        ...
                        var stitchLightmapSeamsProperty = meshRendererComponentSO.FindProperty(
                            "m_StitchLightmapSeams"
                        );
                        var stitchLightmapSeams = owner.StitchLightmapSeams;
                        if (
                            stitchLightmapSeamsProperty != null
                            && // Note that some alpha/beta versions of 2017.2 had a different name
                ...
                if (meshRendererComponent && meshRendererComponent.receiveGI != receiveGI)
                {
            ...
            var needMeshCollider =
                !AreBoundsEmpty(instance) && NeedCollider(instance.RenderSurfaceType);
            ...
            if (needMeshCollider)
            {
                ...
                var currentPhysicsMaterial =
                    instance.PhysicsMaterial ?? owner.DefaultPhysicsMaterial;
                ...
                if (instance.RenderSurfaceType == RenderSurfaceType.Trigger || owner.IsTrigger)
                {
        ...
        private static void UpdateName(GeneratedMeshInstance instance)
        {
            ...
            builder.Append(' ');
            builder.Append(instance.ToNativeId());

            if (instance.PhysicsMaterial)
            {
                var physicmaterialName = (
                    (!instance.PhysicsMaterial) ? "default" : instance.PhysicsMaterial.name
                );
                if (builder.Length > 0)
                    builder.Append(' ');
            ...
            if (parentObject.name != objectName)
                parentObject.name = objectName;
            if (instance.SharedMesh && instance.SharedMesh.name != objectName)
        ...
        public static void UpdateHelperSurfaceVisibility(bool force = false)
        {
            ...
            for (var i = 0; i < models.Length; i++)
            {
                ...
                if (force || RefreshModelCounter == currentRefreshModelCount)
                {
                    ...
                    foreach (var instance in generatedMeshes.MeshInstances)
                    {
                        ...
                        Refresh(
                            instance,
                            generatedMeshes.owner,
                            onlyFastRefreshes: !force,
                            skipAssetDatabaseUpdate: false
                        );
                    }
                }
        ...
        {
            if (
                !ModelTraits.IsModelEditable(container.owner)
        ...
        static readonly List<GeneratedMeshInstance> s_foundMeshInstances =
            new List<GeneratedMeshInstance>();

        public static bool ValidateGeneratedMeshesNow(
            GeneratedMeshes generatedMeshes,
            bool skipSiblingCheck = false
        )
        {
            if (RealtimeCSG.CSGModelManager.IsInPlayMode)
                return false;
            ...
            for (var i = 0; i < generatedMeshesTransform.childCount; i++)
            {
                ...
                {
                    if (
                        meshInstanceTransform.gameObject
            ...
            if (generatedMeshes.owner)
                UpdateGeneratedMeshesVisibility(
                    generatedMeshes,
                    generatedMeshes.owner.ShowGeneratedMeshes
                );
        ...
        public static GeneratedMeshInstance GetMeshInstance(
            GeneratedMeshes container,
            GeneratedMeshDescription meshDescription,
            ModelSettingsFlags modelSettings,
            RenderSurfaceType renderSurfaceType
        ...
        public static HelperSurfaceDescription GetHelperSurfaceDescription(
            GeneratedMeshes container,
            ModelSettingsFlags modelSettings,
            GeneratedMeshDescription meshDescription,
            RenderSurfaceType renderSurfaceType
        )
        {
            ...
            return CreateHelperSurfaceDescription(
                container,
                modelSettings,
                meshDescription,
                renderSurfaceType
        ...
        public static List<GameObject> FindUnusedMeshInstances(
            GeneratedMeshes container,
        ...
        static readonly List<GeneratedMeshInstance> __notfoundInstances =
            new List<GeneratedMeshInstance>();
        ...
        private static void UpdateContainerFlags(GeneratedMeshes generatedMeshes)
        {
            ...
            {
                if (
                    CSGPrefabUtility.IsPrefabAsset(generatedMeshes.gameObject)
                    || CSGPrefabUtility.IsPrefabAsset(generatedMeshes.owner.gameObject)
                )
                    return;

                if (
                    !CSGPrefabUtility.IsPrefabInstance(generatedMeshes.gameObject)
            ...
            var previousStaticFlags = GameObjectUtility.GetStaticEditorFlags(
                generatedMeshes.gameObject
            );
            ...
            if (
                ownerStaticFlags != previousStaticFlags
            ...
            )
            {
                ...
                foreach (var meshInstance in generatedMeshes.MeshInstances)
                {
                    ...
                    if (meshInstance.RenderSurfaceType == RenderSurfaceType.Normal)
                        CSGVisibilityUtility.SetGameObjectVisibility(
