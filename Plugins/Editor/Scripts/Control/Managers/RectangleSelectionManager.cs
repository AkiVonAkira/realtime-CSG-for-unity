...
namespace RealtimeCSG
{
    internal sealed class RectangleSelectionManager
    {
        ...
        static void InitReflectedData()
        {
            ...
            UnityRectSelectionType = types.FirstOrDefault(t =>
                t.FullName == "UnityEditor.RectSelection"
            );
            UnityEnumSelectionType = types.FirstOrDefault(t =>
                t.FullName == "UnityEditor.RectSelection+SelectionType"
            );

            if (UnitySceneViewType != null)
            {
                m_RectSelection_field = UnitySceneViewType.GetField(
                    "m_RectSelection",
            ...
            {
                m_RectSelecting_field = UnityRectSelectionType.GetField(
                    "m_RectSelecting",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                s_RectSelectionID_field = UnityRectSelectionType.GetField(
                    "s_RectSelectionID",
                    BindingFlags.NonPublic | BindingFlags.Static
                );
                m_SelectStartPoint_field = UnityRectSelectionType.GetField(
                    "m_SelectStartPoint",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                m_SelectionStart_field = UnityRectSelectionType.GetField(
                    "m_SelectionStart",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                m_LastSelection_field = UnityRectSelectionType.GetField(
                    "m_LastSelection",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                m_CurrentSelection_field = UnityRectSelectionType.GetField(
                    "m_CurrentSelection",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                m_SelectMousePoint_field = UnityRectSelectionType.GetField(
                    "m_SelectMousePoint",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                ...
                if (UnityEnumSelectionType != null)
                {
                    ...
                    UpdateSelection_method = UnityRectSelectionType.GetMethod(
                        "UpdateSelection",
                        BindingFlags.NonPublic | BindingFlags.Static,
                        null,
                        new Type[]
        ...
        internal static void Update(SceneView sceneView)
        {
            ...
            if (GUIUtility.hotControl == s_RectSelectionID_instance)
            {
                ...
                )
                {
                    ...
                    var m_RectSelecting_instance = (bool)
                        m_RectSelecting_field.GetValue(m_RectSelection_instance);
                    if (m_RectSelecting_instance)
                    {
                        // m_SelectStartPoint of RectSelection instance
                        var m_SelectStartPoint_instance = (Vector2)
                            m_SelectStartPoint_field.GetValue(m_RectSelection_instance);

                        // m_SelectMousePoint of RectSelection instance
                        var m_SelectMousePoint_instance = (Vector2)
                            m_SelectMousePoint_field.GetValue(m_RectSelection_instance);
                        ...
                        if (needUpdate)
                        {
                            var rect = CameraUtility.PointsToRect(
                                prevStartScreenPoint,
                                prevMouseScreenPoint
                            );
                            if (rect.width > 3 && rect.height > 3)
                            {
                                var frustum = CameraUtility.GetCameraSubFrustumGUI(
                                    sceneView.camera,
                                    rect
                                );
                                ...
                                if (
                                    SceneQueryUtility.GetItemsInFrustum(
                                        frustum.Planes,
                        ...
                        var m_LastSelection_instance =
                            (Dictionary<GameObject, bool>)
                                m_LastSelection_field.GetValue(m_RectSelection_instance);
                        var m_SelectionStart_instance = (UnityEngine.Object[])
                            m_SelectionStart_field.GetValue(m_RectSelection_instance);
                        if (
                            modified
                            && rectFoundGameObjects != null
                            && rectFoundGameObjects.Count > 0
                        )
                        {
                            ...
                            foreach (var obj in rectFoundGameObjects)
                            {
                                ...
                                var models = obj.GetComponentsInParent<CSGModel>(
                            ...
                            m_CurrentSelection_field.SetValue(
                                m_RectSelection_instance,
                                currentSelection
                            );
                        }
                        for (int j = m_SelectionStart_instance.Length - 1; j >= 0; j--)
                        {
                            ...
                            if (obj.GetComponent<GeneratedMeshInstance>() != null)
                            {
                                ...
                                m_SelectionStart_field.SetValue(
                                    m_RectSelection_instance,
                                    m_SelectionStart_instance
                                );
                                modified = true;
                        ...
                        if ((Event.current.commandName == "ModifierKeysChanged" || modified))
                        {
                            ...
                            for (int j = foundObjects.Length - 1; j >= 0; j--)
                            {
                                ...
                                if (
                                    obj == null
                                    || obj.GetComponent<GeneratedMeshInstance>() != null
                                )
                                {
            ...
            if (
                hotControl == s_RectSelectionID_instance
            ...
            switch (eventType)
            {
                ...
                {
                    rectClickDown = (
                        Event.current.button == 0 && hotControl == s_RectSelectionID_instance
                    );
                    ...
                    foreach (var obj in selectedObjects)
                    {
                        ...
                        if (
                            !(component && component.GetComponent<GeneratedMeshes>())
                ...
                case EventType.Used:
                {
                    ...
                    if (!mouseDragged && rectClickDown && Event.current.button == 0)
                    {
                        // m_RectSelection field of SceneView
                        var m_RectSelection_instance = m_RectSelection_field.GetValue(sceneView);
                ...
                case EventType.ExecuteCommand:
                {
                    if (Event.current.commandName == "SelectAll")
                    {
                        ...
                        for (int sceneIndex = 0; sceneIndex < SceneManager.sceneCount; sceneIndex++)
                        {
                            ...
                            {
                                foreach (
                                    var transform in gameObject.GetComponentsInChildren<Transform>()
                                )
                                {
                                    if (
                                        (
                                            transform.hideFlags
                                            & (HideFlags.NotEditable | HideFlags.HideInHierarchy)
                                        ) == (HideFlags.NotEditable | HideFlags.HideInHierarchy)
                                    )
                                        continue;
