...
namespace RealtimeCSG
{
    internal partial class InternalCSGModelManager
    {
        ...
        private static readonly HashSet<CSGOperation> RegisterOperations =
            new HashSet<CSGOperation>();
        private static readonly HashSet<CSGOperation> ValidateOperations =
            new HashSet<CSGOperation>();
        private static readonly HashSet<CSGOperation> UnregisterOperations =
            new HashSet<CSGOperation>();
        private static readonly HashSet<CSGOperation> OperationTransformChanged =
            new HashSet<CSGOperation>();
        ...
        static void AddNodeRegistration(CSGNode node)
        {
            ...
            if (brush)
            {
                RegisterBrushes.Add(brush);
                return;
            }
            var operation = node as CSGOperation;
            if (operation && !operation.PassThrough)
            {
                RegisterOperations.Add(operation);
                return;
            }
            var model = node as CSGModel;
            if (model)
        ...
        internal static CSGModel CreateCSGModel(GameObject gameObject)
        {
            ...
            StaticEditorFlags defaultFlags = //StaticEditorFlags.LightmapStatic |
                ...
                | StaticEditorFlagsCompat.NavigationStatic
                | StaticEditorFlags.OccludeeStatic
                | StaticEditorFlagsCompat.OffMeshLinkGeneration
        ...
        public static void EnsureInitialized(CSGBrush component)
        {
            ...
            var statement1 =
                !System.Object.ReferenceEquals(component, null)
            ...
            if (statement4)
            {
                ...
                component.hierarchyItem.TransformID = component.transform.ToNativeId();
            ...
            {
                dirty = true;
                BrushFactory.CreateCubeControlMesh(
                    out component.ControlMesh,
                    out component.Shape,
                    Vector3.one
                );
            }
        ...
        public static void EnsureInitialized(CSGOperation component)
        {
            ...
            if (
                component.ParentData.TransformID == 0
                && component.operationNodeID != CSGNode.InvalidNodeID
            )
            {
                component.ParentData.Transform = component.transform;
                component.ParentData.TransformID = component.transform.ToNativeId();
        ...
        static void RegisterChild(ChildNodeData childData)
        {
            ...
            if (childData.Model && !childData.Model.IsRegistered)
            {
                RegisterModel(childData.Model);
            }

            // make sure our parent (if any) has actually been initialized
            if (childData.Parent && !childData.Parent.IsRegistered)
            {
        ...
        {
            if (
                RealtimeCSG.CSGModelManager.IsInPlayMode
                || External == null
                || !brush
            ...
            if (!External.GenerateBrush(NativeUserId.ToNative(brush), out brush.brushNodeID))
            {
            ...
            brush.ChildData.ModelTransform =
                (!brush.ChildData.Model) ? null : brush.ChildData.Model.transform;

            parentData.AddNode(brush.hierarchyItem);

            if (
                brush.Shape == null
                || brush.Shape.Surfaces == null
                || brush.Shape.Surfaces.Length < 4
            )
            {
                Debug.LogError(
                    "The brush (" + brush.name + ") is infinitely thin and is invalid",
                    brush
                );
        ...
        static void RegisterOperation(CSGOperation op)
        {
            ...
            if (!External.GenerateOperation(NativeUserId.ToNative(op), out op.operationNodeID))
            {
            ...
            op.ChildData.ModelTransform =
                (!op.ChildData.Model) ? null : op.ChildData.Model.transform;
        ...
        private static void RegisterModel(CSGModel model)
        {
            ...
            if (model.IsRegistered)
            {
                ...
                Debug.LogWarning(
                    "Model named "
                        + model.name
                        + " marked as registered, but wasn't actually registered",
                    model
                );
            }
            ...
            if (!External.GenerateModel(NativeUserId.ToNative(model), out model.modelNodeID))
            {
        ...
        static bool SetBrushMesh(CSGBrush brush)
        {
            ...
            var brushMesh = Legacy.BrushFactory.GenerateFromControlMesh(
                controlMesh,
                shape,
                brush.ChildData.Model.DefaultPhysicsMaterial
            );
            if (brushMesh == null)
            {
                return false;
            ...
            brushMeshID = External.CreateBrushMesh(NativeUserId.ToNative(brush), brushMesh);
        ...
        static void UnregisterOperation(CSGOperation op)
        {
            ...
            if (op.ChildData.Model && op.ChildData.Model.IsRegistered)
            {
                var parentData = GetParentData(op.ChildData);
        ...
        public static void ValidateOperation(CSGOperation op)
        {
            ...
            {
                op.PrevOperation = op.OperationType;
                External.SetOperationOperationType(op.operationNodeID, op.OperationType);
            }
        }
        #endregion

        #region CheckSurfaces
        public static void CheckSurfaceModifications(
            CSGBrush[] brushes,
            bool surfacesModified = false
        )
        {
        ...
        public static void CheckSurfaceModifications(CSGBrush brush, bool surfacesModified = false)
        {
            ...
            {
                Debug.LogWarning(
                    "storedShape.prevSubShapes == null || storedShape.prevTexGens == null"
                );
                return;
            }

            bool updateTexGens =
                compareShape.prevTexGens == null
            ...
            if (!updateTexGens)
            {
                for (int i = 0; i < shape.TexGens.Length; i++)
                {
                    if ( //shape.TexGens[i].Color			== compareShape.prevTexGens[i].Color &&
                        ...
                        && shape.TexGens[i].SmoothingGroup
                            == compareShape.prevTexGens[i].SmoothingGroup
                ...
                if (!updateTexGens)
                {
                    ...
                    {
                        if (
                            shape.TexGens[i].RenderMaterial
                            == compareShape.prevTexGens[i].RenderMaterial
                        )
                            continue;
            ...
            else if (!surfacesModified)
            {
                ...
                else
                {
                    ...
                    {
                        if (
                            shape.Surfaces[surfaceIndex].Plane.normal
                                != compareShape.prevSurfaces[surfaceIndex].Plane.normal
                            || shape.Surfaces[surfaceIndex].Plane.d
                                != compareShape.prevSurfaces[surfaceIndex].Plane.d
                            || shape.Surfaces[surfaceIndex].Tangent
                                != compareShape.prevSurfaces[surfaceIndex].Tangent
                        )
                        {
                            surfacesModified = true;
                            break;
                        ...
                        if (
                            shape.Surfaces[surfaceIndex].TexGenIndex
                            != compareShape.prevSurfaces[surfaceIndex].TexGenIndex
                        )
                            updateTexGens = true;
                    }
                }
            ...
            if (updateTexGens || surfacesModified)
            {
                ...
                else
                {
                    ...
                    if (
                        compareShape.prevSurfaces == null
        ...
        public static void ValidateBrush(CSGBrush brush, bool surfacesModified = false)
        {
            ...
            if (wasRegistered && brush.controlMeshGeneration != brush.ControlMesh.Generation)
            {
                brush.controlMeshGeneration = brush.ControlMesh.Generation;
        ...
        internal static void UpdateRegistration()
        {
            ...
            if (
                UnregisterBrushes.Count > 0
                || UnregisterOperations.Count > 0
                || DisableModels.Count > 0
            )
            {
                var unregisterBrushesList =
                    (UnregisterBrushes.Count == 0) ? EmptyBrushArray : UnregisterBrushes.ToArray();
                var unregisterOperationsList =
                    (UnregisterOperations.Count == 0)
                        ? EmptyOperationArray
                        : UnregisterOperations.ToArray();
                var disableModelsList =
                    (DisableModels.Count == 0) ? EmptyModelArray : DisableModels.ToArray();
                UnregisterBrushes.Clear();
                UnregisterOperations.Clear();
                DisableModels.Clear();

                if (
                    unregisterBrushesList.Length > 0
            ...
            if (
                RegisterModels.Count > 0
                || EnableModels.Count > 0
                || RegisterOperations.Count > 0
                || RegisterBrushes.Count > 0
            )
            {
                var registerModelsList =
                    (RegisterModels.Count == 0) ? EmptyModelArray : RegisterModels.ToArray();
                var enableModelsList =
                    (EnableModels.Count == 0) ? EmptyModelArray : EnableModels.ToArray();
                var registerOperationsList =
                    (RegisterOperations.Count == 0)
                        ? EmptyOperationArray
                        : RegisterOperations.ToArray();
                var registerBrushesList =
                    (RegisterBrushes.Count == 0) ? EmptyBrushArray : RegisterBrushes.ToArray();
                RegisterModels.Clear();
                EnableModels.Clear();
                RegisterOperations.Clear();
                RegisterBrushes.Clear();
                ...
                var prefabInstances = new HashSet<GameObject>();
                {
                    ...
                    for (var i = registerModelsList.Length - 1; i >= 0; i--)
                    {
                        ...
                        if (!component)
                            continue;
                        var parent = CSGPrefabUtility.GetOutermostPrefabInstanceRoot(
                    ...
                    foreach (var instance in prefabInstances)
                    {
                        ...
                        if (instanceParent)
                            newInstance = (GameObject)
                                UnityEngine.Object.Instantiate(instance, instanceParent);
                        else
            ...
            {
                var validateBrushesList =
                    (ValidateBrushes.Count == 0) ? EmptyBrushArray : ValidateBrushes.ToArray();
                var validateOperationsList =
                    (ValidateOperations.Count == 0)
                        ? EmptyOperationArray
                        : ValidateOperations.ToArray();
                ValidateBrushes.Clear();
                ValidateOperations.Clear();
