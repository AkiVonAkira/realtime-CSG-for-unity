...
namespace RealtimeCSG
{
    internal sealed class SceneDragToolMaterials : ISceneDragTool
    {
        ...
        public bool ValidateDropPoint(SceneView sceneView)
        {
            ...
            if (
                !SceneQueryUtility.FindClickWorldIntersection(
                    ...
                    out foundObject,
                    out hoverOverMeshRenderer,
                    out hoverMaterialIndex,
                    ignoreDeepClick: true
                )
            )
                return false;
        ...
        public bool DragUpdated(
            SceneView sceneView,
            Transform transformInInspector,
            Rect selectionRect
        )
        {
            ...
            if (hoverBrushSurfaces != null)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < highlight_brushes.Length; i++)
                    {
                        ...
                        {
                            if (
                                hoverBrushSurfaces[j].surfaceIndex == -1
        ...
        public bool BrushDragUpdated(SceneView sceneView)
        {
            ...
            if (hoverBrushSurfaces != null)
            {
                ...
                {
                    if (
                        hoverBrushSurfaces[i].brush == highlight_brush
            ...
            if (modified)
            {
                ...
                hoverBrushSurfaces = HoverOnBrush(
                    new CSGBrush[1] { highlight_brush },
                    highlight_surface
                );
        ...
        public void BrushDragExited(SceneView sceneView)
        {
            if (hoverBrushSurfaces != null)
            {
                ...
                for (int i = 0; i < hoverBrushSurfaces.Length; i++)
                {
                    ...
                    if (brush.ChildData == null || brush.ChildData.Model == null)
                        continue;
        ...
        {
            hoverOnSelectedSurfaces = false;
            if (hoverBrushes == null || hoverBrushes.Length == 0 || hoverBrushes[0] == null)
                return null;
            ...
            if (activetool != null)
            {
                ...
                {
                    if (
                        selectedBrushSurfaces[i].surfaceIndex == surfaceIndex
                ...
                for (int i = 0; i < hoverBrushSurfaces.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        // per surface
                        {
                            ...
                            {
                                var material = GetMaterial();
                                var highlight_texGen = shape
                                    .Surfaces[highlight_surface]
                                    .TexGenIndex;
        ...
        public void OnPaint(Camera camera)
        {
            ...
            if (hoverBrushSurfaces != null)
            {
                for (int i = 0; i < hoverBrushSurfaces.Length; i++)
                {
                    ...
                    {
                        CSGRenderer.DrawSelectedBrush(
                            brush.brushNodeID,
                            brush.Shape,
                            brush_transformation,
                            ColorSettings.WireframeOutline,
                            0,
                            selectAllSurfaces,
