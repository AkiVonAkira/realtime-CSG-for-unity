            ...
            if (sceneView != null)
            {
                ...
                if (haveOffset)
                {
                    bottomBarRect = new Rect(
                        0,
                        height - (CSG_GUIStyleUtility.BottomToolBarHeight + 18),
                        width,
                    ...
                    bottomBarRect = new Rect(
                        0,
                        height - (CSG_GUIStyleUtility.BottomToolBarHeight + 1),
                        width,
                        CSG_GUIStyleUtility.BottomToolBarHeight
                    );
                ...
                try
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                        CSG_GUIStyleUtility.BottomToolBarStyle.Draw(
                            bottomBarRect,
                            false,
                            false,
                            false,
        ...
        static void OnBottomBarGUI(SceneView sceneView, Rect barSize)
        {
            ...
            GUI.changed = false;
            {
                ...
                if (viewWidth >= 800)
                    layoutX += 6;
#else
                showGrid = RealtimeCSG.CSGSettings.GridVisible;
#endif
                ...
                switch (snapMode)
                {
                    case SnapMode.GridSnapping:
                    {
                        var newValue = GUI.Toggle(
                            currentRect,
                            snapMode == SnapMode.GridSnapping,
                            CSG_GUIStyleUtility.Skin.gridSnapIconOn,
                            EditorStyles.toolbarButton
                        );
                        if (GUI.changed)
                        {
                    ...
                    case SnapMode.RelativeSnapping:
                    {
                        var newValue = GUI.Toggle(
                            currentRect,
                            snapMode == SnapMode.RelativeSnapping,
                            CSG_GUIStyleUtility.Skin.relSnapIconOn,
                            EditorStyles.toolbarButton
                        );
                        if (GUI.changed)
                        {
                    ...
                    case SnapMode.None:
                    {
                        var newValue = GUI.Toggle(
                            ...
                            CSG_GUIStyleUtility.Skin.noSnapIconOn,
                            EditorStyles.toolbarButton
                        );
                        if (GUI.changed)
                        {
                ...
                if (snapMode != SnapMode.None)
                {
                    ...
                    if (uniformGrid || viewWidth < 515)
                    {
                        EditorGUI.showMixedValue = !(
                            moveSnapVector.x == moveSnapVector.y
                            && moveSnapVector.x == moveSnapVector.z
                        );
                        GUI.changed = false;
                        {
                            ...
                            moveSnapVector.x = Units.DistanceUnitToUnity(
                                distanceUnit,
                                EditorGUI.DoubleField(
                                    currentRect,
                                    Units.UnityToDistanceUnit(distanceUnit, moveSnapVector.x),
                                    textInputStyle
                                )
                            ); //, MinSnapWidth, MaxSnapWidth));
                    ...
                    else
                    {
                        GUI.changed = false;
                        {
                            ...
                            moveSnapVector.z = Units.DistanceUnitToUnity(
                                distanceUnit,
                                EditorGUI.DoubleField(
                                    currentRect,
                                    Units.UnityToDistanceUnit(distanceUnit, moveSnapVector.z),
                                    textInputStyle
                                )
                            ); //, MinSnapWidth, MaxSnapWidth));
                            ...
                            moveSnapVector.y = Units.DistanceUnitToUnity(
                                distanceUnit,
                                EditorGUI.DoubleField(
                                    currentRect,
                                    Units.UnityToDistanceUnit(distanceUnit, moveSnapVector.y),
                                    textInputStyle
                                )
                            ); //, MinSnapWidth, MaxSnapWidth));
                    ...
                    if (viewWidth >= 700)
                    {
                        ...
                        {
                            GridUtility.DoubleGridSize();
                            moveSnapVector = RealtimeCSG.CSGSettings.SnapVector;
                        }
                    ...
                    GUI.changed = false;
                    {
                        ...
                        rotationSnap = EditorGUI.FloatField(
                            currentRect,
                            rotationSnap,
                            textInputStyle
                        ); //, MinSnapWidth, MaxSnapWidth);
            ...
            if (
                GUI.Button(
                    currentRect,
                    CSG_GUIStyleUtility.Skin.rebuildIcon,
                    EditorStyles.toolbarButton
                )
            )
            {
                ...
                text.AppendFormat(
                    ...
                    "Full hierarchy rebuild in {0:F} ms. ",
                ...
                text.AppendFormat(
                    CultureInfo.InvariantCulture,
                    "Full CSG rebuild done in {0:F} ms. ",
                ...
                text.AppendFormat(
                    CultureInfo.InvariantCulture,
                    "{0} brushes. ",
                    Foundation.CSGManager.TreeBrushCount
                );
            ...
            if (viewWidth >= 250)
            {
                GUI.changed = false;
                {
                    ...
                    currentRect.width = Mathf.Max(
                        20,
                        Mathf.Min(165, (viewWidth - prevLayoutX - currentRect.width))
                    );
            ...
            if (showWireframe)
            {
                showWireframe = GUI.Toggle(
                    currentRect,
                    showWireframe,
                    CSG_GUIStyleUtility.Skin.wireframe,
                    EditorStyles.toolbarButton
                );
            ...
            else
            {
                showWireframe = GUI.Toggle(
                    currentRect,
                    showWireframe,
                    CSG_GUIStyleUtility.Skin.wireframeOn,
                    EditorStyles.toolbarButton
                );
                //(x:1237.00, y:0.00, width:26.00, height:18.00)
            }
            ...
            switch (Event.current.GetTypeForControl(controlID))
            {
                case EventType.MouseDown:
                {
                    if (barSize.Contains(mousePoint))
                    {
                        GUIUtility.hotControl = controlID;
                        GUIUtility.keyboardControl = controlID;
                        Event.current.Use();
