...
namespace RealtimeCSG
{
    ...
    internal sealed class EditModeClip : ScriptableObject, IEditMode
    {
        ...
        [SerializeField]
        BackupBrushData[] backupData = new BackupBrushData[0];
        ...
        [SerializeField]
        CSGBrush[] clipBrushes = new CSGBrush[0];
        ...
        [SerializeField]
        Vector3 clipPlaneTangent;

        [SerializeField]
        Vector3 clipPlaneBinormal;
        ...
        [NonSerialized]
        CSGPlane prevMovePlane;

        [NonSerialized]
        bool commitOnClick = false;

        [NonSerialized]
        bool mouseDragged = false;

        [NonSerialized]
        int planeCreationID = -1;

        [NonSerialized]
        List<Vector3> visualSnappedLines; // used to visualize lines which we're snapping against

        [NonSerialized]
        CSGBrush currentOnBrush;
        ...
        [NonSerialized]
        bool forceWireframeUpdate = false;
        ...
        [SerializeField]
        internal ClipMode clipMode = ClipMode.RemovePositive;
        ...
        public int ClipBrushCount
        {
            get { return clipBrushes.Length; }
        }
        ...
        int RememberTargetData(CSGBrush[] addedBrushes = null)
        {
            ...
            else if (clipBrushes == null || clipBrushes.Length == 0)
            {
                backupData = new BackupBrushData[0];
                return 0;
            }
            ...
            for (int srcIndex = srcBrushes.Length - 1; srcIndex >= 0; srcIndex--)
            {
                ...
                backupData[dstIndex].wireFrame = BrushOutlineManager.GetBrushOutline(
                    srcBrush.brushNodeID
                );

                if (srcBrush.ChildData == null || !srcBrush.ChildData.Model)
                {
                ...
                backupData[dstIndex].localToWorldMatrix = srcBrush
        ...
        void RestoreTargetData(bool hoverMode = false, HashSet<int> selectedIndices = null)
        {
            ...
            try
            {
                ...
                for (int t = 0; t < backupData.Length; t++)
                {
                    if (selectedIndices != null && !selectedIndices.Contains(t))
                        continue;
        ...
        void UpdateTargetClipping(int offset = 0)
        {
            ...
            switch (clipMode)
            {
                ...
                case ClipMode.RemovePositive:
                {
                    break;
                }
                case ClipMode.RemoveNegative:
                {
                    cuttingPlane = cuttingPlane.Negated();
                    break;
                }
            }
            ...
            for (int t = backupData.Length - 1; t >= offset; t--)
            {
                ...
                if ((inFrontCount != 0) && (behindCount != 0))
                    clipSides[t] = ClipSide.Split;
                else if ((inFrontCount == 0) && (behindCount > 0))
                    clipSides[t] = ClipSide.CompletelyInside;
                else
                    clipSides[t] = ClipSide.CompletelyOutside;
            ...
            try
            {
                ...
                for (int t = backupData.Length - 1; t >= offset; t--)
                {
                    ...
                    if (clipSides[t] == ClipSide.Split)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                if (
                                    ControlMeshUtility.SplitEdgeLoop(
                                        brush.ControlMesh,
                                        brush.Shape,
                                        edge_loop
                                    )
                                )
                                {
                                    ...
                                    )
                                    {
                                        if (needCopies)
                                        {
                                            ...
                                            backupData[t].brushCopy.ControlMesh =
                                                brush.ControlMesh.Clone();
                                            backupData[t].brushCopy.ControlMesh.SetDirty();
                                            backupData[t].brushCopy.gameObject.hideFlags =
                        ...
                        if (backupData[t].brushCopy != null)
                        {
                            var new_active = !needCopies
                                ? false
                                : (backupData[t].keepCopy ? backupData[t].active : false);
                ...
                for (int t = offset; t < backupData.Length; t++)
                {
                    ...
                    if (
                        backupData[t].brush
                        && !backupData[t].brush.IsRegistered
                        && backupData[t].brush.isActiveAndEnabled
                    )
                    {
                        InternalCSGModelManager.OnCreated(backupData[t].brush);
                        have_copy = true;
                    ...
                    if (
                        ...
                        && !backupData[t].brushCopy.IsRegistered
                        && backupData[t].brushCopy.isActiveAndEnabled
                    )
                    {
                ...
                for (int t = offset; t < backupData.Length; t++)
                {
                    ...
                    if (
                        needCopies
                        && backupData[t].keepCopy
                        && backupData[t].brushCopy
                        && backupData[t].brushCopy.gameObject.activeSelf
                    )
                    {
        ...
        static CSGBrush GetDeepCopy(CSGBrush brush)
        {
            ...
            dstGameObject.name = GameObjectUtility.GetUniqueNameForSibling(
                srcTransform.parent,
                srcGameObject.name
            );
        ...
        {
            if (
                Mathf.Abs(movePlane.Distance(points[0])) < MathConstants.DistanceEpsilon
            ...
            if (
                float.IsNaN(newClipPlane.a)
                || float.IsInfinity(newClipPlane.a)
                || float.IsNaN(newClipPlane.b)
                || float.IsInfinity(newClipPlane.b)
                || float.IsNaN(newClipPlane.c)
                || float.IsInfinity(newClipPlane.c)
                || float.IsNaN(newClipPlane.d)
                || float.IsInfinity(newClipPlane.d)
            ...
            GeometryUtility.CalculateTangents(
                clipPlane.normal,
                out clipPlaneTangent,
                out clipPlaneBinormal
            );
            return true;
        }
        ...
        void UpdateMouseIntersection(SceneView sceneView)
        {
            ...
            if (currentMousePoint.HasValue)
            {
                ...
                currentMousePoint = EditModeManager.SnapPointToGrid(
                    camera,
                    currentMousePoint.Value,
                    movePlane,
                    ref visualSnappedLines,
                    out snapOnBrush,
                    null
                );
                if (snapOnBrush != null)
        ...
        void UpdateMouseCursor()
        {
            ...
            switch (SelectionUtility.GetEventSelectionType())
            {
                ...
                case SelectionType.Subtractive:
                    currentCursor = MouseCursor.ArrowMinus;
                    break;
        ...
        void CreateFirstPoint(SceneView sceneView)
        {
            ...
            points[0] = points[1] = points[2] = intersectionPoint;
                ...
                )
                {
                    ...
                    if (
                        SceneQueryUtility.FindBrushIntersection(
                            currentOnBrush,
                            currentOnBrush.ChildData.ModelTransform.localToWorldMatrix,
                            ray_start,
                            ray_end,
                            out intersectionFar
                        )
        ...
        {
            int meshEditBrushToolID = GUIUtility.GetControlID(
        ...
        [NonSerialized]
        LineMeshManager zTestLineMeshManager2 = new LineMeshManager();

        [NonSerialized]
        LineMeshManager noZTestLineMeshManager2 = new LineMeshManager();

        [NonSerialized]
        LineMeshManager zTestLineMeshManager1 = new LineMeshManager();

        [NonSerialized]
        LineMeshManager noZTestLineMeshManager1 = new LineMeshManager();
        ...
        void OnPaint(SceneView sceneView)
        {
            ...
            if (forceWireframeUpdate)
            {
                ...
                if (clipPlaneValid && backupData.Length > 0)
                {
                    ...
                    if (wireframes.Count > 0)
                    {
                        CSGRenderer.DrawSelectedBrushes(
                            zTestLineMeshManager2,
                            noZTestLineMeshManager2,
                            wireframes.ToArray(),
                ...
                if (clipBrushes != null)
                {
                    ...
                    {
                        CSGRenderer.DrawSelectedBrushes(
                            zTestLineMeshManager1,
                            noZTestLineMeshManager1,
                            ids.ToArray(),
                            transformations.ToArray(),
                            ColorSettings.SelectedOutlines,
                            GUIConstants.thickLineScale
                ...
                if (backupData.Length > 0)
                {
                    ...
                    switch (GeometryUtility.GetPrincipleAxis(x_dir))
                    {
                        ...
                        case PrincipleAxis.X:
                            size_x = (bounds.MaxX - bounds.MinX);
                            break;
                        case PrincipleAxis.Y:
                            size_x = (bounds.MaxY - bounds.MinY);
                            break;
                        case PrincipleAxis.Z:
                            size_x = (bounds.MaxZ - bounds.MinZ);
                            break;
                    }
                    ...
                    switch (GeometryUtility.GetPrincipleAxis(y_dir))
                    {
                        default:
                        case PrincipleAxis.X:
                            size_y = (bounds.MaxX - bounds.MinX);
                            break;
                        case PrincipleAxis.Y:
                            size_y = (bounds.MaxY - bounds.MinY);
                            break;
                        case PrincipleAxis.Z:
                            size_y = (bounds.MaxZ - bounds.MinZ);
                            break;
                    }
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                {
                    ...
                    if (currentMousePoint.HasValue)
                    {
                        ...
                        //if (TryFindIntersection(world_ray, out intersectionPoint))
                        {
                            ...
                            if (this.currentOnBrush == null)
                                PaintUtility.DiamondDotCap(
                                    -1,
                                    intersectionPoint,
                                    MathConstants.identityQuaternion,
                                    handleSize * 1.2f * GUIConstants.handleScale
                                );
                            else
                    ...
                    if (visualSnappedLines != null)
                        PaintUtility.DrawLines(
                            visualSnappedLines.ToArray(),
                            GUIConstants.oldThinLineScale * 2.0f,
                            ColorSettings.SnappedEdges
                        );
                    break;
                }
                case EventType.MouseMove:
                {
                    ...
                    if (
                        ...
                        && clipBrushes != null
                        && currentOnBrush
                        && !ArrayUtility.Contains(clipBrushes, currentOnBrush)
                    )
                    {
                        currentCursor = MouseCursor.ArrowPlus;
                    }
                ...
                case EventType.KeyDown:
                {
                    if (GUIUtility.hotControl == 0)
                    {
                        if (Keys.CycleClipModes.IsKeyPressed())
                        {
                            Event.current.Use();
                            break;
                        }
                    ...
                    if (GUIUtility.hotControl == planeCreationID)
                    {
                        ...
                        if (editMode != EditMode.CreatingPoint1)
                        {
                            ...
                            for (int i = 0; i < pointsShown; i++)
                            {
                                ...
                                Handles.color =
                                    (i == 0)
                                        ? ColorSettings.PointInnerStateColor[3]
                                        : ColorSettings.PointInnerStateColor[0];
                                PaintUtility.SquareDotCap(
                                    -1,
                                    points[i],
                                    MathConstants.identityQuaternion,
            ...
            try
            {
                switch (Event.current.type)
                {
                    case EventType.Layout:
                    {
                        for (int i = 0; i < pointsShown; i++)
                        {
                            HandleUtility.AddControl(
                                ids[i],
                                HandleUtility.DistanceToCircle(points[i], sizes[i] * 1.2f)
                            );
                        }
                        break;
                    }
                    ...
                    case EventType.MouseUp:
                    {
                        ...
                        {
                            if (
                                (camera == null || !assume2DView)
                                && pointsShown == 2
                                && currentMousePoint.HasValue
                                &&
                            ...
                            )
                            {
                                ...
                                if (camera != null && assume2DView)
                                {
                                    intersectionPoint = new CSGPlane(
                                        camera.transform.forward,
                                        points[0]
                                    ).Project(intersectionPoint);
                                }
                                points[2] = intersectionPoint;
                            }
                            else if (!mouseDragged && SelectionUtility.CanHandleSceneClick())
                            {
                                SelectionUtility.DoSelectionClick(sceneView);
                            }
                        }
                    ...
                    case EventType.Repaint:
                    {
                        ...
                        {
                            bool isSelected =
                                (ids[i] == GUIUtility.keyboardControl)
                                || (ids[i] == GUIUtility.hotControl);
                ...
                for (int i = 0; i < pointsShown; i++)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case EventType.MouseDrag:
                        {
                            ...
                            {
                                var newPoint = HandleMovement(
                                    sceneView,
                                    startMousePosition,
                                    Event.current.mousePosition
                                );
                                if (camera != null && assume2DView)
                                    newPoint = new CSGPlane(
                                        camera.transform.forward,
                                        points[i]
                                    ).Project(newPoint);
