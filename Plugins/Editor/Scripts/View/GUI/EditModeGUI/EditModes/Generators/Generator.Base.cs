...
namespace RealtimeCSG
{
    internal abstract class GeneratorBase : ScriptableObject //, IBrushGenerator
    {
        ...
        internal static readonly int ShapeBuilderTangentHash =
            "CSGShapeBuilderTangent".GetHashCode();
        internal static readonly int shapeEditIDHash = "shapeEditID".GetHashCode();

        protected virtual bool IgnoreForcedGridForTangents
        {
            get { return false; }
        }

        public delegate Vector3 SnapToGridFunction(
            Camera camera,
            Vector3 worldPosition,
            CSGPlane plane,
            ref List<Vector3> snappingEdges,
            out CSGBrush snappedOnBrush,
            CSGBrush[] ignoreBrushes,
            bool ignoreAllBrushes = false
        );
        public delegate Vector3 SnapToRayFunction(
            Camera camera,
            Vector3 worldPosition,
            Ray ray,
            ref List<Vector3> snappingEdges,
        ...
        [NonSerialized]
        internal SnapToGridFunction snapFunction = null;

        [NonSerialized]
        internal SnapToRayFunction raySnapFunction = null;

        [NonSerialized]
        internal Action shapeCommitted = null;

        [NonSerialized]
        internal Action shapeCancelled = null;
        ...
        [SerializeField]
        protected EditMode editMode = EditMode.CreatePlane;

        [SerializeField]
        protected EditMode prevEditMode = EditMode.CreatePlane;
        ...
        {
            get { return (editMode != EditMode.CreatePlane); }
        }
        public bool HaveBrushes
        {
            get { return (editMode != EditMode.CreatePlane); }
        }
        ...
        [SerializeField]
        protected CSGOperationType currentCSGOperationType = invalidCSGOperationType;

        [SerializeField]
        protected CSGOperationType forceCurrentCSGOperationType = invalidCSGOperationType;
        ...
        [SerializeField]
        protected CSGModel parentModel;

        [SerializeField]
        protected GameObject operationGameObject;

        [NonSerialized]
        protected CSGModel geometryModel = null;

        [SerializeField]
        protected GameObject[] generatedGameObjects;

        [SerializeField]
        protected CSGBrush[] generatedBrushes;

        [NonSerialized]
        protected int undoGroupIndex;

        [NonSerialized]
        protected List<Vector3> visualSnappedEdges; // used to visualize edges which we're snapping against

        [NonSerialized]
        protected List<Vector3> visualSnappedGrid; // used to visualize grid lines we're snapping against

        [NonSerialized]
        protected CSGBrush visualSnappedBrush; // used to visualize the brush we're snapping on

        [NonSerialized]
        protected CSGPlane buildPlane = new CSGPlane(0, 1, 0, 0);

        [SerializeField]
        protected UnityEngine.Object[] prevSelection;

        [NonSerialized]
        protected bool planeOnGeometry = false;

        [NonSerialized]
        protected bool shapeIsValid = true;
        ...
        [NonSerialized]
        protected bool ignoreOrbit = false;

        [NonSerialized]
        Quaternion originalGridRotation;

        [NonSerialized]
        Vector3 originalGridCenter;

        [NonSerialized]
        bool originalForceGrid;

        [SerializeField]
        protected Vector3 brushPosition;

        [SerializeField]
        protected Quaternion? brushRotation;

        [SerializeField]
        protected bool ignoreModelRotation = false;

        [NonSerialized]
        protected int shapeEditId;

        [NonSerialized]
        protected int shapeId;

        [SerializeField]
        protected Vector3 gridNormal;

        [SerializeField]
        protected Vector3 gridTangent;

        [SerializeField]
        protected Vector3 gridBinormal;

        [SerializeField]
        protected Matrix4x4 toGridMatrix = Matrix4x4.identity;

        [SerializeField]
        protected Matrix4x4 fromGridMatrix = Matrix4x4.identity;

        [SerializeField]
        protected Quaternion toGridQuaternion = Quaternion.identity;

        [SerializeField]
        protected Quaternion fromGridQuaternion = Quaternion.identity;
        ...
        {
            if (editMode == EditMode.EditShape || editMode == EditMode.ExtrudeShape)
            {
                return false;
            }
        ...
        protected void PaintSnapVisualisation()
        {
            ...
            {
                if (
                    visualSnappedBrush.compareTransformation != null
                    && visualSnappedBrush.ChildData != null
                    && visualSnappedBrush.ChildData.ModelTransform
                )
                {
                    var color = ColorSettings.HoverOutlines;
                    var brush_transformation = visualSnappedBrush
                        .compareTransformation
                        .localToWorldMatrix;
                    CSGRenderer.DrawOutlines(
                        visualSnappedBrush.brushNodeID,
        ...
        protected void HandleKeyboard(EventType type)
        {
            ...
            switch (type)
            {
                ...
                case EventType.KeyUp:
                {
                    ...
                    else if (Keys.CancelActionKey.IsKeyPressed())
                    {
                        PerformDeselectAll();
                        Event.current.Use();
                        break;
                    }
                    else if (Keys.DeleteSelectionKey.IsKeyPressed())
                    {
                        PerformDelete();
                        Event.current.Use();
                        break;
        ...
        protected void PaintSideLength(
            Camera camera,
            Vector3 edgeCenter,
        ...
        )
        {
            ...
            else
            {
                PaintUtility.DrawScreenText(
                    textCenter2D,
                    name + Units.ToRoundedDistanceString(length)
                );
            }
        }
        ...
        protected void CalculateWorldSpaceTangents(Camera camera)
        {
            ...
            GeometryUtility.CalculateTangents(
                gridSpaceNormal,
                out gridSpaceTangent,
                out gridSpaceBinormal
            );
        ...
        protected bool GenerateBrushObjects(int brushObjectCount, bool inGridSpace = true)
        {
            ...
            if (!lastUsedModelTransform || !lastUsedModel.isActiveAndEnabled)
            {
            ...
            if (generatedGameObjects == null || generatedGameObjects.Length != brushObjectCount)
            {
                if (generatedBrushes != null && generatedBrushes.Length > 0)
                {
                    for (int i = 0; i < generatedBrushes.Length; i++)
                ...
                {
                    parentGameObject = OperationsUtility.CreateGameObject(
                        lastUsedModelTransform,
                        "Model",
                        true
                    );
                    ...
                    {
                        operationGameObject = OperationsUtility.CreateGameObject(
                ...
                for (int p = 0; p < brushObjectCount; p++)
                {
                    ...
                    if (brushObjectCount == 1)
                        ...
                        name = "Brush (" + p + ")";
                    var gameObject = OperationsUtility.CreateGameObject(
                        parentTransform,
                        name,
                        false
                    );
        ...
        public void EndCommit()
        {
            ...
            { /*
                ...
                var models = operationGameObject.GetComponentsInParent<CSGModel>(
