...
namespace RealtimeCSG
{
    ...
    internal abstract class GeneratorExtrudedBase : GeneratorBase //, IBrushGenerator
    {
        ...
        [SerializeField]
        ShapePolygon[] polygons;

        [SerializeField]
        ShapeEdge[] shapeEdges;

        [SerializeField]
        protected ExtrusionPoint[] extrusionPoints = new ExtrusionPoint[2];

        [SerializeField]
        internal bool haveForcedDirection = false;

        [SerializeField]
        internal Vector3 forcedDirection;

        [SerializeField]
        internal bool smearTextures = true;

        [NonSerialized]
        internal bool forceDragHandle;

        [NonSerialized]
        internal CSGOperationType? forceDragSource = null;

        [NonSerialized]
        internal bool commitExtrusionAfterRelease = false;

        [NonSerialized]
        CSGPlane movePlane;

        [NonSerialized]
        Vector3 movePolygonDirection;

        [NonSerialized]
        bool firstClick = false;

        [NonSerialized]
        Vector3 dragPositionStart = Vector3.zero;

        [NonSerialized]
        Vector3 heightHandleOffset = Vector3.zero;

        [NonSerialized]
        Vector2 heightPosition = Vector2.zero;
        ...
        public bool HaveHeight
        {
            get
            {
                return (editMode == EditMode.ExtrudeShape || editMode == EditMode.EditShape)
        ...
        public bool HaveExtrusion
        {
            get
            {
                return (extrusionPoints.Length > 2)
                    || (
                        (extrusionPoints.Length == 2)
                        && (extrusionPoints[0].Position - extrusionPoints[1].Position).sqrMagnitude
        ...
        public float Height
        {
            ...
            set
            {
                ...
                extrusionPoints[1].Position =
                    extrusionPoints[0].Position + (direction * cleanValue);
                UpdateBaseShape();
            }
        }
        ...
        protected override bool StartEditMode(Camera camera)
        {
            ...
            extrusionPoints = new ExtrusionPoint[2];
            extrusionPoints[1].Position = extrusionPoints[0].Position = brushPosition;
            return true;
        }
        ...
        protected override void CreateControlIDs()
        {
            ...
            {
                var newControlID = GUIUtility.GetControlID(
                    ShapeBuilderCenterHash,
                    FocusType.Passive
                );
        ...
        bool Commit(Camera camera, bool useDefaultHeight)
        {
            ...
            if (!HaveExtrusion)
            {
                if (editMode != EditMode.ExtrudeShape)
                {
                    ...
                    extrusionPoints[1].Position =
                        extrusionPoints[0].Position
                        + (buildPlane.normal.normalized * DefaultHeight);
                }
            }
        ...
        protected void GenerateBrushesFromPolygons(
            ShapeEdge[] shapeEdges = null,
            bool inGridSpace = true
        )
        {
        ...
        protected void UpdateBrushOperation()
        {
            if (forceCurrentCSGOperationType == invalidCSGOperationType)
            {
                if (HaveHeight)
                {
                    bool invertedWorld =
                        parentModel != null
                        && (parentModel.Settings & ModelSettingsFlags.InvertedWorld)
                    ...
                    {
                        if (
                            forceDragSource.HasValue
                            && forceDragSource.Value != CSGOperationType.Additive
                        )
                        {
                            if (Height > 0)
                ...
                if (generatedGameObjects != null && generatedGameObjects.Length > 0)
                {
                    ...
                    for (int slice = 0; slice < extrusionPoints.Length - 1; slice++)
                    {
                        for (int p = 0; p < polygons.Length; p++)
                        {
                            ...
                            var direction = haveForcedDirection
                                ? forcedDirection
                                : buildPlane.normal;
                            var distance = new CSGPlane(
                                direction,
                                extrusionPoints[slice].Position
                            ).Distance(extrusionPoints[slice + 1].Position);
                            if (float.IsInfinity(distance) || float.IsNaN(distance))
                                distance = 1.0f;

                            var poly2dToWorldMatrix =
                                ...
                                * Matrix4x4.TRS(
                                    ...
                                    Vector3.one
                                ); // * parentModel.transform.localToWorldMatrix;

                            ControlMesh newControlMesh;
                            Shape newShape;
                            if (
                                !CreateControlMeshForBrushIndex(
                                    parentModel,
                                    brush,
                                    polygons[p],
                                    poly2dToWorldMatrix,
        ...
        protected void PaintHeightMessage(
            Camera camera,
            Vector3 start,
            Vector3 end,
            Vector3 normal,
        ...
        )
        {
            ...
            var textCenter =
                ...
                + textCenterRay.direction * ((camera.farClipPlane + camera.nearClipPlane) * 0.5f);
            ...
            PaintUtility.DrawScreenText(
                textCenter2D,
                "Y: " + Units.ToRoundedDistanceString(Mathf.Abs(distance))
            );
        }
        ...
        Vector3 GetHeightHandlePosition(SceneView sceneView, Vector3 point)
        {
            ...
            var worldPosition = GeometryUtility.ProjectPointOnInfiniteLine(
                planePosition,
                brushPosition,
                movePolygonDirection
            );
        ...
        )
        {
            ...
            if (dot != 1)
            {
                ...
                else if (closestAxisForward.x == 0 && closestAxisForward.z == 0)
                {
                    v1 = new Vector3(1, 0, 0);
            ...
            {
                tangent = GeometryUtility.SnapToClosestAxis(
                    Vector3.Cross(surfaceDirection, -camera.transform.forward)
                );
            }
            ...
            heightHandleOffset = (
                plane.Distance(GetHeightHandlePosition(sceneView, extrusionPoints[index].Position))
                * movePolygonDirection
            );

            if (
                float.IsInfinity(heightHandleOffset.x)
                || float.IsNaN(heightHandleOffset.x)
                || float.IsInfinity(heightHandleOffset.y)
                || float.IsNaN(heightHandleOffset.y)
                || float.IsInfinity(heightHandleOffset.z)
                || float.IsNaN(heightHandleOffset.z)
            )
        ...
        protected void HandleHeightHandles(
            SceneView sceneView,
            Rect sceneRect,
            bool showHeightValue
        )
        {
            var camera = sceneView.camera;
            for (int p = 0; p < extrusionPoints.Length; p++)
            {
                ...
                switch (type)
                {
                    case EventType.Repaint:
                    {
                        ...
                        else if (HandleUtility.nearestControl == extrusionPoints[p].ID)
                        {
                            state |= SelectState.Hovering;
                        }
                        ...
                        if (polygons != null && outlineVertices.Length >= 3)
                        {
                            ...
                            if (GUIUtility.hotControl == extrusionPoints[p].ID)
                            {
                                ...
                                surfaceColor = ColorSettings.PolygonInnerStateColor[
                                ...
                                && HandleUtility.nearestControl == extrusionPoints[p].ID
                            )
                            {
                                topWireframeColor = ColorSettings.BoundsEdgeHover;
                                surfaceColor = ColorSettings.PolygonInnerStateColor[
                            ...
                            var poly2dToWorldMatrix = Matrix4x4.TRS(
                                extrusionPoints[p].Position,
                                Quaternion.FromToRotation(
                                    MathConstants.upVector3,
                                    buildPlane.normal
                                ),
                            ...
                            for (int i = 0; i < polygons.Length; i++)
                                PaintUtility.DrawPolygon(
                                    poly2dToWorldMatrix,
                                    polygons[i].Vertices,
                                    surfaceColor
                                );

                            poly2dToWorldMatrix = Matrix4x4.TRS(
                                extrusionPoints[p].Position,
                                Quaternion.identity,
                                MathConstants.oneVector3
                            );
                            ...
                            PaintUtility.DrawLine(
                                poly2dToWorldMatrix,
                                outlineVertices[outlineVertices.Length - 1],
                                outlineVertices[0],
                                GUIConstants.oldLineScale,
                                topWireframeColor
                            );
                            ...
                            if (p > 0)
                            {
                                ...
                                var prevPoly2dToWorldMatrix = Matrix4x4.TRS(
                                    prevOffset,
                                    Quaternion.identity,
                                    MathConstants.oneVector3
                                );
                                for (int i = 0; i < outlineVertices.Length; i++)
                                {
                                    var from = prevPoly2dToWorldMatrix.MultiplyPoint(
                                        outlineVertices[i]
                                    );
                        ...
                        var handleSize = CSG_HandleUtility.GetHandleSize(
                            extrusionPoints[p].Position
                        );
                        ...
                        var distance = new CSGPlane(
                            direction,
                            extrusionPoints[p].Position
                        ).Distance(extrusionPoints[1 - p].Position);
                        if (distance <= MathConstants.DistanceEpsilon)
                            PaintUtility.DrawArrowCap(
                                extrusionPoints[p].Position,
                                direction,
                                HandleUtility.GetHandleSize(extrusionPoints[p].Position)
                            );
                        if (distance > -MathConstants.DistanceEpsilon)
                            PaintUtility.DrawArrowCap(
                                extrusionPoints[p].Position,
                    ...
                    case EventType.Layout:
                    {
                        ...
                        var poly2dToWorldMatrix = Matrix4x4.TRS(
                            extrusionPoints[p].Position,
                            Quaternion.FromToRotation(MathConstants.upVector3, buildPlane.normal),
                            MathConstants.oneVector3
                        );
                        var forceOverHandle = IsMouseOverShapePolygons(poly2dToWorldMatrix);
                        HandleUtility.AddControl(
                            extrusionPoints[p].ID,
                            forceOverHandle ? 3.0f : float.PositiveInfinity
                        );

                        var origMatrix = Handles.matrix;
                        Handles.matrix = MathConstants.identityMatrix;
                        float handleSize = CSG_HandleUtility.GetHandleSize(
                            extrusionPoints[p].Position
                        );
                        float scaledHandleSize =
                            handleSize * GUIConstants.handleScale * handle_extension;
                        float distanceToCircle = HandleUtility.DistanceToCircle(
                            extrusionPoints[p].Position,
                            scaledHandleSize
                        );
                        ...
                        if (distance <= MathConstants.DistanceEpsilon)
                            PaintUtility.AddArrowCapControl(
                                extrusionPoints[p].ID,
                                extrusionPoints[p].Position,
                                direction,
                                HandleUtility.GetHandleSize(extrusionPoints[p].Position)
                    ...
                    case EventType.MouseDown:
                    {
                        ...
                        {
                            if (editMode != EditMode.ExtrudeShape && !StartExtrudeMode(camera))
                            {
                                Cancel();
                            }
                            else
                    ...
                    case EventType.MouseMove:
                    {
                        ...
                        if (GUIUtility.hotControl == extrusionPoints[p].ID) // && Event.current.button == 0)
                        {
                            ...
                            Vector3 worldPosition =
                                GetHeightHandlePosition(sceneView, extrusionPoints[p].Position)
                                - heightHandleOffset;
                            if (
                            ...
                            if (raySnapFunction != null)
                            {
                                ...
                                worldPosition = raySnapFunction(
                                    camera,
                                    worldPosition,
                                    new Ray(brushPosition, movePolygonDirection),
                                    ref visualSnappedEdges,
                                    out snappedOnBrush
                                );
                                visualSnappedBrush = snappedOnBrush;
                            }

                            visualSnappedGrid = RealtimeCSG.CSGGrid.FindAllGridEdgesThatTouchPoint(
                                camera,
                                worldPosition
                            );
                    ...
                    case EventType.MouseUp:
                    {
                        ...
                        //forceDragSource = null;
                        if (
                            GUIUtility.hotControl == extrusionPoints[p].ID
                            && Event.current.button == 0
                            && (Tools.viewTool == ViewTool.None || Tools.viewTool == ViewTool.Pan)
                        )
                        {
        ...
        internal abstract bool CreateControlMeshForBrushIndex(
            CSGModel parentModel,
            CSGBrush brush,
            ShapePolygon polygon,
            Matrix4x4 localToWorld,
            float height,
            out ControlMesh newControlMesh,
            out Shape newShape
        );
    }
}
