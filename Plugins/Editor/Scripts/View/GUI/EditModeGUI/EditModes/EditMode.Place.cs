...
namespace RealtimeCSG
{
    internal sealed class EditModePlace : ScriptableObject, IEditMode
    {
        ...
        bool HaveTarget
        {
            get
            {
                return hoverOnTarget != -1 || hoverOnBoundsCenter != -1 || hoverOnBoundsEdge != -1;
            }
        }
        public bool HaveSelection
        {
            get { return brushes.Length != 0 || topTransforms.Length != 0; }
        }
        public bool HaveBrushSelection
        {
            get { return brushes.Length != 0; }
        }
        ...
        [SerializeField]
        PrefabDestinationAlignment destinationSurfaceAlignment =
            PrefabDestinationAlignment.AlignToSurface;
        ...
        #region Rotation
        [SerializeField]
        bool prevPivotModeSet;

        [SerializeField]
        PivotMode prevPivotMode;

        [SerializeField]
        bool worldSpacePivotCenterSet;

        [SerializeField]
        Vector3 worldSpacePivotCenter;
        Vector3 boundsCenter;
        static bool loggedBoundsCenterNaN;
        ...
        {
            get { return worldSpacePivotCenterSet ? worldSpacePivotCenter : Vector3.zero; }
            set
            {
        ...
        #region Bounds
        [SerializeField]
        int[] targetBoundCenterIDs = new int[6];

        [SerializeField]
        int[] targetBoundEdgeIDs = new int[12];

        [SerializeField]
        Vector3[] targetLocalPoints = new Vector3[8];

        [SerializeField]
        Vector3[] targetLocalCenterPoints = new Vector3[6];

        [SerializeField]
        Vector3[] targetLocalEdgePoints = new Vector3[12];

        [SerializeField]
        Vector3[] targetLocalDirections = new Vector3[6];

        [SerializeField]
        AABB targetLocalBounds = new AABB();

        [SerializeField]
        Vector3[] renderLocalCenterPoints = new Vector3[6];

        [SerializeField]
        float[] renderLocalCenterPointSizes = new float[6];

        [SerializeField]
        Color[] renderLocalCenterPointColors = new Color[12];

        [SerializeField]
        Vector3[] renderLocalEdgePoints = new Vector3[12];

        [SerializeField]
        float[] renderLocalEdgePointSizes = new float[12];

        [SerializeField]
        Color[] renderLocalEdgePointColors = new Color[24];

        [SerializeField]
        AABB originalLocalTargetBounds = new AABB();

        [SerializeField]
        Vector3[] originalLocalPoints = new Vector3[8];

        [SerializeField]
        Vector3[] originalLocalCenterPoints = new Vector3[6];

        [SerializeField]
        Vector3[] originalLocalEdgePoints = new Vector3[12];

        [SerializeField]
        bool[] backfaced = new bool[6]; // [-X, +X, -Y, +Y, -Z, +Z]

        [SerializeField]
        CSGPlane[] planes = new CSGPlane[6]; // [-X, +X, -Y, +Y, -Z, +Z]
        #endregion
        ...
        [SerializeField]
        Vector3 prevActiveTransformPosition = Vector3.zero;

        [SerializeField]
        Quaternion prevActiveTransformRotation = Quaternion.identity;

        [SerializeField]
        Vector3 prevActiveTransformScale = Vector3.one;

        [SerializeField]
        Matrix4x4 prevInverseMatrix = Matrix4x4.identity;

        [SerializeField]
        PivotRotation prevToolsRotation = PivotRotation.Local;

        [SerializeField]
        SpaceMatrices activeSpaceMatrices = new SpaceMatrices();
        #endregion

        #region TargetInfo
        [SerializeField]
        Transform[] topTransforms = new Transform[0];

        [SerializeField]
        Vector3[] backupPositions = new Vector3[0]; // transforms

        [SerializeField]
        Quaternion[] backupRotations = new Quaternion[0]; // transforms
        bool ignoreSetTargets = false;

        [SerializeField]
        CSGBrush[] brushes = new CSGBrush[0];

        [SerializeField]
        Transform[] parentModelTransforms = new Transform[0]; // brush

        [SerializeField]
        int[] brushControlIDs = new int[0]; // brush

        [SerializeField]
        Shape[] backupShapes = null; // brush

        [SerializeField]
        ControlMesh[] backupControlMeshes = null; // brush
        #endregion
        ...
        public void SetTargets(FilteredSelection filteredSelection)
        {
            ...
            if (
                brushes.Length == newBrushes.Length
            ...
            for (int i = 0; i < brushes.Length; i++)
            {
                ...
                if (brushes[i].ChildData == null || brushes[i].ChildData.ModelTransform == null)
                {
            ...
            shouldHideTool =
                (CSGSettings.SnapNonCSGObjects && Tools.current == Tool.Move)
        ...
        void UpdateTargetBounds()
        {
            ...
            {
                targetLocalBounds = BoundsUtilities.GetLocalBounds(
                    brushes,
                    activeSpaceMatrices.activeWorldToLocal
                );
            }
            else
            {
            ...
            )
            {
                ...
                if (found)
                {
                    if (Tools.pivotRotation == PivotRotation.Local)
                    {
                        var transformToGridspace =
                            activeSpaceMatrices.activeWorldToLocal
                            * activeTransform.localToWorldMatrix;
                        targetLocalBounds.Extend(
                            transformToGridspace.MultiplyPoint(
                ...
                else if (targetLocalBounds.IsEmpty())
                {
                    if (Tools.pivotRotation == PivotRotation.Local)
                    {
                        var transformToGridspace = activeSpaceMatrices.activeWorldToLocal;
                        targetLocalBounds.Extend(
                            transformToGridspace.MultiplyPoint(activeTransform.position)
                        );
                    }
                    else
        ...
        void UpdateTargetBoundsHandles(Camera camera)
        {
            ...
            var minX = targetLocalPoints[0].x;
            var maxX = targetLocalPoints[1].x;
            var minY = targetLocalPoints[0].y;
            var maxY = targetLocalPoints[4].y;
            var minZ = targetLocalPoints[0].z;
            var maxZ = targetLocalPoints[2].z;
            ...
            var camera_position = activeSpaceMatrices.activeWorldToLocal.MultiplyPoint(
                camera.transform.position
            );
            var camera_direction = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(
            ...
            for (int i = 0; i < BoundsUtilities.AABBSidePointIndices.Length; i++)
            {
                ...
                {
                    backfaced[i] =
                        Mathf.Abs(Vector3.Dot(planes[i].normal, camera_direction)) > 0.9999f;
                }
                else
                {
                    backfaced[i] =
                        Vector3.Dot(
                            planes[i].normal,
                            (camera_position - renderLocalCenterPoints[i])
                        ) < 0;
                }
            }
            ...
            //if (!camera.orthographic)
            {
                ...
                else if (ortho && hoverOnBoundsEdge != -1)
                {
                ...
                {
                    var state = (int)(
                        (i == point1 || i == point2) ? SelectState.Hovering : SelectState.None
                    );
                    ...
                    {
                        case PrincipleAxis.X:
                            disabled = RealtimeCSG.CSGSettings.LockAxisX;
                            break;
                        case PrincipleAxis.Y:
                            disabled = RealtimeCSG.CSGSettings.LockAxisY;
                            break;
                        case PrincipleAxis.Z:
                            disabled = RealtimeCSG.CSGSettings.LockAxisZ;
                            break;
                    }
                    if (disabled)
                        color2 = Color.red;
                ...
                {
                    var state = (int)(
                        (i == hoverOnBoundsEdge) ? SelectState.Hovering : SelectState.None
                    );
                    var color1 = ColorSettings.MeshEdgeOutline;
                    ...
                    {
                        case PrincipleAxis.X:
                            disabled1 = RealtimeCSG.CSGSettings.LockAxisX;
                            break;
                        case PrincipleAxis.Y:
                            disabled1 = RealtimeCSG.CSGSettings.LockAxisY;
                            break;
                        case PrincipleAxis.Z:
                            disabled1 = RealtimeCSG.CSGSettings.LockAxisZ;
                            break;
                    }
                    ...
                    {
                        case PrincipleAxis.X:
                            disabled2 = RealtimeCSG.CSGSettings.LockAxisX;
                            break;
                        case PrincipleAxis.Y:
                            disabled2 = RealtimeCSG.CSGSettings.LockAxisY;
                            break;
                        case PrincipleAxis.Z:
                            disabled2 = RealtimeCSG.CSGSettings.LockAxisZ;
                            break;
                    }
        ...
        public void RecenterPivot()
        {
            ...
            if (Tools.pivotMode == PivotMode.Center || !Selection.activeTransform)
                return;

            UpdateTargetBounds();

            ignorePivotChange = true;
            boundsCenter = activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                targetLocalBounds.Center
            );
                ...
                if (difference != Vector3.zero)
                {
                    if (cloneDragKeyPressed && !haveCloned)
                    {
                        ...
                        topTransforms = EditModeManager.CloneTargets(
                            delegate(Transform newTransform, Transform originalTransform)
                            {
                                newTransform.localScale = originalTransform.localScale;
                                newTransform.localPosition =
                                    originalTransform.localPosition + difference;
        ...
        void UpdateGrid(Camera camera)
        {
            ...
            if (hoverOnBoundsCenter != -1)
            {
                ...
                worldDirection = activeSpaceMatrices.activeLocalToWorld.MultiplyVector(
                    worldDirection
                );
        ...
        {
            if (Math.Abs(movePlane.a) > 1 - MathConstants.NormalEpsilon)
                offset.x = 0.0f;
            if (Math.Abs(movePlane.b) > 1 - MathConstants.NormalEpsilon)
                offset.y = 0.0f;
            if (Math.Abs(movePlane.c) > 1 - MathConstants.NormalEpsilon)
                offset.z = 0.0f;
            return offset;
        }

        bool MoveRotateObjects(
        ...
        )
        {
            ...
            {
                transforms[t].position = GridUtility.CleanPosition(
                    rotate_matrix.MultiplyPoint(backupPositions[t] + offset)
                );
        ...
        void RotateObjects(Transform[] transforms, Vector3 center, Quaternion rotationQuaternion)
        {
            ...
            {
                transforms[t].position = GridUtility.CleanPosition(
                    rotate_matrix.MultiplyPoint(backupPositions[t])
                );
                transforms[t].rotation = rotationQuaternion * backupRotations[t];
            }
        ...
        {
            currentAngle = GeometryUtility.SignedAngle(
                center - pointStart,
                center - currentPoint,
                normal
            );
        ...
        )
        {
            ...
            switch (snapMode)
            {
                ...
                {
                    var localPlane = GeometryUtility.TransformPlane(
                        activeSpaceMatrices.activeWorldToLocal,
                        movePlane
                    );
                    ...
                    offset = RealtimeCSG.CSGGrid.SnapDeltaToGrid(
                        camera,
                        offset,
                        localPoints,
                        snapToSelf: true
            ...
            if (offset != MathConstants.zeroVector3 && cloneDragKeyPressed)
            {
                if (toolEditMode == ToolEditMode.MovingObject)
                {
                    ...
                    transforms = EditModeManager.CloneTargets(
                        delegate(Transform newTransform, Transform originalTransform)
                        {
                            newTransform.localScale = originalTransform.localScale;
        ...
        void MoveBoundsCenter(
            Camera camera,
            int boundsCenterIndex,
            Vector3 offset,
            out Vector3 snappedOffset,
            SnapMode snapMode
        )
        {
            ...
            switch (snapMode)
            {
                case SnapMode.GridSnapping:
                {
                    var worldLineOrg = activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                        originalLocalCenterPoints[boundsCenterIndex]
                    );
                    var worldLineDir = activeSpaceMatrices.activeLocalToWorld.MultiplyVector(
                        targetLocalDirections[boundsCenterIndex]
                    );
                    var worldPoints = new Vector3[]
                    {
                        activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                            originalLocalCenterPoints[boundsCenterIndex]
                        ),
                    };
                    worldOffset = RealtimeCSG.CSGGrid.SnapDeltaToRayGrid(
                        camera,
                    ...
                    localOffset = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(
                        worldOffset
                    );
                    break;
                }
                case SnapMode.RelativeSnapping:
                {
                    ...
                    worldOffset = RealtimeCSG.CSGGrid.SnapDeltaToRayRelative(
                        camera,
                        new Ray(worldLineOrg, worldLineDir),
                        worldOffset
                    );
            ...
            targetLocalCenterPoints[boundsCenterIndex] = GridUtility.CleanPosition(
                originalLocalCenterPoints[boundsCenterIndex] + localOffset
            );
            ...
            for (int p = 0; p < boundPoints.Length; p++)
            {
                ...
                targetLocalPoints[index] = GridUtility.CleanPosition(
                    originalLocalPoints[index] + localOffset
                );
            }
            ...
            var old_delta_x = -(
                new CSGPlane(targetLocalDirections[0], originalLocalCenterPoints[0]).Distance(
                    originalLocalCenterPoints[1]
                )
            );
            var old_delta_y = -(
                new CSGPlane(targetLocalDirections[2], originalLocalCenterPoints[2]).Distance(
                    originalLocalCenterPoints[3]
                )
            );
            var old_delta_z = -(
                new CSGPlane(targetLocalDirections[4], originalLocalCenterPoints[4]).Distance(
                    originalLocalCenterPoints[5]
                )
            );

            var new_delta_x =
                old_delta_x + (((boundsCenterIndex & 1) == 1) ? localOffset.x : -localOffset.x);
            var new_delta_y =
                old_delta_y + (((boundsCenterIndex & 1) == 1) ? localOffset.y : -localOffset.y);
            var new_delta_z =
                old_delta_z + (((boundsCenterIndex & 1) == 1) ? localOffset.z : -localOffset.z);

            var world_center_before =
                (originalLocalCenterPoints[index1] + originalLocalCenterPoints[index2]) * 0.5f;
            var world_center_after = world_center_before + (localOffset * 0.5f);

            var world_scale = new Vector3(
                new_delta_x / old_delta_x,
                new_delta_y / old_delta_y,
                new_delta_z / old_delta_z
            );
            ...
            ControlMeshUtility.TransformBrushes(
                brushes,
                world_center_before,
                world_center_after,
                world_scale,
        ...
        )
        {
            ...
            switch (snapMode)
            {
                case SnapMode.GridSnapping:
                {
                    ...
                    {
                        activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                            originalLocalEdgePoints[edgeCenter]
                        ),
                    };
                    worldOffset = RealtimeCSG.CSGGrid.SnapDeltaToGrid(
                        camera,
                        worldOffset,
                        worldPoints,
                        snapToSelf: true
        ...
        {
            currentCursor = CursorUtility.GetSelectionCursor(
                SelectionUtility.GetEventSelectionType()
            );
        }

        [NonSerialized]
        bool ignorePivotChange = false;
        ...
        void UpdateTransforms(Camera camera)
        {
            ...
            else if (activeTransform != null && !mouseIsDragging)
            {
                if (
                ...
                )
                {
                    ...
                    {
                        Matrix4x4 transform =
                            activeTransform.localToWorldMatrix * prevInverseMatrix;
        ...
        )
        {
            ...
            var worldVertex1 = localToWorld.MultiplyPoint(
                targetLocalPoints[BoundsUtilities.AABBEdgeIndices[hoverOnBoundsEdge][0]]
            );
            var worldVertex2 = localToWorld.MultiplyPoint(
                targetLocalPoints[BoundsUtilities.AABBEdgeIndices[hoverOnBoundsEdge][1]]
            );
            ...
            rotateStartAngle = GeometryUtility.SignedAngle(
                rotateCenter - rotateStart,
                rotateCenter - rotateMousePosition,
                rotateNormal
            );
        ...
        void RenderHighlightedEdge(int highlightEdge)
        {
            ...
            PaintUtility.DrawLine(
                activeSpaceMatrices.activeLocalToWorld,
                localPoint1,
                localPoint2,
                GUIConstants.oldThickLineScale,
                ColorSettings.BoundsEdgeHover
            );
            PaintUtility.DrawDottedLine(
                activeSpaceMatrices.activeLocalToWorld,
                localPoint2,
                localPoint1,
                ColorSettings.BoundsEdgeHover
        ...
        void RenderRotationCircle(Camera camera)
        {
            ...
            switch (rotateAxis)
            {
                default:
                case PrincipleAxis.X:
                    name = "X"; /*color = ColorSettings.XAxis;*/
                    break;
                case PrincipleAxis.Y:
                    name = "Y"; /*color = ColorSettings.YAxis;*/
                    break;
                case PrincipleAxis.Z:
                    name = "Z"; /*color = ColorSettings.ZAxis;*/
                    break;
            }
            ...
            if (mouseIsDragging)
            {
                PaintUtility.DrawRotateCircle(
                    ...
                    rotateCurrentAngle,
                    rotateStartAngle,
                    rotateCurrentSnappedAngle,
                    color,
                    name
                ); //, ColorSettings.RotateCircleHatches, name);
                PaintUtility.DrawRotateCirclePie(
                    ...
                    rotateCurrentSnappedAngle,
                    color
                ); //, RotateCirclePieFill, ColorSettings.RotateCirclePieOutline);
            }
            else
            {
                PaintUtility.DrawRotateCircle(
            ...
            {
                PaintUtility.DrawDottedLine(
                    worldSpacePivotCenter,
                    rotateCenter,
                    ColorSettings.BoundsEdgeHover
                );
                PaintUtility.DrawProjectedPivot(
                    camera,
                    rotateCenter,
                    ColorSettings.BoundsEdgeHover
                );
        ...
        void RenderArrowCap(int boundsCenterIndex)
        {
            ...
            var localOrigin = activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                targetLocalCenterPoints[boundsCenterIndex]
            );
            var localDirection = activeSpaceMatrices
                .activeLocalToWorld.MultiplyVector(targetLocalDirections[boundsCenterIndex])
                .normalized;
            PaintUtility.DrawArrowCap(
                localOrigin,
                localDirection,
                HandleUtility.GetHandleSize(localOrigin) * 0.75f
            );
        ...
        public void HandleEvents(SceneView sceneView, Rect sceneRect)
        {
            ...
            else if (originalEventType == EventType.MouseDrag)
            {
                if (
                    !mouseIsDragging
                    && (prevMousePos - Event.current.mousePosition).magnitude > 4.0f
                )
                {
            ...
            try
            {
                if (draggingOnCamera != null)
                    inCamera = false;
                if (draggingOnCamera == camera)
                    inCamera = true;
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    if (!SceneDragToolManager.IsDraggingObjectInScene)
                    {
                        ...
                        {
                            if (
                                (prevModifiers & EventModifiers.Shift)
                    ...
                    if (lastLineMeshGeneration != InternalCSGModelManager.MeshGeneration)
                    {
                        ...
                        for (int i = brushes.Length - 1; i >= 0; i--)
                        {
                            ...
                            brushTransformations[i] = brush
                                .compareTransformation
                                .localToWorldMatrix;
                            brushNodeIDs[i] = brush.brushNodeID;
                        }
                        CSGRenderer.DrawSelectedBrushes(
                            zTestLineMeshManager,
                            noZTestLineMeshManager,
                            brushNodeIDs,
                            brushTransformations,
                            ColorSettings.SelectedOutlines,
                    ...
                    if (!SceneDragToolManager.IsDraggingObjectInScene)
                    {
                        ...
                        {
                            PaintUtility.DrawLines(
                                activeSpaceMatrices.activeLocalToWorld,
                                targetLocalPoints,
                                BoundsUtilities.AABBLineIndices,
                                GUIConstants.oldLineScale * 2.0f,
                                meshEdgeOutlineColor
                            );
                            PaintUtility.DrawUnoccludedLines(
                                activeSpaceMatrices.activeLocalToWorld,
                                targetLocalPoints,
                                BoundsUtilities.AABBLineIndices,
                                meshEdgeOutlineColor
                            );
                            PaintUtility.DrawDottedLines(
                                activeSpaceMatrices.activeLocalToWorld,
                                targetLocalPoints,
                                BoundsUtilities.AABBLineIndices,
                                boundOutlinesColor,
                        ...
                        if (!sceneView.camera.orthographic && CSGSettings.GridVisible)
                        {
                            ...
                            if (!mouseIsDragging || (Tools.current != Tool.Rotate))
                            {
                                ...
                                {
                                    var pt0 = activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                                        targetLocalPoints[i]
                                    );
                            ...
                            if (max > 0)
                            {
                                ...
                                PaintUtility.DrawDottedWirePolygon(
                                    vertices,
                                    ColorSettings.ShadowOutlineColor
                                );
                                PaintUtility.DrawWirePolygon(
                                    vertices,
                                    ColorSettings.ShadowOutlineColor
                                );
                            }
                ...
                if (Selection.activeTransform != null)
                {
                    if (Tools.current == Tool.Rotate)
                    {
                        if (worldSpacePivotCenterSet && HaveBrushSelection)
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            {
                                ...
                                newPivot = PaintUtility.HandlePivot(
                                    camera,
                                    newPivot,
                                    rotation,
                                    ColorSettings.BoundsEdgeHover,
                    ...
                    {
                        bool boundsCenterIsNaN =
                            float.IsNaN(boundsCenter.x)
                            || float.IsNaN(boundsCenter.y)
                            || float.IsNaN(boundsCenter.z);
                        if (boundsCenterIsNaN && HaveSelection && !loggedBoundsCenterNaN)
                        {
                            loggedBoundsCenterNaN = true;
                            Debug.LogWarning(
                                "RealtimeCSG Place mode: boundsCenter is NaN; skipping Move handle to avoid Scene View lockup."
                            );
                        }
                        if (HaveSelection && !boundsCenterIsNaN)
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            {
                                ...
                                newPosition = RealtimeCSG.Helpers.CSGHandles.PositionHandle(
                                    camera,
                                    newPosition,
                ...
                switch (Event.current.type)
                {
                    ...
                    case EventType.KeyUp:
                    {
                        ...
                        {
                            UpdateTargetInfo();
                            var center = activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                                targetLocalBounds.Center
                            );
                            RotateObjects(
                                topTransforms,
                                ...
                                RealtimeCSG.CSGSettings.SnapRotation
                            );
                            Event.current.Use();
                            break;
                        }
                        ...
                        if (Keys.MoveSelectionLeft.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.rightVector3 * RealtimeCSG.CSGSettings.SnapVector.x
                            );
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MoveSelectionRight.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.leftVector3 * RealtimeCSG.CSGSettings.SnapVector.x
                            );
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MoveSelectionBack.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.forwardVector3 * RealtimeCSG.CSGSettings.SnapVector.z
                            );
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MoveSelectionForward.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.backVector3 * RealtimeCSG.CSGSettings.SnapVector.z
                            );
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MoveSelectionDown.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.downVector3 * RealtimeCSG.CSGSettings.SnapVector.y
                            );
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MoveSelectionUp.IsKeyPressed())
                        {
                            ...
                            MoveObjects(
                                topTransforms,
                                MathConstants.upVector3 * RealtimeCSG.CSGSettings.SnapVector.y
                            );
                            Event.current.Use();
                            break;
                        }
                    ...
                    case EventType.MouseDown:
                    {
                        ...
                        if (
                            RealtimeCSG.CSGSettings.LockAxisX
                        ...
                        try
                        {
                            if (inCamera && !mouseIsDragging && GUIUtility.hotControl == 0)
                            {
                                ...
                                if (activeTransform != null)
                                {
                                    ...
                                    var world_ray = HandleUtility.GUIPointToWorldRay(
                                    ...
                                    for (int t = 0; t < brushes.Length; t++)
                                    {
                                        ...
                                        var modelTransformation =
                                            model_transform.localToWorldMatrix;

                                        LegacyBrushIntersection intersection;
                                        if (
                                            SceneQueryUtility.FindBrushIntersection(
                                                brush,
                                                modelTransformation,
                                                ray_start,
                                                ray_end,
                                        ...
                                        {
                                            var distance = (
                                                intersection.worldIntersection - ray_start
                                            ).magnitude;
                                    ...
                                    if (closest_brush == -1 && brushes.Length > 0)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            local_ray.direction = worldToLocal.MultiplyVector(
                                                local_ray.direction
                                            );
                                            local_ray.origin = worldToLocal.MultiplyPoint(
                                                local_ray.origin
                                            );
                                    ...
                                    //if (!camera.orthographic)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            {
                                                float radius =
                                                    renderLocalCenterPointSizes[t]
                                                    * GUIConstants.hoverHandleScale;
                                                if (radius == 0)
                                                    continue;
                                                float distance =
                                                    HandleUtility.DistanceToCircle(
                                                        matrix.MultiplyPoint(
                                                            targetLocalCenterPoints[t]
                                                        ),
                                                        radius
                                                    ) * 0.5f;
                                                HandleUtility.AddControl(
                                                    targetBoundCenterIDs[t],
                                                    distance
                                                );
                                            }
                                            ...
                                            if (nearestControl > 0)
                                            {
                                                ...
                                                )
                                                {
                                                    if (targetBoundCenterIDs[t] == nearestControl)
                                                    {
                                                        ...
                                                        newCursor =
                                                            CursorUtility.GetCursorForDirection(
                                                                Matrix4x4.identity,
                                                                targetLocalCenterPoints[t],
                                                                targetLocalDirections[t]
                                                            );
                                                        foundControl = true;
                                                        break;
                                                    }
                                        ...
                                        )
                                        {
                                            ...
                                            {
                                                var cameraPlane = CSG_HandleUtility.GetNearPlane(
                                                    camera
                                                );
                                                ...
                                                */
                                                ...
                                                {
                                                    var side1 = backfaced[
                                                        BoundsUtilities.AABBEdgeSides[t][0]
                                                    ];
                                                    var side2 = backfaced[
                                                        BoundsUtilities.AABBEdgeSides[t][1]
                                                    ];
                                                    if (
                                                        Tools.current == Tool.Rotate
                                                        && (side1 && side2)
                                                    ...
                                                    var point1 = targetLocalPoints[
                                                        BoundsUtilities.AABBEdgeIndices[t][0]
                                                    ];
                                                    var point2 = targetLocalPoints[
                                                        BoundsUtilities.AABBEdgeIndices[t][1]
                                                    ];
                                                    ...
                                                    )
                                                    {
                                                        ...
                                                        )
                                                        {
                                                            distance =
                                                                HandleUtility.DistanceToCircle(
                                                                    matrix.MultiplyPoint(
                                                                        renderLocalEdgePoints[t]
                                                                    ),
                                                                    renderLocalEdgePointSizes[t]
                                                                ) * 0.5f;
                                                            HandleUtility.AddControl(
                                                                targetBoundEdgeIDs[t],
                                                                distance
                                                            );
                                                        }
                                                        else
                                                    ...
                                                    {
                                                        distance = CameraUtility.DistanceToLine(
                                                            cameraPlane,
                                                            mousePoint,
                                                            matrix.MultiplyPoint(point1),
                                                    ...
                                                    if (targetBoundEdgeIDs[t] == nearestControl)
                                                    {
                                                        ...
                                                        )
                                                        {
                                                            ...
                                                            var side2 =
                                                                BoundsUtilities.AABBEdgeSides[t][1];
                                                            var localSideDir1 =
                                                                targetLocalDirections[side1];
                                                            var localSideDir2 =
                                                                targetLocalDirections[side2];
                                                            var localNormal = Vector3.Cross(
                                                                localSideDir1,
                                                                localSideDir2
                                                            );

                                                            movePlane = new CSGPlane(
                                                                GridUtility.CleanNormal(
                                                                    activeSpaceMatrices.activeLocalToWorld.MultiplyVector(
                                                                        localNormal
                                                                    )
                                                                ),
                                                                activeSpaceMatrices.activeLocalToWorld.MultiplyPoint(
                                                            ...
                                                            if (
                                                                ...
                                                                RealtimeCSG.CSGGrid.SetForcedGrid(
                                                                    camera,
                                                                    movePlane
                                                                );
                                                            ...
                                                            for (int i = 0; i < 2; i++)
                                                            {
                                                                ...
                                                                if (radius != 0)
                                                                {
                                                                    ...
                                                                    newCursor =
                                                                        CursorUtility.GetCursorForDirection(
                                                                            Matrix4x4.identity,
                                                                            targetLocalCenterPoints[
                                                                                side
                                                                            ],
                                                                            targetLocalDirections[
                                                                                side
                                                                            ]
                                                                        );
                                                                    break;
                ...
                for (int i = 0; i < targetBoundCenterIDs.Length; i++)
                {
                    ...
                    switch (type)
                    {
                        case EventType.MouseDrag:
                        {
                            ...
                            if (
                                float.IsNaN(worldIntersection.x)
                                || float.IsNaN(worldIntersection.y)
                                || float.IsNaN(worldIntersection.z)
                            )
                                break;
                            ...
                            var localIntersection =
                                activeSpaceMatrices.activeWorldToLocal.MultiplyPoint(
                                    worldIntersection
                                );
                        ...
                        case EventType.Repaint:
                        {
                            ...
                            if (Tools.pivotRotation == PivotRotation.Local)
                            {
                                delta = GridUtility.CleanPosition(
                                    activeSpaceMatrices.activeLocalToWorld.MultiplyVector(delta)
                                );
                            }
                                ...
                                switch (BoundsUtilities.AABBSideAxis[hoverOnBoundsCenter])
                                {
                                    case PrincipleAxis.X:
                                        PaintUtility.DrawScreenText(
                                            textCenter2D,
                                            "X axis is locked",
                                            CSG_GUIStyleUtility.redTextArea
                                        );
                                        break;
                                    case PrincipleAxis.Y:
                                        PaintUtility.DrawScreenText(
                    ...
                    switch (type)
                    {
                        case EventType.MouseDrag:
                        {
                            ...
                            if (firstMove)
                            {
                                ...
                                if (activeTransform != null)
                                {
                                    ...
                                    if (topNode)
                                    {
                                        ...
                                        destinationSurfaceAlignment =
                                            topNode.PrefabDestinationAlignment;
                                    }
                                }

                                BoundsUtilities.GetBoundsCornerPoints(
                                    targetLocalBounds,
                                    projectedBounds
                                );
                            }
                            ...
                            )
                            {
                                ...
                                worldIntersection = GeometryUtility.ProjectPointOnPlane(
                                    movePlane,
                                    worldIntersection
                                );
                                if (firstMove)
                            ...
                            if (useRaySnapping)
                            {
                                ...
                                try
                                {
                                    if (hoverOverBrushIndex < backupRotations.Length)
                                    {
                                        ...
                                        var intersection = SceneQueryUtility.FindMeshIntersection(
                                            camera,
                                            Event.current.mousePosition
                                        );
                                        ...
                                        var hoverRotation = SelectionUtility.FindDragOrientation(
                                            sceneView,
                                            normal,
                                            sourceSurfaceAlignment,
                                            destinationSurfaceAlignment
                                        );

                                        RealtimeCSG.CSGGrid.SetForcedGrid(
                                            camera,
                                            intersection.worldPlane
                                        );
                                        ...
                                        switch (activeSnappingMode)
                                        {
                                            case SnapMode.GridSnapping:
                                            {
                                                ...
                                                for (var p = 0; p < localPoints.Length; p++)
                                                    localPoints[p] =
                                                        GeometryUtility.ProjectPointOnPlane(
                                                            localPlane,
                                                            (hoverRotation * projectedBounds[p])
                                                                + hoverPosition
                                                        );
                                        ...
                                        hoverPosition = GeometryUtility.ProjectPointOnPlane(
                                            intersection.worldPlane,
                                            hoverPosition
                                        ); // + (normal * 0.01f);
                                        ...
                                        UpdateTargetBounds();
                                        CSG_EditorGUIUtility.RepaintAll();
                                        var center = GridUtility.CleanPosition(
                                            brushPosition + worldDeltaMovement
                                        );
                                        var rotation =
                                            hoverRotation * Quaternion.Inverse(brushRotation);
                                ...
                                finally
                                {
                                    ...
                                    {
                                        if (brushes[b])
                                            BrushOutlineManager.ForceUpdateOutlines(
                                                brushes[b].brushNodeID
                                            );
                                    }
                        ...
                        case EventType.Repaint:
                        {
                            ...
                            var lockX =
                                (Mathf.Abs(movement.x) < MathConstants.ConsideredZero)
                                && (
                                    RealtimeCSG.CSGSettings.LockAxisX
                                    || (Mathf.Abs(movePlane.a) >= 1 - MathConstants.EqualityEpsilon)
                                );
                            var lockY =
                                (Mathf.Abs(movement.y) < MathConstants.ConsideredZero)
                                && (
                                    RealtimeCSG.CSGSettings.LockAxisY
                                    || (Mathf.Abs(movePlane.b) >= 1 - MathConstants.EqualityEpsilon)
                                );
                            var lockZ =
                                (Mathf.Abs(movement.z) < MathConstants.ConsideredZero)
                                && (
                                    RealtimeCSG.CSGSettings.LockAxisZ
                                    || (Mathf.Abs(movePlane.c) >= 1 - MathConstants.EqualityEpsilon)
                                );
        ...
        public void FlipX()
        {
            BrushOperations.FlipX(brushes);
            UpdateTargetInfo();
        }

        public void FlipY()
        {
            BrushOperations.FlipY(brushes);
            UpdateTargetInfo();
        }

        public void FlipZ()
        {
            BrushOperations.FlipZ(brushes);
            UpdateTargetInfo();
        }
