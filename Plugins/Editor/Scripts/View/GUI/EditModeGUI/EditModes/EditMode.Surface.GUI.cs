...
namespace RealtimeCSG
{
    ...
    {
        private static readonly int SceneViewSurfaceOverlayHash =
            "SceneViewSurfaceOverlay".GetHashCode();
        ...
        static void OnGUIContentsJustify(
            bool isSceneGUI,
            SelectedBrushSurface[] selectedBrushSurfaces
        )
        {
        ...
        static void OnGUIContentsJustifyScene(
            Rect rect,
            SelectedBrushSurface[] selectedBrushSurfaces
        )
        {
        ...
        )
        {
            GUILayout.BeginHorizontal(materialWidth, materialHeight);
            {
                ...
                else
                {
                    GUILayout.Box(
                        new GUIContent(),
                        CSG_GUIStyleUtility.emptyMaterialStyle,
                        materialWidth,
                        materialHeight
                    );
                }
            ...
            {
                if (Event.current.type == EventType.DragUpdated && GetDragMaterial())
                {
        ...
        sealed class SurfaceState
        {
            ...
            public EditModeCommonGUI.SurfaceFlagState surfaceFlagState =
                new EditModeCommonGUI.SurfaceFlagState();

            public void Init(EditModeSurface tool)
            {
                selectedBrushSurfaces =
                    (tool == null) ? new SelectedBrushSurface[0] : tool.GetSelectedSurfaces();
                ...
                if (selectedBrushSurfaces.Length > 0)
                {
                    for (var i = 0; i < selectedBrushSurfaces.Length; i++)
                    {
                        ...
                        else if ((texGenFlags & TexGenFlags.NoRender) == TexGenFlags.NoRender)
                        {
                            foundHelperMaterial = true;
                            if (!firstRenderSurfaceType.HasValue)
                            {
                        ...
                        else
                        {
                            ...
                            {
                                var surfaceType = MaterialUtility.GetMaterialSurfaceType(
                                    surfaceMaterial
                                );
                                if (!firstRenderSurfaceType.HasValue)
                        ...
                        if (
                            (texGenFlags & TexGenFlags.WorldSpaceTexture)
                            == TexGenFlags.WorldSpaceTexture
                        )
                        {
                            if (i == 0)
                                textureLocked = false;
                        ...
                        { /*
                            ...
                            if (!multipleTranslationX || !multipleTranslationY)
                            {
                                ...
                                multipleTranslationX =
                                    multipleTranslationX
                                    || currentTexGen.Translation.x != translation.x;
                                multipleTranslationY =
                                    multipleTranslationY
                                    || currentTexGen.Translation.y != translation.y;
                            }

                            if (!multipleRotationAngle)
                            {
                                var rotationAngle = brush.Shape.TexGens[texGenIndex].RotationAngle;
                                multipleRotationAngle =
                                    currentTexGen.RotationAngle != rotationAngle;
                            }
                        }
                    }
                    ...
                    {
                        if (firstRenderSurfaceType.HasValue)
                            firstMaterial = MaterialUtility.GetSurfaceMaterial(
                                firstRenderSurfaceType.Value
                            );
                        else
                ...
                currentTexGen.Scale.x =
                    Mathf.RoundToInt(currentTexGen.Scale.x * scale_round) / scale_round;
                currentTexGen.Scale.y =
                    Mathf.RoundToInt(currentTexGen.Scale.y * scale_round) / scale_round;
                currentTexGen.Translation.x =
                    Mathf.RoundToInt(currentTexGen.Translation.x * scale_round) / scale_round;
                currentTexGen.Translation.y =
                    Mathf.RoundToInt(currentTexGen.Translation.y * scale_round) / scale_round;
                currentTexGen.RotationAngle =
                    Mathf.RoundToInt(currentTexGen.RotationAngle * scale_round) / scale_round;
        ...
        const int surfaceEditWindowHeight =
            surfaceEditTitleHeight
            + materialViewBoxHeight
            + boxOffset
            + textureLockedBoxHeight
            + boxOffset
            + selectionBoxHeight
            + boxOffset
            + transformationBoxHeight
            + boxOffset
            + uvCommandsBoxHeight
            + boxOffset
            + surfaceFlagsBoxHeight
            + boxOffset
            + 13;

        private static void OnGUIContents(
            Rect rect,
            EditModeSurface tool,
            bool needLightmapUVUpdate
        )
        {
            ...
            {
                {
                    tempRect.Set(
                        ...
                        rect.y + offset,
                        boxWidth,
                        materialViewBoxHeight + contentOffset
                    );
                    ...
                    EditorGUI.BeginDisabledGroup(!surfaceState.enabled);
                    {
                        //EditorGUI.BeginDisabledGroup(false);
                        {
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                if (newMaterial)
                                {
                                    SurfaceUtility.SetMaterials(
                                        surfaceState.selectedBrushSurfaces,
                                        newMaterial
                                    );
...

                        {
                            ...
                            OnGUIContentsMaterialImage(
                                tempRect,
                                surfaceState.firstMaterial,
                                surfaceState.multipleMaterials,
                                surfaceState.selectedBrushSurfaces
                            );
            ...
            {
                {
                    ...
                    GUI.Box(tempRect, GUIContent.none, GUI.skin.box);
                    {
                        EditorGUI.BeginChangeCheck();
                        {
                            ...
                            surfaceState.textureLocked = EditorGUI.ToggleLeft(
                                tempRect,
                                ContentLockTexture,
                                surfaceState.textureLocked.HasValue
                                    ? surfaceState.textureLocked.Value
                                    : false
                            );
                    ...
                    {
                        {
...

                            {
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    surfaceState.currentTexGen.Scale.x = EditorGUI.FloatField(
                                        tempRect,
                                        surfaceState.currentTexGen.Scale.x
                                    );
                                }
                                if (EditorGUI.EndChangeCheck())
                                {
                                    SurfaceUtility.SetScaleX(
                                        surfaceState.selectedBrushSurfaces,
                                        surfaceState.currentTexGen.Scale.x
                                    );
                                }
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    surfaceState.currentTexGen.Scale.y = EditorGUI.FloatField(
                                        tempRect,
                                        surfaceState.currentTexGen.Scale.y
                                    );
                                }
                                if (EditorGUI.EndChangeCheck())
                                {
                                    SurfaceUtility.SetScaleY(
                                        surfaceState.selectedBrushSurfaces,
                                        surfaceState.currentTexGen.Scale.y
                                    );
                                }
                        ...
                        }
                        {
...

                            {
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    surfaceState.currentTexGen.Translation.x = EditorGUI.FloatField(
                                        tempRect,
                                        surfaceState.currentTexGen.Translation.x
                                    );
                                }
                                if (EditorGUI.EndChangeCheck())
                                {
                                    SurfaceUtility.SetTranslationX(
                                        surfaceState.selectedBrushSurfaces,
                                        surfaceState.currentTexGen.Translation.x
                                    );
                                }
                                ...
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    surfaceState.currentTexGen.Translation.y = EditorGUI.FloatField(
                                        tempRect,
                                        surfaceState.currentTexGen.Translation.y
                                    );
                                }
                                if (EditorGUI.EndChangeCheck())
                                {
                                    SurfaceUtility.SetTranslationY(
                                        surfaceState.selectedBrushSurfaces,
                                        surfaceState.currentTexGen.Translation.y
                                    );
                                }
                        ...
                        }
                        {
...

                            {
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    surfaceState.currentTexGen.RotationAngle = EditorGUI.FloatField(
                                        tempRect,
                                        surfaceState.currentTexGen.RotationAngle
                                    );
                                }
                                if (EditorGUI.EndChangeCheck())
                                {
                                    SurfaceUtility.SetRotationAngle(
                                        surfaceState.selectedBrushSurfaces,
                                        surfaceState.currentTexGen.RotationAngle
                                    );
                                }
                            }
                            {
                                ...
                                {
                                    SurfaceUtility.AddRotationAngle(
                                        surfaceState.selectedBrushSurfaces,
                                        -90.0f
                                    );
                                }
                    ...
                    GUI.Box(tempRect, GUIContent.none, GUI.skin.box);
                    {
                        ...
                        EditModeCommonGUI.OnSurfaceFlagButtons(
                            tempRect,
                            surfaceState.surfaceFlagState,
                            surfaceState.selectedBrushSurfaces
                        );
                        {
            ...
            EditorGUI.BeginDisabledGroup(!surfaceState.enabled);
            {
                GUILayout.BeginVertical(CSG_GUIStyleUtility.ContentEmpty);
                {
                    ...
                    GUILayout.BeginVertical(GUIStyle.none);
                    {
                        ...
                        {
                            EditorGUI.showMixedValue = !surfaceState.textureLocked.HasValue;
                            surfaceState.textureLocked = EditorGUILayout.ToggleLeft(
                                ContentLockTexture,
                                surfaceState.textureLocked.HasValue
                                    ? surfaceState.textureLocked.Value
                                    : false
                    ...
                    GUILayout.BeginVertical(GUIStyle.none);
                    {
                        GUILayout.BeginHorizontal(CSG_GUIStyleUtility.ContentEmpty);
                        {
                            ...
                            GUILayout.BeginHorizontal(CSG_GUIStyleUtility.ContentEmpty);
                            {
                                ...
                                {
                                    EditorGUI.showMixedValue = surfaceState.multipleScaleX;
                                    surfaceState.currentTexGen.Scale.x = EditorGUILayout.FloatField(
                                        surfaceState.currentTexGen.Scale.x,
                                        minFloatFieldWidth
                                    );
                                }
                                ...
                                {
                                    EditorGUI.showMixedValue = surfaceState.multipleScaleY;
                                    surfaceState.currentTexGen.Scale.y = EditorGUILayout.FloatField(
                                        surfaceState.currentTexGen.Scale.y,
                                        minFloatFieldWidth
                                    );
                                }
        ...
        public static void OnSceneGUI(Rect windowRect, EditModeSurface tool)
        {
            ...
            var originalHeight =
                surfaceEditWindowHeight + (needLightmapUVUpdate ? lightMapUVButtonOffset : 0);
            ...
            int windowControlID = GUIUtility.GetControlID(
                SceneViewSurfaceOverlayHash,
                FocusType.Keyboard,
                boxArea
            );
            {
            ...
            switch (Event.current.GetTypeForControl(windowControlID))
            {
                ...
                {
                    if (boxArea.Contains(Event.current.mousePosition))
                    {
                        GUIUtility.hotControl = windowControlID;
                        GUIUtility.keyboardControl = windowControlID;
                        Event.current.Use();
                    }
                    break;
                }
                ...
                {
                    if (GUIUtility.hotControl == windowControlID)
                    {
