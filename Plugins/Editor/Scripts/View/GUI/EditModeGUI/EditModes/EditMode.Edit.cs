...
namespace RealtimeCSG
{
    internal sealed class EditModeMeshEdit : ScriptableObject, IEditMode
    {
        ...
        public bool UsesUnitySelection
        {
            get { return true; }
        }
        public bool IgnoreUnityRect
        {
            get { return true; }
        }
        ...
        private bool HavePointSelection
        {
            get
            {
                for (var t = 0; t < _brushSelection.Brushes.Length; t++)
                    if (_brushSelection.States[t].HaveSelection)
                        return true;
                return false;
            }
        }
        private bool HaveEdgeSelection
        {
            ...
            {
                for (var t = 0; t < _brushSelection.Brushes.Length; t++)
                    if (_brushSelection.States[t].HaveEdgeSelection)
                        return true;
                return false;
            }
        }
        ...
        [NonSerialized]
        private bool _doMarquee; //= false;
        #endregion


        [NonSerialized]
        private Transform _rotateBrushParent; //= null;

        [NonSerialized]
        private Vector3 _rotateStart = MathConstants.zeroVector3;

        [NonSerialized]
        private Vector3 _rotateCenter = MathConstants.zeroVector3;

        [NonSerialized]
        private Vector3 _rotateTangent = MathConstants.zeroVector3;

        [NonSerialized]
        private Vector3 _rotateNormal = MathConstants.zeroVector3;

        [NonSerialized]
        private CSGPlane _rotatePlane;

        [NonSerialized]
        private float _rotateRadius; //= 0;

        [NonSerialized]
        private float _rotateStartAngle; //= 0;

        [NonSerialized]
        private float _rotateCurrentAngle; //= 0;

        [NonSerialized]
        private float _rotateCurrentSnappedAngle; //= 0;

        [NonSerialized]
        private int _rotationUndoGroupIndex = -1;

        [NonSerialized]
        private bool _movePlaneInNormalDirection; //= false;

        [NonSerialized]
        private Vector3 _movePolygonOrigin;

        [NonSerialized]
        private Vector3 _movePolygonDirection;

        [NonSerialized]
        private Vector3 _worldDeltaMovement;

        [NonSerialized]
        private Vector3 _extraDeltaMovement = MathConstants.zeroVector3;

        [SerializeField]
        private bool _useHandleCenter; //= false;

        [SerializeField]
        private Vector3 _handleCenter;

        [SerializeField]
        private Vector3 _startHandleCenter;

        [SerializeField]
        private Vector3 _startHandleDirection;

        [SerializeField]
        private Vector3 _handleScale = Vector3.one;

        [SerializeField]
        private Vector3 _dragEdgeScale = Vector3.one;

        [SerializeField]
        private Quaternion _dragEdgeRotation;

        [SerializeField]
        private Vector3[] _handleWorldPoints; //= null;

        [NonSerialized]
        private int _hoverOnEdgeIndex = -1;

        [NonSerialized]
        private int _hoverOnPointIndex = -1;

        [NonSerialized]
        private int _hoverOnPolygonIndex = -1;

        [NonSerialized]
        private int _hoverOnTarget = -1;

        [NonSerialized]
        private int _rectSelectionId = -1;

        [NonSerialized]
        private int _chamferId = -1;

        [NonSerialized]
        private bool _nonZeroMouseIsDown; //= false;

        [NonSerialized]
        private bool _mouseIsDragging; //= false;

        [NonSerialized]
        private bool _mouseIsDown; //= false;

        [NonSerialized]
        private bool _showMarquee; //= false;
        ...
        [NonSerialized]
        private CSGPlane _movePlane;

        [NonSerialized]
        private bool _usingControl; //= false

        [SerializeField]
        private readonly TransformSelection _transformSelection = new TransformSelection();

        [SerializeField]
        private readonly BrushSelection _brushSelection = new BrushSelection();

        [SerializeField]
        private UnityEngine.Object[] _undoAbleTransforms = new UnityEngine.Object[0];

        [SerializeField]
        private UnityEngine.Object[] _undoAbleBrushes = new UnityEngine.Object[0];

        [SerializeField]
        private SpaceMatrices _activeSpaceMatrices = new SpaceMatrices();

        [NonSerialized]
        private bool _isEnabled; //= false;

        [NonSerialized]
        private bool _hideTool; //= false;
        ...
        public void SetTargets(FilteredSelection filteredSelection)
        {
            ...
            _hideTool =
                filteredSelection.NodeTargets != null && filteredSelection.NodeTargets.Length > 0;
        ...
        private void SelectMarquee(Camera camera, Rect rect, SelectionType selectionType)
        {
            ...
            try
            {
                ...
                var selectedPoints = SceneQueryUtility.GetPointsInFrustum(
                    camera,
                    frustum.Planes,
                    _brushSelection.Brushes,
                    _brushSelection.States,
                    ignoreHiddenPoints
                );

                Undo.RecordObject(this, "Select points");
                _brushSelection.RevertSelection();
                ControlMeshState.SelectFrustumPoints(
                    _brushSelection.States,
                    selectedPoints,
                    selectionType,
                    onlyOnHover: false
        ...
        public bool DeselectAll()
        {
            try
            {
                if (_brushSelection.States == null || _brushSelection.States.Length == 0)
                    return false;
        ...
        private bool UpdateSelection(bool allowSubstraction = true)
        {
            try
            {
                ...
                for (var p = 0; p < hoverMeshState.Selection.Polygons.Length; p++)
                    needRepaint =
                        hoverMeshState.SelectPolygon(p, selectionType, onlyOnHover: true)
                        || needRepaint;

                for (var e = 0; e < hoverMeshState.Selection.Edges.Length; e++)
                    needRepaint =
                        hoverMeshState.SelectEdge(e, selectionType, onlyOnHover: true)
                        || needRepaint;
                ...
                for (var b = 0; b < _brushSelection.States.Length; b++)
                {
                    ...
                    {
                        needRepaint =
                            curMeshState.SelectPoint(p, selectionType, onlyOnHover: true)
                            || needRepaint;
                    }
                }
        ...
        private void UpdateMouseCursor()
        {
            ...
            _currentCursor = CursorUtility.GetSelectionCursor(
                SelectionUtility.GetEventSelectionType()
            );
        }
        ...
        private EditMode HoverOnPoint(
            ControlMeshState meshState,
            int brushNodeIndex,
            int pointIndex
        )
        {
        ...
        private EditMode HoverOnPolygon(
            ControlMeshState meshState,
            int brushNodeIndex,
            int polygonIndex
        )
        {
            ...
            var point2 = HandleUtility.WorldToGUIPoint(
                meshState.PolygonCenterPoints[polygonIndex]
                    + (meshState.PolygonCenterPlanes[polygonIndex].normal * 10.0f)
            );
        ...
        private EditMode HoverOnEdge(ControlMeshState meshState, int brushIndex, int edgeIndex)
        {
            ...
            if (
                surfaceIndex1 < 0
                || surfaceIndex1 >= surfaces.Length
                || surfaceIndex2 < 0
                || surfaceIndex2 >= surfaces.Length
            )
                return EditMode.None;
        ...
        private void ExtrudeSurface(Camera camera, bool drag)
        {
            ...
            var worldNormal = targetMeshState
                .BrushTransform.localToWorldMatrix.MultiplyVector(localNormal)
                .normalized;
            ...
            EditModeManager.GenerateFromSurface(
                camera,
                brush,
                polygonPlane,
                worldNormal,
                ...
                drag,
                forceDragSource,
                CSGSettings.AutoCommitExtrusion
            );
        }
        ...
        private void MergeDuplicatePoints()
        {
            ...
            try
            {
                ...
                ControlMeshUtility.MergeDuplicatePoints(
                    _brushSelection.Brushes,
                    _brushSelection.States
                );
                UpdateWorkControlMesh();
        ...
        private void MergeHoverEdgePoints()
        {
            try
            {
                ...
                ControlMeshUtility.MergeHoverEdgePoints(
                    _brushSelection.Brushes[_hoverOnTarget],
                    _brushSelection.States[_hoverOnTarget],
                    _hoverOnEdgeIndex
                );
                UpdateWorkControlMesh();
            }
        ...
        private void MergeHoverPolygonPoints()
        {
            ...
            {
                Undo.RegisterCompleteObjectUndo(_undoAbleBrushes, "Merge edge-points");
                ControlMeshUtility.MergeHoverPolygonPoints(
                    _brushSelection.Brushes[_hoverOnTarget],
                    _hoverOnPolygonIndex
                );
                UpdateWorkControlMesh();
            }
            finally
        ...
        private void DoRotateControlPoints(
            Vector3 center,
        ...
        private void DeleteSelectedPoints()
        {
            try
            {
                ...
                ControlMeshUtility.DeleteSelectedPoints(
                    _brushSelection.Brushes,
                    _brushSelection.States
                );
                UpdateWorkControlMesh();
        ...
        private void MergeSelectedEdgePoints()
        {
            ...
            {
                Undo.RegisterCompleteObjectUndo(_undoAbleBrushes, "Merge edge-points");
                ControlMeshUtility.MergeSelectedEdgePoints(
                    _brushSelection.Brushes,
                    _brushSelection.States
                );
                UpdateWorkControlMesh();
        ...
        private bool UpdateRotationCircle(SceneView sceneView)
        {
            switch (_editMode)
            {
                case EditMode.RotateEdge:
                {
                    ...
                    _rotateNormal = camera.orthographic
                    ...
                    _rotateStartAngle = GeometryUtility.SignedAngle(
                        _rotateCenter - _rotateStart,
                        _rotateCenter - newMousePos,
                        _rotateNormal
                    );
        ...
        {
            if (_hoverOnTarget <= -1 || _hoverOnTarget >= _brushSelection.States.Length || !camera)
            {
                return;
            }
            ...
            )
            {
                ...
                var worldDirection = brushLocalToWorld
                    .MultiplyVector(
                        targetMeshState.PolygonCenterPlanes[_hoverOnPolygonIndex].normal
                    )
                    .normalized;
                if (Tools.pivotRotation == PivotRotation.Global)
                    worldDirection = GeometryUtility.SnapToClosestAxis(worldDirection);
                RealtimeCSG.CSGGrid.SetupRayWorkPlane(
                    camera,
                    worldOrigin,
                    worldDirection,
                    ref _movePlane
            ...
            )
            {
                ...
                if (Tools.pivotRotation == PivotRotation.Global)
                    worldDirection = GeometryUtility.SnapToClosestAxis(worldDirection);
                RealtimeCSG.CSGGrid.SetupWorkPlane(
                    camera,
                    worldOrigin,
                    worldDirection,
                    ref _movePlane
        ...
        {
            if (Math.Abs(_movePlane.a) > 1 - MathConstants.NormalEpsilon)
                offset.x = 0.0f;
            if (Math.Abs(_movePlane.b) > 1 - MathConstants.NormalEpsilon)
                offset.y = 0.0f;
            if (Math.Abs(_movePlane.c) > 1 - MathConstants.NormalEpsilon)
                offset.z = 0.0f;
        ...
        private readonly Dictionary<Camera, BrushOutlineInfo> _brushOutlineInfos =
            new Dictionary<Camera, BrushOutlineInfo>();
        ...
        private void UpdateMeshStates()
        {
            for (var t = 0; t < _brushSelection.Brushes.Length; t++)
            {
                ...
                _brushSelection
                    .States[t]
                    .UpdateMesh(
                        _brushSelection.BackupControlMeshes[t],
        ...
        private void UpdatePointSizes(SceneView sceneView)
        {
            ...
            for (var t = 0; t < _brushSelection.Brushes.Length; t++)
                _brushSelection
                    .States[t]
                    .UpdateHandles(camera, _brushSelection.BackupControlMeshes[t]);
        }

        [NonSerialized]
        private ViewTool _preViewTool = ViewTool.None;
        ...
        private void UpdateLineMeshes(SceneView sceneView)
        {
            ...
            {
                if (_preViewTool != Tools.viewTool || _prevTool != Tools.current)
                {
            ...
            brushOutlineRenderer.Update(
                camera,
                _brushSelection.Brushes,
                _brushSelection.ControlMeshes,
                _brushSelection.States
            );
        }
        ...
        )
        {
            ...
            var textCenter =
                textCenterRay.origin
                + textCenterRay.direction
                    * ((sceneView.camera.farClipPlane + sceneView.camera.nearClipPlane) * 0.5f);
        ...
        {
            return ShouldDrawBrushPoints() || ShouldDrawPolygonPoints(camera);
        }
...

            {
                ...
                if (ShouldDrawAnyPoints(camera))
                {
                    ...
                    if (ShouldDrawBrushPoints() && CSGSettings.SelectionVertex)
                    {
                        for (var t = 0; t < _brushSelection.Brushes.Length; t++)
                        {
                            ...
                            var CameraState = _brushSelection
                                .States[t]
                                .GetCameraState(camera, false);
                            if (CameraState == null)
                                continue;

                            pointMeshManager.DrawPoints(
                                camera,
                                meshState.WorldPoints,
                    ...
                    pointMeshManager.Render(
                        PaintUtility.CustomDotMaterial,
                        PaintUtility.CustomSurfaceNoDepthMaterial
                    );
                }
                    ...
                    //if (rotateBrushParent != null)
                    {
                        if (_mouseIsDragging)
                        {
                            PaintUtility.DrawRotateCircle(
                                ...
                                0,
                                _rotateStartAngle,
                                _rotateCurrentSnappedAngle,
                                ColorSettings.RotateCircleOutline
                            );
                            PaintUtility.DrawRotateCirclePie(
                                _rotateCenter,
                                _rotateNormal,
                                _rotateTangent,
                                _rotateRadius,
                ...
                )
                {
                    ...
                    if (t > -1 && t < _brushSelection.States.Length)
                    {
                        ...
                        if (modelTransform != null)
                        {
                            if (_hoverOnTarget == t && p == _hoverOnPolygonIndex)
                                Handles.color = ColorSettings.PolygonInnerStateColor[
                                    (int)(SelectState.Selected | SelectState.Hovering)
                                ];
                            ...
                            if (p < targetMeshState.PolygonCenterPoints.Length)
                            {
                                ...
                                PaintUtility.DrawArrowCap(
                                    origin,
                                    worldNormal,
                                    HandleUtility.GetHandleSize(origin)
                                );
                                Handles.matrix = MathConstants.identityMatrix;
                            }
                        }
                    ...
                    if (modelTransform != null)
                    {
                        if (_hoverOnPolygonIndex != -1)
                        {
                            ...
                            var textCenter =
                                textCenterRay.origin
                                + textCenterRay.direction
                                    * ((camera.farClipPlane + camera.nearClipPlane) * 0.5f);

                            Handles.color = Color.black;

                            Handles.DrawLine(origin, textCenter);
                            if (Tools.pivotRotation == PivotRotation.Local)
                                PaintUtility.DrawScreenText(
                                    textCenter2D,
                                    "Normal Aligned Extrude [local]"
                                );
                            else
                                PaintUtility.DrawScreenText(
                                    textCenter2D,
                                    "Axis Aligned Extrude [global]"
                                );
                        }
                        else if (
                            _hoverOnEdgeIndex != -1
                        ...
                        )
                        {
                            ...
                            DrawEdgeText(
                                sceneView,
                                meshState.BrushCenter,
                                vertexA,
                                vertexB,
                                "Chamfer"
                            );
                        }
                        else if (_hoverOnPointIndex != -1)
                        {
                    ...
                    )
                    {
                        ...
                        if (((_hoverOnEdgeIndex * 2) + 1) < meshState.Edges.Length)
                        {
                            ...
                            DrawEdgeText(
                                ...
                                vertexA,
                                vertexB,
                                Units.ToRoundedDistanceString(length)
                            );
        ...
        private void RenderOffsetText()
        {
            ...
            if (Tools.pivotRotation == PivotRotation.Local)
            {
                ...
                delta = GridUtility.CleanPosition(
                    _activeSpaceMatrices.activeLocalToWorld.MultiplyVector(delta).normalized
                );
            }
            ...
            var lockX =
                (Mathf.Abs(delta.x) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisX
                    || (Mathf.Abs(_movePlane.a) >= 1 - MathConstants.EqualityEpsilon)
                );
            var lockY =
                (Mathf.Abs(delta.y) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisY
                    || (Mathf.Abs(_movePlane.b) >= 1 - MathConstants.EqualityEpsilon)
                );
            var lockZ =
                (Mathf.Abs(delta.z) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisZ
                    || (Mathf.Abs(_movePlane.c) >= 1 - MathConstants.EqualityEpsilon)
                );
        ...
        {
            _meshEditBrushToolId = GUIUtility.GetControlID(
            ...
            for (var t = 0; t < _brushSelection.States.Length; t++)
            {
                ...
                for (var p = 0; p < meshState.WorldPoints.Length; p++)
                    meshState.PointControlId[p] = GUIUtility.GetControlID(
                        MeshEditBrushPointHash,
                        FocusType.Keyboard
                    );

                for (var e = 0; e < meshState.Edges.Length / 2; e++)
                    meshState.EdgeControlId[e] = GUIUtility.GetControlID(
                        MeshEditBrushEdgeHash,
                        FocusType.Keyboard
                    );

                for (var p = 0; p < meshState.PolygonCenterPoints.Length; p++)
                    meshState.PolygonControlId[p] = GUIUtility.GetControlID(
                        MeshEditBrushPolygonHash,
                        FocusType.Keyboard
                    );
            }
        }
        ...
        private Quaternion GetRealHandleRotation()
        {
            ...
            if (
                rotation.x <= MathConstants.EqualityEpsilon
        ...
        {
            var lockX =
                ((Mathf.Abs(scale.x) - 1) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisX
                    || (Mathf.Abs(_movePlane.a) >= 1 - MathConstants.EqualityEpsilon)
                );
            var lockY =
                ((Mathf.Abs(scale.y) - 1) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisY
                    || (Mathf.Abs(_movePlane.b) >= 1 - MathConstants.EqualityEpsilon)
                );
            var lockZ =
                ((Mathf.Abs(scale.z) - 1) < MathConstants.ConsideredZero)
                && (
                    CSGSettings.LockAxisZ
                    || (Mathf.Abs(_movePlane.c) >= 1 - MathConstants.EqualityEpsilon)
                );
            ...
            PaintUtility.RenderBoundsSizes(
            ...
            for (int p0 = 0; p0 < chamferPlanes.Length; p0++)
            {
                ...
                for (int p1 = 0; p1 < chamferPlanes.Length; p1++)
                {
                    if (p0 == p1)
                        continue;
                    if (
                        chamferPlanes[p1].Distance(tryIntersection) >= MathConstants.DistanceEpsilon
                    )
                    {
                        tryFound = false;
                        break;
            ...
            switch (chamferMode)
            {
                ...
                case ChamferMode.Point:
                {
                    ...
                    switch (activeSnapMode)
                    {
                        ...
                        {
                            chamferIntersectionPoint =
                                intersection
                                + RealtimeCSG.CSGGrid.SnapDeltaToGrid(
                                    camera,
                                    intersection - projectedPoint,
                                    new Vector3[] { intersection }
                                ); //, snapToSelf: true);
                            break;
                        }
                        ...
                        case SnapMode.None:
                        {
                            chamferIntersectionPoint =
                                intersection
                                + RealtimeCSG.CSGGrid.HandleLockedAxi(
                                    intersection - projectedPoint
                                );
                            break;
                        }
                ...
                {
                    projectedPoint = GeometryUtility.ProjectPointOnInfiniteLine(
                        intersection,
                        chamferEdgePoints[0],
                        chamferDirection
                    );
                ...
                if (Math.Abs(distance) > MathConstants.EqualityEpsilon)
                {
                    //if (chamferMode == ChamferMode.Edge)
                    {
                        for (int e = 0; e < meshState.Selection.Edges.Length; e++)
                        {
                            ...
                            if (
                                (
                                    meshState.Selection.Edges[e]
                                    & (SelectState.Selected | SelectState.Hovering)
                                ) == SelectState.None
                                && (
                                    meshState.Selection.Polygons[polygonIndex1]
                                    & SelectState.Selected
                                ) != SelectState.Selected
                                && (
                                    meshState.Selection.Polygons[polygonIndex2]
                                    & SelectState.Selected
                                ) != SelectState.Selected
                            )
                            {
                            ...
                            var chamferTangents0 = Vector3.Cross(
                                chamferPlanes0.normal,
                                chamferEdge
                            );
                            var chamferTangents1 = Vector3.Cross(
                                chamferPlanes1.normal,
                                chamferEdge
                            );
                            ...
                            var chamferPlane = new CSGPlane(
                                chamferPoints0,
                                chamferPoints1,
                                chamferPoints2
                            );

                            var localCuttingPlane = GeometryUtility.InverseTransformPlane(
                    ...
                    //if (chamferMode == ChamferMode.Point)
                    {
                        ...
                        {
                            if (
                                (meshState.Selection.Polygons[p] & SelectState.Selected)
                                != SelectState.Selected
                            )
                                continue;
                        ...
                        for (int p = 0; p < meshState.Selection.Points.Length; p++)
                        {
                            ...
                            var currPoint = localToWorldMatrix.MultiplyPoint(
                                controlMesh.Vertices[currPointIndex]
                            );
                            ...
                            for (int e = 0; e < controlMesh.Edges.Length; e++)
                            {
                                ...
                                var twinPoint = localToWorldMatrix.MultiplyPoint(
                                    controlMesh.Vertices[twinPointIndex]
                                );
                            ...
                            var chamferPlane = GeometryUtility.CalcPolygonPlane(
                                found_points.ToArray()
                            );
                    ...
                    foreach (var plane in planes)
                    {
                        ...
                        {
                            var edge_loop = ControlMeshUtility.FindEdgeLoop(
                                controlMesh,
                                ref selected_edges
                            );
                            if (edge_loop != null)
                            {
                                if (ControlMeshUtility.SplitEdgeLoop(controlMesh, shape, edge_loop))
                                {
                                    ...
                                    if (
                                        ControlMeshUtility.FindAndDetachSeparatePiece(
                                            controlMesh,
                                            shape,
                                            plane,
                                            out foundControlMesh,
                                            out foundShape
                                        )
                                    )
                                    {
        ...
        void HandleChamfering(SceneView sceneView)
        {
            ...
            switch (type)
            {
                ...
                case EventType.Repaint:
                {
                    ...
                    PaintUtility.SquareDotCap(
                        -1,
                        chamferIntersectionPoint,
                        MathConstants.identityQuaternion,
                        CSG_HandleUtility.GetHandleSize(chamferIntersectionPoint)
                            * GUIConstants.handleScale
                    );
                    break;
                }
            }
        ...
        public void HandleEvents(SceneView sceneView, Rect sceneRect)
        {
            ...
            switch (originalEventType)
            {
                ...
                case EventType.MouseDrag:
                {
                    ...
                    if (
                        !_mouseIsDragging
                        && (_prevMousePos - Event.current.mousePosition).magnitude > 4.0f
                    )
            ...
            if (
                SelectionUtility.CurrentModifiers == EventModifiers.None
                && currentHotControl != _chamferId
                && _useHandleCenter
            )
            {
                ...
                switch (Tools.current)
                {
                    ...
                    case Tool.Rotate:
                    {
                        RealtimeCSG.Helpers.CSGHandles.InitFunction shutdown = delegate
                        {
                            ...
                            if (UpdateWorkControlMesh(forceUpdate: true))
                                ...
                                Undo.PerformUndo();
                            InternalCSGModelManager.CheckSurfaceModifications(
                                _brushSelection.Brushes,
                                true
                            );
                            _usingControl = false;
                        ...
                        var newRotation = PaintUtility.HandleRotation(
                            camera,
                            _handleCenter,
                            handleRotation,
                            init,
                        ...
                        {
                            ForceLineUpdate();
                            DoRotateControlPoints(
                                _handleCenter,
                                handleRotation,
                                Quaternion.Inverse(handleRotation) * newRotation
                            );
                            ControlMeshUtility.MergeDuplicatePoints(
                                _brushSelection.Brushes,
                                _brushSelection.States
                    ...
                    case Tool.Scale:
                    {
                        ...
                        var newHandleScale = PaintUtility.HandleScale(
                            _handleScale,
                            _handleCenter,
                            rotation,
                            init,
                        ...
                        if (GUI.changed)
                        {
                            ...
                            if (
                                float.IsInfinity(newScale.x)
                                || float.IsNaN(newScale.x)
                                || float.IsInfinity(newScale.y)
                                || float.IsNaN(newScale.y)
                                || float.IsInfinity(newScale.z)
                                || float.IsNaN(newScale.z)
                            )
                                newScale = Vector3.zero;

                            if (newScale.x <= MathConstants.EqualityEpsilon)
                            {
                                newScale.x = 0.0f;
                            }
                            if (newScale.y <= MathConstants.EqualityEpsilon)
                            {
                                newScale.y = 0.0f;
                            }
                            if (newScale.z <= MathConstants.EqualityEpsilon)
                            {
                                newScale.z = 0.0f;
                            }
                    ...
                    case Tool.Move:
                    {
                        ...
                        var newHandleCenter = PaintUtility.HandlePosition(
                            camera,
                            _handleCenter,
                            rotation,
                            _handleWorldPoints,
                        ...
                        {
                            var lockX =
                                (Mathf.Abs(_worldDeltaMovement.x) < MathConstants.ConsideredZero)
                                && (
                                    CSGSettings.LockAxisX
                                    || (
                                        Mathf.Abs(_movePlane.a) >= 1 - MathConstants.EqualityEpsilon
                            ...
                            var lockY =
                                (Mathf.Abs(_worldDeltaMovement.y) < MathConstants.ConsideredZero)
                                && (
                                    CSGSettings.LockAxisY
                                    || (
                                        Mathf.Abs(_movePlane.b) >= 1 - MathConstants.EqualityEpsilon
                            ...
                            var lockZ =
                                (Mathf.Abs(_worldDeltaMovement.z) < MathConstants.ConsideredZero)
                                && (
                                    CSGSettings.LockAxisZ
                                    || (
                                        Mathf.Abs(_movePlane.c) >= 1 - MathConstants.EqualityEpsilon
                            ...
                            var text = Units.ToRoundedDistanceString(
                                _worldDeltaMovement,
                                lockX,
                                lockY,
                                lockZ
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                {
                    PaintUtility.DrawSelectionRectangle(
                        CameraUtility.PointsToRect(_startMousePoint, Event.current.mousePosition)
                    );
                }
            }
            ...
            try
            {
                switch (Event.current.type)
                {
                    case EventType.MouseDown:
                    {
                        ...
                        if (
                            ...
                            || (SelectionUtility.CurrentModifiers & EventModifiers.Alt)
                                == EventModifiers.Alt
                        )
                            break;
                        ...
                        if (_hoverOnTarget > -1 && _hoverOnTarget < _brushSelection.States.Length)
                        {
                            ...
                            switch (_editMode)
                            {
                                case EditMode.RotateEdge:
                                {
                                    newControlId = _brushSelection
                                        .States[_hoverOnTarget]
                                        .EdgeControlId[_hoverOnEdgeIndex];
                                ...
                                case EditMode.MovingPoint:
                                {
                                    newControlId = _brushSelection
                                        .States[_hoverOnTarget]
                                        .PointControlId[_hoverOnPointIndex];
                                    break;
                                }
                                case EditMode.MovingPolygon:
                                {
                                    newControlId = _brushSelection
                                        .States[_hoverOnTarget]
                                        .PolygonControlId[_hoverOnPolygonIndex];
                                    if (Tools.current == Tool.Scale)
                                    {
                    ...
                    case EventType.MouseUp:
                    {
                        ...
                        if (!UpdateSelection())
                        {
                            ...
                            else if (SelectionUtility.DoSelectionClick(sceneView))
                            {
                                Event.current.Use();
                            }
                        }
                    ...
                    case EventType.KeyDown:
                    {
                        if (GUIUtility.hotControl != 0)
                            break;
                        if (HavePointSelection && Keys.DeleteSelectionKey.IsKeyPressed())
                        {
                            Event.current.Use();
                            break;
                        }
                        ...
                        if (Keys.SnapToGridKey.IsKeyPressed())
                        {
                            Event.current.Use();
                            break;
                        }
                        if (Keys.MergeEdgePoints.IsKeyPressed() && HaveEdgeSelection)
                        {
                            Event.current.Use();
                            break;
                        }
                    ...
                    case EventType.Layout:
                    {
                        ...
                        try
                        {
                            ...
                            var inCamera =
                                camera && camera.pixelRect.Contains(Event.current.mousePosition);
                            ...
                            for (int t = 0; t < _brushSelection.Brushes.Length; t++)
                            {
                                ...
                                for (int j = 0, e = 0; j < meshState.Edges.Length; e++, j += 2)
                                {
                                    ...
                                    if (hotControl == newControlId)
                                        hoverControl = newControlId;
                                    var distance = meshState.GetClosestEdgeDistance(
                                        cameraPlane,
                                        meshState.Edges[j + 0],
                                        meshState.Edges[j + 1]
                                    );
                                ...
                                for (var p = 0; p < meshState.PolygonCenterPoints.Length; p++)
                                {
                                    ...
                                    if (
                                        camera.orthographic
                                        || p >= cameraState.PolygonCenterPointSizes.Length
                                    ...
                                    if (
                                        _useHandleCenter
                                        && (_handleCenter - center).sqrMagnitude
                                            <= MathConstants.EqualityEpsilonSqr
                                    )
                                        continue;

                                    var radius = cameraState.PolygonCenterPointSizes[p] * 1.2f;
                                    var centerDistance = CameraUtility.DistanceToCircle(
                                        center,
                                        radius
                                    );
                            ...
                            try
                            {
                                ...
                                _brushSelection.FindClosestIntersection(
                                    sceneView,
                                    out closestBrushIndex,
                                    out closestSurfaceIndex
                                );
                                if (closestBrushIndex != -1)
                                {
                                    var meshState = _brushSelection.States[closestBrushIndex];
                                    var newControlId = meshState.PolygonControlId[
                                        closestSurfaceIndex
                                    ];
                            ...
                            if (nearestControl == _meshEditBrushToolId)
                                nearestControl = 0; // liar
                                    ...
                                    if (newEditMode == EditMode.None)
                                    {
                                        ...
                                        )
                                        {
                                            for (var p = 0; p < meshState.WorldPoints.Length; p++)
                                            {
                                                ...
                                                for (
                                                    var t2 = 0;
                                                    t2 < _brushSelection.Brushes.Length;
                                                    t2++
                                                )
                                                {
                                                    if (t2 == t)
                                                    ...
                                                    for (
                                                        var p2 = 0;
                                                        p2 < meshState2.WorldPoints.Length;
                                                        p2++
                                                    )
                                                    {
                                                        var worldPoint2 = meshState2.WorldPoints[
                                                            p2
                                                        ];
                                                        if (
                                                            (worldPoint - worldPoint2).sqrMagnitude
                                                            < MathConstants.EqualityEpsilonSqr
                                                        )
                                                        {
                                                            meshState2.Selection.Points[p2] |=
                                        ...
                                        if (
                                            newEditMode == EditMode.None
                                            && CSGSettings.SelectionSurface
                                        )
                                        {
                                            for (
                                                var p = 0;
                                                p < meshState.PolygonCenterPoints.Length;
                                                p++
                                            )
                                            {
                ...
                if (currentHotControl == _rectSelectionId)
                {
                    ...
                    switch (type)
                    {
                        case EventType.MouseDrag:
                        {
                            ...
                            {
                                if (
                                    (_startMousePoint - Event.current.mousePosition).magnitude
                                    > (
                                        MathConstants.MinimumMouseMovement
                                        * MathConstants.MinimumMouseMovement
                                    )
                                )
                                {
                ...
                else if (currentHotControl == _chamferId)
                {
                    HandleChamfering(sceneView);
                }
                else if (_brushSelection.States != null)
                {
                    for (var t = 0; t < _brushSelection.States.Length; t++)
                    {
                        var meshState = _brushSelection.States[t];
                        ...
                        for (int p = 0; p < meshState.WorldPoints.Length; p++)
                        {
                            ...
                            {
                                var type = Event.current.GetTypeForControl(
                                    meshState.PointControlId[p]
                                );
                                switch (type)
                                {
                                    case EventType.KeyUp:
                                    {
                                        ...
                                        if (Keys.MergeEdgePoints.IsKeyPressed())
                                        {
                                            MergeSelected();
                                            Event.current.Use();
                                            break;
                                    ...
                                    case EventType.KeyDown:
                                    {
                                        ...
                                        if (
                                            Keys.MergeEdgePoints.IsKeyPressed() && HaveEdgeSelection
                                        )
                                        {
                                            Event.current.Use();
                                            break;
                                    ...
                                    case EventType.MouseDrag:
                                    {
                                        ...
                                        var lockX = (
                                            CSGSettings.LockAxisX
                                            || (
                                                Mathf.Abs(_movePlane.a)
                                                >= 1 - MathConstants.EqualityEpsilon
                                            )
                                        );
                                        var lockY = (
                                            CSGSettings.LockAxisY
                                            || (
                                                Mathf.Abs(_movePlane.b)
                                                >= 1 - MathConstants.EqualityEpsilon
                                            )
                                        );
                                        var lockZ = (
                                        ...
                                        {
                                            EditModeManager.ShowMessage(
                                                "Positional snapping is set to zero, cannot move."
                                            );
                                            break;
                                        }
                                        else if (lockX && lockY && lockZ)
                                        {
                                            EditModeManager.ShowMessage(
                                                "All axi are disabled (X Y Z), cannot move."
                                            );
                                            break;
                                        }
                                        EditModeManager.ResetMessage();

                                        var mouseRay = HandleUtility.GUIPointToWorldRay(
                                            Event.current.mousePosition
                                        );
                                        ...
                                        if (
                                            ...
                                            || float.IsNaN(intersection.z)
                                        )
                                            break;

                                        intersection = GeometryUtility.ProjectPointOnPlane(
                                            _movePlane,
                                            intersection
                                        );
                                        ...
                                        {
                                            _worldDeltaMovement = SnapMovementToPlane(
                                                intersection - _originalPoint
                                            );
                                        }

                                        // try to snap selected points against non-selected points
                                        var activeSnappingMode = RealtimeCSG
                                            .CSGSettings
                                            .ActiveSnappingMode;
                                        switch (activeSnappingMode)
                                        {
                                            ...
                                            {
                                                var worldPoints =
                                                    _brushSelection.GetSelectedWorldPoints(
                                                        useBackupPoints: true
                                                    );
                                ...
                                switch (type)
                                {
                                    ...
                                    case EventType.MouseDrag:
                                    {
                                        ...
                                        if (_editMode == EditMode.RotateEdge)
                                        {
                                            ...
                                            {
                                                EditModeManager.ShowMessage(
                                                    "Rotational snapping is set to zero, cannot rotate."
                                                );
                                                break;
                                            }
                                            EditModeManager.ResetMessage();
                                            ...
                                            _rotateCurrentAngle = GeometryUtility.SignedAngle(
                                                _rotateCenter - _rotateStart,
                                                _rotateCenter - newMousePos,
                                                _rotateNormal
                                            );
                                            _rotateCurrentSnappedAngle =
                                                GridUtility.SnappedAngle(
                                                    _rotateCurrentAngle - _rotateStartAngle
                                                ) + _rotateStartAngle;
                                        ...
                                        )
                                        {
                                            ...
                                            //if (_firstMove)
                                            {
                                                var originalVertexIndex1 = meshState.Edges[
                                                    (e * 2) + 0
                                                ];
                                                var originalVertexIndex2 = meshState.Edges[
                                                    (e * 2) + 1
                                                ];
                                                var originalVertex1 = meshState.WorldPoints[
                                                    originalVertexIndex1
                                                ];
                                                var originalVertex2 = meshState.WorldPoints[
                                                    originalVertexIndex2
                                                ];
                                                ...
                                                float squaredDist,
                                                    s;
                                                Vector3 closestRay;
                                                _originalPoint = MathUtils.ClosestPtSegmentRay(
                                                    originalVertex1,
                                                    originalVertex2,
                                                    ray,
                                                    out squaredDist,
                                        ...
                                        if (_firstMove)
                                        {
                                            ...
                                            _startHandleDirection =
                                                GeometryUtility.SnapToClosestAxis(
                                                    inverseRotation * _startHandleDirection
                                                );
                                            _startHandleDirection =
                                                rotation * _startHandleDirection;
                                        ...
                                        if (Tools.current == Tool.Scale)
                                        {
                                            ...
                                            var start = GeometryUtility.ProjectPointOnInfiniteLine(
                                                _originalPoint,
                                                _startHandleCenter,
                                                _startHandleDirection
                                            );
                                            var end = GeometryUtility.ProjectPointOnInfiniteLine(
                                                intersection,
                                                _startHandleCenter,
                                                _startHandleDirection
                                            );
                                            ...
                                            var oldDistance =
                                                inverseRotation * (start - _startHandleCenter);
                                            var newDistance =
                                                inverseRotation * (end - _startHandleCenter);
                                            if (
                                                Mathf.Abs(oldDistance.x)
                                                > MathConstants.DistanceEpsilon
                                            )
                                                _dragEdgeScale.x = newDistance.x / oldDistance.x;
                                            if (
                                                Mathf.Abs(oldDistance.y)
                                                > MathConstants.DistanceEpsilon
                                            )
                                                _dragEdgeScale.y = newDistance.y / oldDistance.y;
                                            if (
                                                Mathf.Abs(oldDistance.z)
                                                > MathConstants.DistanceEpsilon
                                            )
                                                _dragEdgeScale.z = newDistance.z / oldDistance.z;

                                            if (
                                                float.IsNaN(_dragEdgeScale.x)
                                                || float.IsInfinity(_dragEdgeScale.x)
                                            )
                                                _dragEdgeScale.x = 1.0f;
                                            if (
                                                float.IsNaN(_dragEdgeScale.y)
                                                || float.IsInfinity(_dragEdgeScale.y)
                                            )
                                                _dragEdgeScale.y = 1.0f;
                                            if (
                                                float.IsNaN(_dragEdgeScale.z)
                                                || float.IsInfinity(_dragEdgeScale.z)
                                            )
                                                _dragEdgeScale.z = 1.0f;

                                            _dragEdgeScale.x =
                                                Mathf.Round(
                                                    _dragEdgeScale.x / CSGSettings.SnapScale
                                                ) * CSGSettings.SnapScale;
                                            _dragEdgeScale.y =
                                                Mathf.Round(
                                                    _dragEdgeScale.y / CSGSettings.SnapScale
                                                ) * CSGSettings.SnapScale;
                                            _dragEdgeScale.z =
                                                Mathf.Round(
                                                    _dragEdgeScale.z / CSGSettings.SnapScale
                                                ) * CSGSettings.SnapScale;

                                            DoScaleControlPoints(
                                                rotation,
                                                _dragEdgeScale,
                                                _startHandleCenter
                                            );
                                ...
                                switch (type)
                                {
                                    ...
                                    case EventType.MouseDrag:
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            intersection =
                                                GeometryUtility.ProjectPointOnInfiniteLine(
                                                    intersection,
                                                    _movePolygonOrigin,
                                                    _movePolygonDirection
                                                );
                                        }
                                        else
                                        {
                                        ...
                                        switch (activeSnappingMode)
                                        {
                                            case SnapMode.GridSnapping:
                                            {
                                                ...
                                                )
                                                {
                                                    ...
                                                    _worldDeltaMovement =
                                                        RealtimeCSG.CSGGrid.SnapDeltaToRayGrid(
                                                            camera,
                                                            new Ray(worldLineOrg, worldLineDir),
                                                            _worldDeltaMovement,
                                                            worldPoints
                                                        );
                                                }
                                            ...
                                            case SnapMode.RelativeSnapping:
                                            {
                                                ...
                                                )
                                                {
                                                    ...
                                                    var worldLineDir = _movePolygonDirection;
                                                    _worldDeltaMovement =
                                                        RealtimeCSG.CSGGrid.SnapDeltaToRayRelative(
                                                            camera,
                                                            new Ray(worldLineOrg, worldLineDir),
                                                            _worldDeltaMovement
                                                        );
