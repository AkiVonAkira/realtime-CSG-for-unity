...
namespace RealtimeCSG
{
    ...
    {
        static readonly int surfaceSelectPaintControlToolHash =
            "surfaceSelectPaintControl".GetHashCode();
        ...
        [NonSerialized]
        bool forceOutlineUpdate = false;

        [NonSerialized]
        int surfaceSelectPaintControl = -1;

        [NonSerialized]
        int textureCopyDragControlID = -1;

        [NonSerialized]
        int hoverOnSurfaceIndex = -1;

        [NonSerialized]
        int hoverOnBrushNodeID = CSGNode.InvalidNodeID;
        ...
        [NonSerialized]
        int hoverOnlyTarget = -1;

        [NonSerialized]
        int undoGroupIndex = -1;

        [NonSerialized]
        RotationCircle rotationCircle = new RotationCircle();

        [NonSerialized]
        Vector2 deltaMovement;

        [NonSerialized]
        Vector3 originalSurfacePoint;

        [NonSerialized]
        Vector3 snappingOffset;

        [NonSerialized]
        Vector3 snappingOffset2;

        [NonSerialized]
        SelectedBrushSurface lastSelectedSurface = null;

        [SerializeField]
        CSGBrush[] brushTargets = new CSGBrush[0];

        [SerializeField]
        TexGenState[] surfaceStates = new TexGenState[0];

        [SerializeField]
        SelectedBrushSurface[] selectedSurfaces = new SelectedBrushSurface[0];
        ...
        public void SelectSurfaces(
            SelectedBrushSurface[] brushSurfaces,
            HashSet<GameObject> gameObjects,
            bool selectAllSurfacesOnBrushes
        )
        {
            if (brushTargets != null && brushTargets.Length > 0)
            {
                for (int i = surfaceStates.Length - 1; i >= 0; i--)
                {
                    if (gameObjects == null || !gameObjects.Contains(brushTargets[i].gameObject))
                    {
                        ArrayUtility.RemoveAt(ref surfaceStates, i);
                        ArrayUtility.RemoveAt(ref brushTargets, i);
                        if (hoverOnTarget > i)
            ...
            if (brushSurfaces != null && brushSurfaces.Length > 0)
            {
                ...
                for (int i = surfaceStates.Length - 1; i >= 0; i--)
                {
                    ...
                    for (int j = 0; j < brushSurfaces.Length; j++)
                    {
                        ...
                        else
                        {
                            ...
                            if (surfaceIndex >= 0)
                            {
                                surfaceState.surfaceSelectState[surfaceIndex] =
        ...
        {
            if (surfaceStates == null || surfaceStates.Length == 0)
            {
            ...
            for (int b = 0; b < surfaceStates.Length; b++)
            {
                ...
                {
                    if (
                        (surfaceStates[b].surfaceSelectState[s] & SelectState.Selected)
                        == SelectState.Selected
                    )
                    {
        ...
        {
            if (intersection.gameObject == null || intersection.surfaceIndex == -1)
                return false;
            ...
            if (
                (
                    surfaceStates[brushIndex].surfaceSelectState[intersection.surfaceIndex]
                    & SelectState.Selected
                ) == SelectState.Selected
            )
                return true;
        ...
        bool UpdateSelection(bool allowSubstraction = true)
        {
            ...
            if (selectionType == SelectionType.Replace || selectionType == SelectionType.Additive)
            {
                //	if (surfaceState.IsSurfaceSelected(hoverOnSurfaceIndex))
                //		return false;
            }
        ...
        void CopyBackupTexgens()
        {
            for (int t = 0; t < brushTargets.Length; t++)
            {
                ...
                Array.Copy(
                    targetShape.TexGens,
                    surfaceStates[t].backupTexGens,
                    targetShape.TexGens.Length
                );
            }
        }
        ...
        void RestoreBackupTexgens()
        {
            for (int t = 0; t < this.brushTargets.Length; t++)
            {
                ...
                for (int s = 0; s < targetShape.Surfaces.Length; s++)
                {
                    ...
                    {
                        Debug.LogWarning(
                            "this.surfaceStates[t].backupTexGens.Length <= texGenIndex"
                        );
                        continue;
                    }
                    ...
                    targetShape.TexGens[texGenIndex] = this.surfaceStates[t].backupTexGens[
                        texGenIndex
                    ];
                }
            }
        ...
        bool UpdateGrid(Camera camera)
        {
            ...
            var zero_point = SurfaceUtility.ConvertTextureToModelSpace(
                brush,
                hoverOnSurfaceIndex,
                MathConstants.zeroVector2
            );
            ...
            movePlane = GeometryUtility.InverseTransformPlane(
                brush_transform.worldToLocalMatrix,
                movePlane
            );
        ...
        bool DragTextureCopy(SceneView sceneView)
        {
            ...
            if (
                SceneQueryUtility.FindWorldIntersection(
                    camera,
            ...
            )
            {
                if (
                    lastSelectedSurface != null
                    && (
                        lastSelectedSurface.brush != intersection.brush
                        || lastSelectedSurface.surfaceIndex != intersection.surfaceIndex
                    )
                )
                {
                    if (lastSelectedSurface.surfacePlane.HasValue)
                    {
                        SurfaceUtility.CopyLastMaterial(
                            intersection.brush,
                            intersection.surfaceIndex,
                            intersection.worldPlane,
                            lastSelectedSurface.brush,
                            lastSelectedSurface.surfaceIndex,
                            lastSelectedSurface.surfacePlane.Value
                        );
                ...
                if (lastSelectedSurface == null)
                    lastSelectedSurface = new SelectedBrushSurface(
                        intersection.brush,
                        intersection.surfaceIndex,
                        intersection.worldPlane
                    );
        ...
        bool DragTranslateTextureCoordinates(SceneView sceneView)
        {
            ...
            var brush = brushTargets[hoverOnTarget];
            if (brush.ChildData == null || brush.ChildData.ModelTransform == null)
            {
                return false;
            }
            ...
            var camera = sceneView.camera;
            if (
                !SceneQueryUtility.FindSurfaceIntersection(
                    camera,
                    brush,
                    modelTransformation,
                    hoverOnSurfaceIndex,
            ...
            new_world_position = GridUtility.FixPosition(
                new_world_position,
                modelTransform,
                old_world_position
            );
            ...
            using (new UndoGroup(selectedSurfaces, "Translating surface"))
            {
                ...
                if (
                    SurfaceUtility.TranslateSurfaces(
                        selectedSurfaces,
                        modelTransform,
                        old_world_position,
                        new_world_position
                ...
                {
                    deltaMovement =
                        hoverOnShape.TexGens[hoverOnTexGenIndex].Translation
                        - backupTexGen.Translation;
        ...
        [NonSerialized]
        LineMeshManager selectedVisibleLineMeshManager = new LineMeshManager();

        [NonSerialized]
        LineMeshManager selectedInvisibleLineMeshManager = new LineMeshManager();

        [NonSerialized]
        LineMeshManager hoverVisibleLineMeshManager = new LineMeshManager();

        [NonSerialized]
        LineMeshManager hoverInvisibleLineMeshManager = new LineMeshManager();

        [NonSerialized]
        PolygonMeshManager hoverVisiblePolygonMeshManager = new PolygonMeshManager();

        [NonSerialized]
        PolygonMeshManager selectedVisiblePolygonMeshManager = new PolygonMeshManager();

        [NonSerialized]
        int outlineGeneration = -1;

        [NonSerialized]
        int prevHoverSurfaceIndex = -1;

        [NonSerialized]
        int prevHoverBrushNodeID = CSGNode.InvalidNodeID;
        ...
        void DrawOutlines()
        {
            ...
            {
                var visibleInnerColor = ColorSettings.PointInnerStateColor[
                    (int)SelectState.Selected
                ];
                var visibleOuterColor = ColorSettings.SurfaceOuterStateColor[
                    (int)SelectState.Selected
                ];
                var visibleOuterline = ColorSettings.MeshEdgeOutline;
                var invisibleInnerColor = ColorSettings.PointInnerStateColor[
                    (int)SelectState.Selected
                ];
                var invisibleOuterColor = ColorSettings.SurfaceOuterStateColor[
                    (int)SelectState.Selected
                ];
                var invisibleOuterline = ColorSettings.MeshEdgeOutline;
                var surfaceColor = ColorSettings.SurfaceTriangleStateColor[
                    (int)SelectState.Selected
                ];
                ...
                for (int t = 0; t < this.brushTargets.Length; t++)
                {
                    ...
                    for (int s = 0; s < surfaceState.surfaceSelectState.Length; s++)
                    {
                        ...
                        if (
                            texGen_index < 0
                            || texGen_index >= surfaceState.surfaceSelectState.Length
                        )
                        {
                            continue;
                        }
                ...
                {
                    outlines = BrushOutlineManager.GetSurfaceOutlines(
                        foundSelectedSurfaces.ToArray()
                    );
                ...
                if (outlines != null && outlines.Length > 0)
                {
                    ...
                    CSGRenderer.DrawSurfaceOutlines(
                        selectedVisibleLineMeshManager,
                        selectedInvisibleLineMeshManager,
                        selectedVisiblePolygonMeshManager,
                        outlines,
                        foundTransformations.ToArray(),
                        visibleInnerColor,
                        visibleOuterColor,
                        visibleOuterline,
                        invisibleInnerColor,
            ...
            )
            {
                ...
                var visibleInnerColor = ColorSettings.SurfaceInnerStateColor[
                    (int)SelectState.Hovering
                ];
                var visibleOuterColor = ColorSettings.SurfaceOuterStateColor[
                    (int)SelectState.Hovering
                ];
                var visibleOuterline = ColorSettings.MeshEdgeOutline;
                var invisibleInnerColor = ColorSettings.SurfaceInnerStateColor[
                    (int)SelectState.Hovering
                ];
                var invisibleOuterColor = ColorSettings.SurfaceOuterStateColor[
                    (int)SelectState.Hovering
                ...
                if (outlines != null && outlines.Length > 0)
                {
                    ...
                    CSGRenderer.DrawSurfaceOutlines(
                        hoverVisibleLineMeshManager,
                        hoverInvisibleLineMeshManager,
        ...
        void CreateControlIDs()
        {
            ...
            surfaceSelectPaintControl = GUIUtility.GetControlID(
                surfaceSelectPaintControlToolHash,
                FocusType.Keyboard
            );
            textureCopyDragControlID = GUIUtility.GetControlID(
                textureCopyDragControlToolHash,
                FocusType.Keyboard
            );
            ...
            for (int t = 0; t < brushTargets.Length; t++)
            {
                ...
                for (int s = 0; s < surfaceState.surfaceSelectState.Length; s++)
                {
                    surfaceState.surfaceControlID[s] = GUIUtility.GetControlID(
                        textureSurfaceControlToolHash + t,
                        FocusType.Keyboard
                    );
                }
            }
            ...
            try
            {
                ...
                switch (Event.current.type)
                {
                    ...
                    case EventType.MouseDrag:
                    {
                        ...
                        if (
                            SceneQueryUtility.FindUnityWorldIntersection(
                                camera,
                                Event.current.mousePosition,
                                out gameObject
                            )
                        )
                        {
                            if (
                                ...
                                && !mouseIsDragging
                                && SelectionUtility.CanHandleSceneClick()
                            )
                            {
                                if (gameObject == null)
                        ...
                        if (hoverOnTarget == -1 || hoverOnSurfaceIndex == -1)
                        {
                            if (Event.current.type == EventType.MouseUp && !mouseIsDragging)
                            {
                        ...
                        int newControlID = surfaceStates[hoverOnTarget].surfaceControlID[
                            hoverOnSurfaceIndex
                        ];
                        if (newControlID <= 0)
                        {
                        ...
                        {
                            var brush = brushTargets[hoverOnTarget];
                            rotationCircle.Initialize(
                                brush.GetComponent<Transform>(),
                                brush.Shape.Surfaces[hoverOnSurfaceIndex],
                                surfaceStates[hoverOnTarget].rayIntersectionWorldPoint
                            );
                        }
                        else if (Tools.current == Tool.Move)
                        {
                            var brush = brushTargets[hoverOnTarget];
                            InternalCSGModelManager.SetBrushMeshSurfaces(brush);
                            snappingOffset = SurfaceUtility.ConvertTextureToModelSpace(
                                brush,
                                hoverOnSurfaceIndex,
                                MathConstants.zeroVector2
                            );
                    ...
                    case EventType.KeyDown:
                    {
                        ...
                        if (Keys.CopyMaterialTexGen.IsKeyPressed())
                        {
                            if (dragMode == DragMode.None)
                                dragMode = DragMode.TextureCopy;
                            Event.current.Use();
                            break;
                        }
                    ...
                    case EventType.Repaint:
                    {
                        ...
                        if (currentControl == -1 || currentControl == surfaceSelectPaintControl)
                            DrawOutlines();
                        break;
                    }
                    /*
                    ...
                    */
                    case EventType.Layout:
                    {
                        ...
                        if (hoverControl == -1)
                        {
                            ...
                            )
                            {
                                ...
                                if (!ArrayUtility.Contains(targets, brush))
                                {
                                    if (
                                        hoverOnlyTarget > 0
                                        && hoverOnlyTarget < surfaceStates.Length
                                    )
                                    {
                                        ...
                                        for (
                                            int s = 0;
                                            s < surfaceState.surfaceSelectState.Length;
                                            s++
                                        )
                                        {
                                            if (
                                                (
                                                    surfaceState.surfaceSelectState[s]
                                                    & SelectState.Selected
                                                ) == SelectState.Selected
                                            )
                                                have_selected = true;
                            ...
                            for (int t = 0; t < brushTargets.Length; t++)
                            {
                                ...
                                if (
                                    intersection == null
                                    || intersection.brushNodeID != brush.brushNodeID
                                )
                                {
                                    surfaceState.rayIntersectionWorldPoint =
                                        MathConstants.zeroVector3;
                                    surfaceState.rayIntersectionSurfacePoint =
                                        MathConstants.zeroVector3;
                                    continue;
                                }
                            ...
                            for (int t = 0; t < brushTargets.Length; t++)
                            {
                                ...
                                for (int s = 0; s < surfaceState.surfaceSelectState.Length; s++)
                                {
                                    ...
                                    HandleUtility.AddControl(
                                        surfaceControlID,
                                        (closest_brush == t && closest_surface == s)
                                            ? 0.0f
                        ...
                        for (int t = 0; t < brushTargets.Length; t++)
                        {
                            ...
                            {
                                if (
                                    oldSurfaceStates.Length < surfaceState.surfaceSelectState.Length
                                )
                                    oldSurfaceStates = new SelectState[
                                        surfaceState.surfaceSelectState.Length
                                    ];
                                Array.Copy(
                                    surfaceState.surfaceSelectState,
                                    oldSurfaceStates,
                                    surfaceState.surfaceSelectState.Length
                                );
                            }
                            ...
                            if (nearestControl != -1)
                            {
                                ...
                                if (newEditMode == EditMode.None)
                                {
                                    for (int s = 0; s < surfaceState.surfaceSelectState.Length; s++)
                                    {
                                        ...
                                        {
                                            newEditMode = SetHoverOn(
                                                EditMode.TranslateSurface,
                                                t,
                                                s
                                            );
                                            surfaceState.surfaceSelectState[s] |=
                            ...
                            if (!repaint)
                            {
                                ...
                                {
                                    if (
                                        surfaceState.surfaceSelectState[p] != oldSurfaceStates[p]
                                        || oldSurfaceStates[p] == SelectState.None
                                    )
                                        repaint = true;
                                }
                            }
                        ...
                        if (
                            hoverOnTarget >= 0
                            && hoverOnSurfaceIndex >= 0
                            && surfaceStates.Length > hoverOnTarget
                            && surfaceStates[hoverOnTarget].surfaceSelectState.Length
                                > hoverOnSurfaceIndex
                        )
                        {
                            if (
                                (
                                    surfaceStates[hoverOnTarget].surfaceSelectState[
                                        hoverOnSurfaceIndex
                                    ] & SelectState.Selected
                                ) != SelectState.Selected
                            )
                        ...
                        for (int s = 0; s < surfaceControls.Length; s++)
                        {
                            ...
                            switch (eventType)
                            {
                                case EventType.MouseDrag:
                                {
                                    ...
                                    {
                                        Debug.LogWarning(
                                            "Please change the Unity Tool to Move or Rotate"
                                        );
                                    }
                                    break;
                                }
                                ...
                                case EventType.Repaint:
                                {
                                    ...
                                    {
                                        if (
                                            editMode == EditMode.TranslateSurface
                                            && mouseIsDragging
                                        )
                                        {
