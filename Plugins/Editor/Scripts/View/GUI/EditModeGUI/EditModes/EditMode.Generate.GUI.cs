    ...
    internal sealed partial class EditModeGenerateGUI
    {
        ...
        static void OnGUIContents(EditModeGenerate tool, bool isSceneGUI, float height)
        {
            if (!isSceneGUI)
            {
                ...
                tool.BuilderMode = (ShapeMode)
                    CSG_EditorGUIUtility.ToolbarWrapped(
                        (int)tool.BuilderMode,
                        ref shapeModeRects,
                        out shapeModeBounds,
                        csg_skin.shapeModeNames,
                        tooltips: CSG_GUIStyleUtility.shapeModeTooltips,
                        yOffset: height,
                        areaWidth: EditorGUIUtility.currentViewWidth - 4
                    );
