    ...
    internal sealed partial class EditmodePlaceGUI
    {
        ...
        )
        {
            bool operations_enabled = (
                ...
                && filteredSelection.NodeTargets.Length
                    == (
                        filteredSelection.BrushTargets.Length
                        + filteredSelection.OperationTargets.Length
                    )
            );
            EditorGUI.BeginDisabledGroup(!operations_enabled);
            {
                ...
                if (tool != null)
                {
                    ...
                    else if (filteredSelection.OperationTargets.Length > 0)
                    {
                    ...
                    ) // only operations
                    {
                        ...
                        for (int i = 1; i < filteredSelection.OperationTargets.Length; i++)
                        {
                            if (
                                ...
                                && passThrough.Value
                                    != filteredSelection.OperationTargets[i].PassThrough
                            )
                            {
                                passThrough = null;
                                break;
                        ...
                        if (CSG_EditorGUIUtility.PassThroughButton(passThroughValue, ptMixedValues))
                        {
                            Undo.RecordObjects(
                                ...
                                "Changed CSG operation of nodes"
                            );
        ...
        static void OnGUIContents(bool isSceneGUI, EditModePlace tool)
        {
            ...
            var displayNewCenter = GridUtility.CleanPosition(
            ...
            GUILayout.BeginVertical(CSG_GUIStyleUtility.ContentEmpty);
            {
                        ...
                        EditorGUI.BeginDisabledGroup(!tool.HaveSelection);
                        {
                            if (Tools.current == Tool.Move)
                            {
                                EditorGUI.BeginDisabledGroup(
                                    defaultMoveOffset.sqrMagnitude
                            ...
                            else if (Tools.current == Tool.Rotate)
                            {
                                ...
                                );
                                {
                                    if (GUILayout.Button(RotateByOffsetContent))
                                    {
                                        tool.RotateByOffset(
                                            Quaternion.Euler(
                                                RealtimeCSG.CSGSettings.DefaultRotateOffset
                                            )
                                        );
                                    }
                ...
                if (Tools.current == Tool.Move)
                {
                    var doubleFieldOptions = isSceneGUI
                        ? MaxWidth150
                        : CSG_GUIStyleUtility.ContentEmpty;
                    EditorGUI.BeginDisabledGroup(!tool.HaveSelection);
                    {
                        EditorGUI.BeginChangeCheck();
                        {
                            ...
                            defaultMoveOffset = CSG_EditorGUIUtility.DistanceVector3Field(
                                defaultMoveOffset,
                                false,
                                doubleFieldOptions
                            );
                ...
                else if (Tools.current == Tool.Rotate)
                {
                    var doubleFieldOptions = isSceneGUI
                        ...
                        : CSG_GUIStyleUtility.ContentEmpty;
                    EditorGUI.BeginDisabledGroup(
                        Tools.pivotMode == PivotMode.Center || !tool.HaveSelection
                    );
                    {
                        EditorGUI.BeginChangeCheck();
                        {
                            GUILayout.Label(RotationCenterContent);
                            defaultRotateOffset = CSG_EditorGUIUtility.EulerDegreeField(
                                defaultRotateOffset
                            );
                        }
                        if (EditorGUI.EndChangeCheck())
                        ...
                        EditorGUI.BeginChangeCheck();
                        {
                            ...
                            displayNewCenter = CSG_EditorGUIUtility.DistanceVector3Field(
                                displayNewCenter,
                                false,
                                doubleFieldOptions
                            );
