...
namespace RealtimeCSG
{
    internal sealed class EditModeManager : ScriptableObject
    {
        ...
        [NonSerialized]
        FilteredSelection filteredSelection = new FilteredSelection();

        [SerializeField]
        ToolEditMode editMode = ToolEditMode.Place;

        [SerializeField]
        IEditMode activeTool = null;

        static IEditMode[] brushTools = null;

        public static FilteredSelection FilteredSelection
        {
            get
            {
                InitTools();
                return instance.filteredSelection;
            }
        }
        public static IEditMode ActiveTool
        {
            get
            {
                InitTools();
                return instance.activeTool;
            }
        }
        ...
        public static IEditMode CurrentTool
        {
            get
            {
                ...
                if (editMode < firstEditMode || editMode > lastEditModes)
                    return brushTools[0];
        ...
        const HideFlags scriptableObjectHideflags =
        ...
        public static void UpdateSelection(bool forceUpdate = false)
        {
            ...
            if (
                !instance.filteredSelection.UpdateSelection(selectedNodes, selectedOthers)
                && !forceUpdate
            )
            {
                return;
        ...
        {
            if (instance.activeTool != null && instance.activeTool.DeselectAll())
                return true;
            return false;
        }
        ...
        static bool GetTargetSelection(ref HashSet<CSGNode> nodes, ref HashSet<Transform> others)
        {
            ...
            foreach (var gameObject in Selection.gameObjects)
            {
                ...
                if (
                    node
                    && node.enabled
                    && (node.hideFlags & HideFlags.HideInInspector) == HideFlags.None
                )
                    nodes.Add(node);
                else
        ...
        static void InitTargets()
        {
            ...
            {
                if (
                    instance.filteredSelection.NodeTargets != null
        ...
        public static Vector3 SnapPointToGrid(
            Camera camera,
            Vector3 point,
            CSGPlane plane,
        ...
        )
        {
            ...
            switch (activeSnappingMode)
            {
                ...
                case SnapMode.GridSnapping:
                {
                    snappedPoint =
                        snappedPoint
                        + RealtimeCSG.CSGGrid.ForceSnapDeltaToGrid(
                            camera,
                            MathConstants.zeroVector3,
                            snappedPoint
                        );
                    snappedPoint = RealtimeCSG.CSGGrid.PointFromGridSpace(
                        RealtimeCSG.CSGGrid.CubeProject(
                            camera,
                            RealtimeCSG.CSGGrid.PlaneToGridSpace(plane),
                            RealtimeCSG.CSGGrid.PointToGridSpace(snappedPoint)
                        )
                    );
                    ...
                    if (!ignoreAllBrushes)
                        return GridUtility.SnapToWorld(
                            camera,
                            plane,
                            point,
                            snappedPoint,
        ...
        public static Vector3 SnapPointToRay(
            Camera camera,
            Vector3 point,
            Ray ray,
            ref List<Vector3> snappingEdges,
        ...
        )
        {
            ...
            if (doGridSnapping)
            {
                var delta = RealtimeCSG.CSGGrid.ForceSnapDeltaToRay(
                    camera,
                    ray,
                    MathConstants.zeroVector3,
                    snappedPoint
            ...
            switch (Event.current.type)
            {
                case EventType.KeyDown:
                {
                    if (
                        ...
                        && !instance.generateMode
                    )
                    {
                        var generateBrushTool =
                            brushTools[(int)ToolEditMode.Generate] as EditModeGenerate;
                    ...
                    else if (Keys.SwitchToObjectEditMode.IsKeyPressed())
                    {
                        Event.current.Use();
                        return true;
                    }
                    else if (Keys.SwitchToGenerateEditMode.IsKeyPressed())
                    {
                        Event.current.Use();
                        return true;
                    }
                    else if (Keys.SwitchToMeshEditMode.IsKeyPressed())
                    {
                        Event.current.Use();
                        return true;
                    }
                    else if (Keys.SwitchToClipEditMode.IsKeyPressed())
                    {
                        Event.current.Use();
                        return true;
                    }
                    else if (Keys.SwitchToSurfaceEditMode.IsKeyPressed())
                    {
                        Event.current.Use();
                        return true;
                    }
                ...
                {
                    if (
                        (GUIUtility.hotControl == 0 && Keys.FreeBuilderMode.IsKeyPressed())
                        || Keys.CylinderBuilderMode.IsKeyPressed()
                        || Keys.SphereBuilderMode.IsKeyPressed()
                        || Keys.BoxBuilderMode.IsKeyPressed()
        ...
        static List<EditModeToolWindowSceneGUI> currentEditorWindows =
            new List<EditModeToolWindowSceneGUI>();
        ...
        public static void OnSceneGUI(SceneView sceneView)
        {
                ...
                if (instance.activeTool != null)
                {
                    if (RealtimeCSG.CSGSettings.EnableRealtimeCSG)
                    {
                        ...
                        var sceneRect = new Rect(
                            ...
                            0,
                            sceneSize.x,
                            sceneSize.y - ((CSG_GUIStyleUtility.BottomToolBarHeight + 4) + 17)
                        );
                    ...
                    else
                    {
                        if (Event.current.type == EventType.Repaint)
                        {
                            ...
                            for (int i = 0; i < brushes.Length; i++)
                            {
                                ...
                                var brushTransformation = brush
                                    .compareTransformation
                                    .localToWorldMatrix;

                                wireframes.Add(
                                    BrushOutlineManager.GetBrushOutline(brushes[i].brushNodeID)
                                );
                            ...
                            if (wireframes.Count > 0)
                            {
                                CSGRenderer.DrawSelectedBrushes(
                                    instance.zTestLineMeshManager,
                                    instance.noZTestLineMeshManager,
                                    wireframes.ToArray(),
            ...
            if (RealtimeCSG.CSGSettings.EnableRealtimeCSG)
            {
                if (
                    sceneView
                    && sceneWindowType != EventType.Used
                    && (
        ...
        public static void GenerateFromSurface(
            Camera camera,
            CSGBrush cSGBrush,
            CSGPlane polygonPlane,
            Vector3 direction,
            Vector3[] points,
            int[] pointIndices,
            uint[] smoothingGroups,
            bool drag,
            CSGOperationType forceDragSource,
            bool autoCommitExtrusion
        )
        {
            ...
            generateBrushTool.GenerateFromPolygon(
                camera,
                cSGBrush,
                polygonPlane,
                direction,
        ...
        public static Transform[] CloneTargets(SetTransformation setTransform = null)
        {
            ...
            for (int i = 0; i < instance.filteredSelection.NodeTargets.Length; i++)
            {
                ...
                if (originalTransform.parent != null)
                {
                    ...
                    newTransform.name = GameObjectUtility.GetUniqueNameForSibling(
                        originalTransform.parent,
                        originalTransform.name
                    );
                }
                ...
                foreach (var childBrush in childBrushes)
                {
                    for (int g = 0; g < childBrush.Shape.TexGens.Length; g++)
                    {
                        ...
                        if (
                            !uniqueSmoothingGroups.TryGetValue(
                                smoothingGroup,
                                out newSmoothingGroup
                            )
                        )
                        {
                ...
                Undo.RegisterCreatedObjectUndo(
                    newTargets[i],
                    "Created clone of " + originalGameObject.name
                );
            }
