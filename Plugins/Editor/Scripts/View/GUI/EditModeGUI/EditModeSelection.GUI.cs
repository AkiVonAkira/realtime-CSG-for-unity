...
namespace RealtimeCSG
{
    internal sealed partial class EditModeSelectionGUI
    {
        ...
        public static float OnEditModeSelectionGUI()
        {
            ...
            Rect editModeBounds;
            var newEditMode = (ToolEditMode)
                CSG_EditorGUIUtility.ToolbarWrapped(
                    (int)EditModeManager.EditMode,
                    ref editModeRects,
                    out editModeBounds,
                    CSG_GUIStyleUtility.brushEditModeContent,
                    CSG_GUIStyleUtility.brushEditModeTooltips
                );
        ...
        {
            GUILayout.Window(
                SceneViewBrushEditorOverlayHash,
                windowRect,
                windowFunction,
                string.Empty,
                sceneViewOverlayTransparentBackground,
        ...
        static void HandleSceneGUI(int id)
        {
            ...
            var originalSkin = GUI.skin;
            {
                ...
                var viewRect = new Rect(
                    4,
                    0,
                    sceneView.rootVisualElement.contentRect.width,
                    sceneView.rootVisualElement.contentRect.height
                        - (CSG_GUIStyleUtility.BottomToolBarHeight + 4)
                );
#else
                var viewRect = new Rect(
                    4,
                    ...
                    sceneView.position.height - (CSG_GUIStyleUtility.BottomToolBarHeight + 4)
                );
#endif
        ...
        static void OnEditModeSelectionSceneGUI()
        {
            CSG_GUIStyleUtility.InitStyles();
            if (
                CSG_GUIStyleUtility.brushEditModeNames == null
        ...
        public static void ShowRealtimeCSGDisabledMessage()
        {
        ...
        public static void OnInspectorGUI(Editor editor, UnityEngine.Object[] targets)
        {
            ...
            try
            {
                ...
                PrefabDestinationAlignment? prefabDestinationAlignment =
                ...
                if (targetNodes.Length > 0)
                {
                    ...
                    for (int i = 1; i < targetNodes.Length; i++)
                    {
                        ...
                        var currentPrefabDestinationAlignment = targetNodes[
                            i
                        ].PrefabDestinationAlignment;
                        if (isPrefab.HasValue && isPrefab.Value != currentIsPrefab)
                            isPrefab = null;
                        if (
                            prefabBehaviour.HasValue
                            && prefabBehaviour.Value != currentPrefabBehaviour
                        )
                            prefabBehaviour = null;
                        if (
                            prefabSourceAlignment.HasValue
                            && prefabSourceAlignment.Value != currentPrefabSourceAlignment
                        )
                            prefabSourceAlignment = null;
                        if (
                            prefabDestinationAlignment.HasValue
                            && prefabDestinationAlignment.Value != currentPrefabDestinationAlignment
                        )
                ...
                GUILayout.BeginVertical(GUI.skin.box);
                {
                    ...
                    EditorGUI.indentLevel++;
                    {
                        if (isPrefab.HasValue && isPrefab.Value)
                        {
                            ...
                            var prefabBehavour = prefabBehaviour.HasValue
                                ? prefabBehaviour.Value
                                : PrefabInstantiateBehaviour.Reference;
                            EditorGUI.BeginChangeCheck();
                            {
                                prefabBehavour = (PrefabInstantiateBehaviour)
                                    EditorGUILayout.EnumPopup(
                                        PrefabInstantiateBehaviourContent,
                                        prefabBehavour
                                    );
                        ...
                        var destinationAlignment = prefabDestinationAlignment.HasValue
                            ? prefabDestinationAlignment.Value
                            : PrefabDestinationAlignment.AlignToSurface;
                        EditorGUI.BeginChangeCheck();
                        {
                            destinationAlignment = (PrefabDestinationAlignment)
                                EditorGUILayout.EnumPopup(
                                    DestinationAlignmentContent,
                        ...
                        var sourceAlignment = prefabSourceAlignment.HasValue
                            ? prefabSourceAlignment.Value
                            : PrefabSourceAlignment.AlignedFront;
                        EditorGUI.BeginChangeCheck();
                        {
                            sourceAlignment = (PrefabSourceAlignment)
                                EditorGUILayout.EnumPopup(SourceAlignmentContent, sourceAlignment);
                ...
                if (targetModels.Length == 0)
                {
                    ...
                    foreach (var operation in targetOperations)
                    {
                        ...
                        else if (handleAsOne.Value != operation.HandleAsOne)
                        {
                            selMixedValues = true;
                        }
                    }
                    GUILayout.BeginVertical(GUI.skin.box);
                    {
                        ...
                        if (targetBrushes.Length == 0 && targetOperations.Length > 0) // only operations
                        {
                            ...
                            {
                                if (
                                    passThrough.HasValue
                                    && passThrough.Value != targetOperations[i].PassThrough
                                )
                                {
                                    passThrough = null;
                                    break;
                            ...
                            if (
                                CSG_EditorGUIUtility.PassThroughButton(
                                    passThroughValue,
                                    ptMixedValues
                                )
                            )
                        ...
                        EditorGUI.BeginChangeCheck();
                        {
                            operationType = CSG_EditorGUIUtility.ChooseOperation(
                                operationType,
                                opMixedValues
                            );
                        }
                    ...
                    if (targetOperations.Length == 0 && targetModels.Length == 0)
                    {
                        ...
                        if (targetBrushes.Length == 1)
                        {
                            ...
                            {
                                EditorGUI.indentLevel++;
                                OpenSurfaces = EditorGUILayout.Foldout(
                                    OpenSurfaces,
                                    SurfacesContent
                                );
                                EditorGUI.indentLevel--;
                                if (OpenSurfaces)
                                {
                                    ...
                                    for (int t = 0; t < texGens.Length; t++)
                                    {
                                        ...
                                        EditorGUI.BeginChangeCheck();
                                        {
                                            ...
                                            {
                                                GUILayout.Space(4);
                                                material = CSG_EditorGUIUtility.MaterialImage(
                                                    material
                                                );
                                                GUILayout.Space(2);
                    ...
                    if (handleAsOne.HasValue)
                    {
                        EditorGUI.BeginChangeCheck();
                        {
                            ...
                            handleAsOne = EditorGUILayout.Toggle(
                                HandleAsOneLabel,
                                handleAsOne.Value
                            );
                        }
                        if (EditorGUI.EndChangeCheck())
                        {
                            Undo.RecordObjects(
                                targetNodes,
                                "Changed CSG operation 'Handle as one object'"
                            );
                            foreach (var operation in targetOperations)
                            {
