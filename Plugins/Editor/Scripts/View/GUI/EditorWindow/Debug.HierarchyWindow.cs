...
namespace RealtimeCSG
{
    ...
    sealed class CSGHierarchyView : EditorWindow
    {
        ...
        static void Create()
        {
            window = (CSGHierarchyView)
                EditorWindow.GetWindow(typeof(CSGHierarchyView), false, "CSG Hierarchy");
        ...
        {
            public StackItem(CSGTreeNode[] _children, float _xpos = 0)
            {
                children = _children;
                index = 0;
        ...
        static int GetVisibleItems(
            Dictionary<int, CSGTreeNode[]> sceneHierarchies,
            ref Dictionary<int, bool> openNodes
        )
        {
        ...
        static void AddFoldOuts(
            ref Rect itemRect,
            ref Rect visibleArea,
            CSGTreeNode[] hierarchyItems,
        ...
        static string NameForCoreNode(CSGTreeNode coreNode)
        {
            ...
            var obj = (userID != 0) ? userID.ToObject() : null;
            if (obj == null)
        ...
        )
        {
            ...
            while (currentStackItem.index < currentStackItem.count)
            {
                ...
                if (itemRect.y > visibleArea.yMin)
                {
                    ...
                    {
                        var obj = userID.ToObject();
                        if (!(obj is GameObject))
                        {
                            var mono = (obj as MonoBehaviour);
                            if (mono)
                                userID = mono.gameObject.ToNativeId();
                        }
                        SelectionIdExtensions.SetEntityIdsFromNative(userID);
                    }
                }
                ...
                if (openNodes[nodeID])
                {
                    ...
                    {
                        itemStack.Add(
                            new StackItem(children[i].ChildrenToArray(), itemRect.x + kItemIndent)
                        );
                        goto ContinueOnNextStackItem;
                    }
                }
        ...
        void OnGUI()
        {
            ...
            foreach (var instanceID in SelectionIdExtensions.GetEntityIds())
            {
                var obj = instanceID.ToObject();
                var go = obj as GameObject;
                if (go != null)
                {
                    foreach (var no in go.GetComponents<CSGNode>())
                    {
                        var instanceID_ = no.ToNativeId();
            ...
            {
                if (
                    allNodes[i].Type != CSGNodeType.Tree
            ...
            {
                var obj = selectedInstanceIDs.First().ToObject() as CSGNode;
                if (obj)
                {
                    ...
                    if (brush)
                        nodeID = brush.brushNodeID;
                    if (operation)
                        nodeID = operation.operationNodeID;
                    if (model)
                    ...
                    if (nodeID != CSGNode.InvalidNodeID)
                    {
                        ...
                        GUI.Label(labelArea, "NodeID: " + nodeID);
                        labelArea.y += kItemHeight;
                        GUI.Label(labelArea, "UserID: " + node.UserID);
                        labelArea.y += kItemHeight;
                        GUI.Label(labelArea, "Valid: " + node.Valid);
                        labelArea.y += kItemHeight;
                        GUI.Label(labelArea, "NodeType: " + node.Type);
                        labelArea.y += kItemHeight;
                        GUI.Label(labelArea, "ChildCount: " + node.Count);
                        labelArea.y += kItemHeight;
                        ...
                        if (nodeType != CSGNodeType.Tree)
                        {
                            GUI.Label(
                                labelArea,
                                "Parent: " + node.Parent.NodeID + " valid " + node.Parent.Valid
                            );
                            labelArea.y += kItemHeight;
                            GUI.Label(
                                labelArea,
                                "Model: " + node.Tree.NodeID + " valid " + node.Tree.Valid
                            );
                            labelArea.y += kItemHeight;
                            ...
                            if (nodeType == CSGNodeType.Brush)
                            {
                                ...
                                GUI.Label(labelArea, "Operation: " + op.ToString());
                                labelArea.y += kItemHeight;
                            }
                            else if (nodeType == CSGNodeType.Branch)
                            {
