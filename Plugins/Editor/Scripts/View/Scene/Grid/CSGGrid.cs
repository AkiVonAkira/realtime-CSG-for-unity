    ...
    internal static class CSGGrid
    {
        ...
        static Material GridMaterial
        {
            get
            {
                if (!gridMaterial_)
                {
                    ...
                    gridMaterial_.SetInt(
                        "_ZTest",
                        (int)UnityEngine.Rendering.CompareFunction.LessEqual
                    );
                    gridMaterial_.SetFloat(
                        "_GridLineThickness",
                        1.0f * EditorGUIUtility.pixelsPerPoint
                    );
                    gridMaterial_.SetFloat(
                        "_CenterLineThickness",
                        2.0f * EditorGUIUtility.pixelsPerPoint
                    );
                    gridMaterial_.hideFlags =
        ...
        )
        {
            ...
            switch (axis)
            {
                ...
                {
                    screenPixelSize = Mathf.Min(pixelSizeX, pixelSizeZ);
                    break;
                } //X/Z
                case GridAxis.AxisYZ:
                {
                    screenPixelSize = Mathf.Min(pixelSizeY, pixelSizeZ);
                    break;
                } //Y/Z
                case GridAxis.AxisXY:
                {
                    screenPixelSize = Mathf.Min(pixelSizeX, pixelSizeY);
                    break;
                } //X/Y
            }
        ...
        public static CSGPlane CurrentGridPlane
        {
            get
            {
                UpdateGridOrientation(Camera.current);
                return gridOrientation.gridPlane;
            }
        }
        public static CSGPlane CurrentWorkGridPlane
        {
            get
            {
                UpdateGridOrientation(Camera.current);
                return gridOrientation.gridWorkPlane;
            }
        }
        public static Vector3 CurrentWorkGridCenter
        {
            get
            {
                UpdateGridOrientation(Camera.current);
                return gridOrientation.gridWorkCenter;
            }
        }
        public static Quaternion CurrentWorkGridRotation
        {
            get
            {
                UpdateGridOrientation(Camera.current);
                return gridOrientation.gridWorkRotation;
            }
        }
        public static Vector3 CurrentGridSnapVector
        {
            get
            {
                UpdateGridOrientation(Camera.current);
                return gridOrientation.gridSnapVector;
            }
        }
        ...
        static void OnRender(Camera camera)
        {
            ...
            if (gridOrientation.gridOrtho)
            {
                if (gridOrientation.gridOrthoXVisible)
                    DrawGrid(
                        gridOrientation.gridCamera,
                        gridOrientation.gridCameraPosition,
                        gridOrientation.gridCenter,
                        gridOrientation.gridOrthoXRotation,
                        GridAxis.AxisYZ,
                        gridOrientation.gridOrthoXAlpha,
                        GridMode.Ortho
                    );
                ...
                if (gridOrientation.gridOrthoYVisible)
                    DrawGrid(
                        ...
                        gridOrientation.gridCameraPosition,
                        gridOrientation.gridCenter,
                        gridOrientation.gridOrthoYRotation,
                        GridAxis.AxisXZ,
                        gridOrientation.gridOrthoYAlpha,
                        GridMode.Ortho
                    );
                ...
                if (gridOrientation.gridOrthoZVisible)
                    DrawGrid(
                        ...
                        gridOrientation.gridCameraPosition,
                        gridOrientation.gridCenter,
                        gridOrientation.gridOrthoZRotation,
                        GridAxis.AxisXY,
                        gridOrientation.gridOrthoZAlpha,
                        GridMode.Ortho
                    );
            }
            else
            { //*
                ...
                Vector3 work_forward =
                    gridOrientation.gridWorkRotation * MathConstants.forwardVector3;
                if (
                    ForceGrid
                    && !(
                ...
                )
                {
                    DrawGrid(
                        gridOrientation.gridCamera,
                        gridOrientation.gridCameraPosition,
                        gridOrientation.gridWorkCenter,
                        gridOrientation.gridWorkRotation,
                        GridAxis.AxisXZ,
                        0.75f,
                        GridMode.WorkPlane
                    );
        ...
        public static void UpdateGridOrientation(Camera camera)
        {
            ...
            var camera_position = camera.cameraToWorldMatrix.MultiplyPoint(
                MathConstants.zeroVector3
            );
            var camera_forward = camera.cameraToWorldMatrix.MultiplyVector(
                MathConstants.forwardVector3
            );
            ...
            if (camera.orthographic)
            {
                ...
                Vector3 dots = new Vector3(
                    Mathf.Clamp01(
                        Mathf.Abs(
                            Vector3.Dot(
                                camera_forward,
                                gridOrientation.gridRotation * MathConstants.rightVector3
                            )
                        ) - 0.6f
                    ),
                    Mathf.Clamp01(
                        Mathf.Abs(
                            Vector3.Dot(
                                camera_forward,
                                gridOrientation.gridRotation * MathConstants.upVector3
                            )
                        ) - 0.3f
                    ),
                    Mathf.Clamp01(
                        Mathf.Abs(
                            Vector3.Dot(
                                camera_forward,
                                gridOrientation.gridRotation * MathConstants.forwardVector3
                            )
                        ) - 0.6f
                    )
                ).normalized;
                ...
                gridOrientation.gridPlane = new CSGPlane(
                    gridOrientation.gridWorkRotation,
                    gridOrientation.gridWorkCenter
                );
            }
            ...
            gridOrientation.gridWorkInvRotation = Quaternion.Inverse(
                gridOrientation.gridWorkRotation
            );
            ...
            gridOrientation.gridSnapVector =
                Quaternion.Euler(euler) * RealtimeCSG.CSGSettings.SnapVector;
            var snap_scale =
                Quaternion.Euler(euler)
                * new Vector3(
                    RealtimeCSG.CSGSettings.LockAxisX ? 0 : 1,
        ...
        public static Matrix4x4 ToGridSpaceMatrix()
        {
            return Matrix4x4.TRS(
                MathConstants.zeroVector3,
                gridOrientation.gridWorkInvRotation,
                MathConstants.oneVector3
            );
        }

        public static Matrix4x4 FromGridSpaceMatrix()
        {
            return Matrix4x4.TRS(
                MathConstants.zeroVector3,
                gridOrientation.gridWorkRotation,
        ...
        public static Quaternion ToGridSpaceQuaternion()
        {
            return gridOrientation.gridWorkInvRotation;
        }

        public static Quaternion FromGridSpaceQuaternion()
        {
            return gridOrientation.gridWorkRotation;
        }

        public static Vector3 PointToGridSpace(Vector3 pos)
        {
            return gridOrientation.gridWorkInvRotation * (pos - gridOrientation.gridWorkCenter);
        }

        public static Vector3 VectorToGridSpace(Vector3 pos)
        {
            return gridOrientation.gridWorkInvRotation * pos;
        }

        public static CSGPlane PlaneToGridSpace(CSGPlane p)
        {
            return new CSGPlane(VectorToGridSpace(p.normal), PointToGridSpace(p.pointOnPlane));
        }

        public static Vector3 PointFromGridSpace(Vector3 pos)
        {
            return (gridOrientation.gridWorkRotation * pos) + gridOrientation.gridWorkCenter;
        }

        public static Vector3 VectorFromGridSpace(Vector3 pos)
        {
            return (gridOrientation.gridWorkRotation * pos);
        }

        public static CSGPlane PlaneFromGridSpace(CSGPlane p)
        {
            return new CSGPlane(VectorFromGridSpace(p.normal), PointFromGridSpace(p.pointOnPlane));
        }
        ...
        {
            currentPosition.x =
                Mathf.FloorToInt(currentPosition.x / snapVector.x + 1) * snapVector.x;
            currentPosition.y =
                Mathf.FloorToInt(currentPosition.y / snapVector.y + 1) * snapVector.y;
            currentPosition.z =
                Mathf.FloorToInt(currentPosition.z / snapVector.z + 1) * snapVector.z;
            return currentPosition;
        }
        ...
        {
            return worldPoint
                + SnapDeltaToGrid(camera, MathConstants.zeroVector3, new Vector3[] { worldPoint });
        }

        public static Vector3 ForceSnapDeltaToGrid(
            Camera camera,
            Vector3 worldDeltaMovement,
            Vector3 worldPoint
        )
        {
        ...
        public static Vector3 SnapLocalPointToWorldGridDelta(
            Camera camera,
            Matrix4x4 pointLocalToWorld,
            Matrix4x4 pointWorldToLocal,
            Vector3[] localPoints
        )
        {
            UpdateGridOrientation(camera);
            ...
            var worldToGridLocal =
                Matrix4x4.TRS(-gridOrientation.gridWorkCenter, Quaternion.identity, Vector3.one)
                * ToGridSpaceMatrix();
            var gridLocalToWorld =
                FromGridSpaceMatrix()
                * Matrix4x4.TRS(gridOrientation.gridWorkCenter, Quaternion.identity, Vector3.one);
            ...
            if (localPoints.Length > 1)
            {
                ...
                for (int i = 0; i < localPoints.Length; i++)
                {
                    ...
                    if (
                        float.IsNaN(localPoint.x)
                        || float.IsNaN(localPoint.y)
                        || float.IsNaN(localPoint.z)
                        || float.IsInfinity(localPoint.x)
                        || float.IsInfinity(localPoint.y)
                        || float.IsInfinity(localPoint.z)
                    )
                        continue;
            ...
            else
            {
                ...
                if (
                    float.IsNaN(localGridSpacePoint.x)
                    || float.IsNaN(localGridSpacePoint.y)
                    || float.IsNaN(localGridSpacePoint.z)
                    || float.IsInfinity(localGridSpacePoint.x)
                    || float.IsInfinity(localGridSpacePoint.y)
                    || float.IsInfinity(localGridSpacePoint.z)
                )
        ...
        public static Vector3 SnapDeltaToGrid(
            Camera camera,
            Vector3 worldDeltaMovement,
            Vector3[] worldPoints,
            bool snapToGridPlane = true,
            bool snapToSelf = false
        )
        {
            UpdateGridOrientation(camera);
            ...
            {
                scaleVector.x *=
                    (Mathf.Abs(gridLocalPlane.a) >= 1 - MathConstants.EqualityEpsilon) ? 0 : 1;
                scaleVector.y *=
                    (Mathf.Abs(gridLocalPlane.b) >= 1 - MathConstants.EqualityEpsilon) ? 0 : 1;
                scaleVector.z *=
                    (Mathf.Abs(gridLocalPlane.c) >= 1 - MathConstants.EqualityEpsilon) ? 0 : 1;
            }
            ...
            if (worldPoints.Length > 1)
            {
                ...
                for (int i = 0; i < worldPoints.Length; i++)
                {
                    ...
                    if (snapToGridPlane)
                        localPoint = GeometryUtility.ProjectPointOnPlane(
                            gridLocalPlane,
                            localPoint
                        );
                    if (
            ...
            else
            {
                ...
                if (snapToGridPlane)
                    projectedPoint = GeometryUtility.ProjectPointOnPlane(
                        gridLocalPlane,
                        localGridSpacePoint
                    );

                if (
                    float.IsNaN(projectedPoint.x)
                    || float.IsNaN(projectedPoint.y)
                    || float.IsNaN(projectedPoint.z)
                    || float.IsInfinity(projectedPoint.x)
                    || float.IsInfinity(projectedPoint.y)
                    || float.IsInfinity(projectedPoint.z)
                )
                    gridLocalPoints = new Vector3[0] { };
                else
            ...
            for (int i = 0; i < gridLocalPoints.Length; i++)
            {
                ...
                if (snapToGridPlane)
                    snappedNewPoint = GeometryUtility.ProjectPointOnPlane(
                        gridLocalPlane,
                        snappedNewPoint
                    );
        ...
        public static Vector3 SnapDeltaRelative(
            Camera camera,
            Vector3 worldDeltaMovement,
            bool snapToGridPlane = true
        )
        {
            ...
            if (snapToGridPlane)
                snappedDeltaMovement = GeometryUtility.ProjectPointOnPlane(
                    gridLocalPlane,
                    snappedDeltaMovement
                );
        ...
        public static Vector3 ForceSnapDeltaToRay(
            Camera camera,
            Ray worldRay,
            Vector3 worldDeltaMovement,
            Vector3 worldPoint
        ...
        public static Vector3 SnapDeltaToRayGrid(
            Camera camera,
            Ray worldRay,
            Vector3 worldDeltaMovement,
            Vector3[] worldPoints,
        ...
        )
        {
            ...
            scaleVector.x *=
                (
                    (Mathf.Abs(localLineDir.y) >= 1 - MathConstants.EqualityEpsilon)
                    || (Mathf.Abs(localLineDir.z) >= 1 - MathConstants.EqualityEpsilon)
                )
                    ? 0
                    : 1;
            scaleVector.y *=
                (
                    (Mathf.Abs(localLineDir.x) >= 1 - MathConstants.EqualityEpsilon)
                    || (Mathf.Abs(localLineDir.z) >= 1 - MathConstants.EqualityEpsilon)
                )
                    ? 0
                    : 1;
            ...
            if (worldPoints.Length > 1)
            {
                ...
                for (int i = 0; i < worldPoints.Length; i++)
                {
                    var localPoint = GeometryUtility.ProjectPointOnInfiniteLine(
                        PointToGridSpace(worldPoints[i]),
                        localLineOrg,
                        localLineDir
                    );
                    bounds.Extend(localPoint);
            ...
            else
            {
                localPoints = new Vector3[]
                {
                    GeometryUtility.ProjectPointOnInfiniteLine(
                        PointToGridSpace(worldPoints[0]),
                        localLineOrg,
                        localLineDir
                    ),
                };
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                var newPoint = GeometryUtility.ProjectPointOnInfiniteLine(
                    oldPoint + localDeltaMovement,
                    localLineOrg,
                    localLineDir
                );
                ...
                snappedNewPoint = GridUtility.CleanPosition(
                    GeometryUtility.ProjectPointOnInfiniteLine(
                        snappedNewPoint,
                        localLineOrg,
                        localLineDir
        ...
        public static Vector3 SnapDeltaToRayRelative(
            Camera camera,
            Ray worldRay,
            Vector3 worldDeltaMovement
        )
        ...
        public static bool SetupWorkPlane(
            Camera camera,
            Vector3 worldCenterPoint,
            ref CSGPlane workPlane
        )
        {
        ...
        public static bool SetupRayWorkPlane(
            Camera camera,
            Vector3 worldOrigin,
            Vector3 worldDirection,
            ref CSGPlane outWorkPlane
        )
        {
            if (camera == null || !camera)
                return false;
        ...
        public static Vector3 CubeProject(Camera camera, CSGPlane plane, Vector3 pos)
        {
            ...
            if (
                ...
                || float.IsInfinity(intersection.y)
                || float.IsNaN(intersection.z)
                || float.IsInfinity(intersection.z)
            )
            {
        ...
        public static bool SetForcedGrid(Camera camera, CSGPlane plane)
        {
            if (
                float.IsNaN(plane.a)
                || float.IsInfinity(plane.a)
                || float.IsNaN(plane.b)
                || float.IsInfinity(plane.b)
                || float.IsNaN(plane.c)
                || float.IsInfinity(plane.c)
                || float.IsNaN(plane.d)
                || float.IsInfinity(plane.d)
            ...
            var tangent = Vector3
                .Cross(
                    normal,
                    Vector3.Cross(normal, GeometryUtility.CalculateTangent(normal)).normalized
                )
                .normalized;
