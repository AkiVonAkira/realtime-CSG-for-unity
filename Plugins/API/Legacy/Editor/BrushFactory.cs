...
namespace RealtimeCSG.Legacy
{
    ...
    partial class BrushFactory
    {
        ...
        public static SurfaceDescription CreateSurfaceDescription(
            Surface surface,
            TexGen texGen,
        ...
        )
        {
            ...
            description.surfaceFlags =
                ((texGenFlags & TexGenFlags.WorldSpaceTexture) == 0)
                    ? SurfaceFlags.None
                    : SurfaceFlags.TextureIsInWorldSpace;
        ...
        public static SurfaceLayers CreateSurfaceLayer(TexGen texGen, TexGenFlags texGenFlags)
        {
            ...
            var receiveShadows = !(
                (texGenFlags & TexGenFlags.NoReceiveShadows) == TexGenFlags.NoReceiveShadows
            );
            var castShadows = !(
                (texGenFlags & TexGenFlags.NoCastShadows) == TexGenFlags.NoCastShadows
            );
            var collidable = !((texGenFlags & TexGenFlags.NoCollision) == TexGenFlags.NoCollision);

            layers.layerUsage =
                (renderable ? LayerUsageFlags.Renderable : LayerUsageFlags.None)
            ...
            layers.layerParameter1 =
                (texGen.RenderMaterial) ? texGen.RenderMaterial.ToNativeId() : 0;
            layers.layerParameter2 =
                (texGen.PhysicsMaterial) ? texGen.PhysicsMaterial.ToNativeId() : 0;
        ...
        public static BrushMesh CreateCube(
